...
plot(time,(bn*0.2*0.5),'k');%アイソレータが1mA[10]0の場合 DrKamoの回路で実際には半分になる
hold on;
Tri1=(1:0.2:3.8);
line([Tri1;Tri1],[ones(1,length(Tri1))*(-1);ones(1,length(Tri1))*(-1.5)],'color',[0 0 0],'linewidth',2);
Tri2=(5.5:0.2:8.3);
line([Tri2;Tri2],[ones(1,length(Tri2))*(-1);ones(1,length(Tri2))*(-1.5)],'color',[0 0 0],'linewidth',2);
bTri1=(4.2:0.2:4.8);
line([bTri1;bTri1],[ones(1,length(bTri1))*(-1.25);ones(1,length(bTri1))*(-1.5)],'color',[0.7 0.7 0.7],'linewidth',3);
bTri2=(8.7:0.2:9.3);
line([bTri2;bTri2],[ones(1,length(bTri2))*(-1.25);ones(1,length(bTri2))*(-1.5)],'color',[0.7 0.7 0.7],'linewidth',3);
hold off;
set(gca,'xlim',[0 10]);set(gca,'ylim',[-1.5 1.5]);
set(gca,'Ygrid','on');set(gca,'Xgrid','on');%grid(gca,'minor')


ylabel('[mA]');
%%% xlabel(['FinalMAXmA=' num2str(Fin_MAXmA,'%10.2f') ' FinalRMSmA=' num2str(Fin_RMSmA,'%10.2f')]);
xlabel(['FinalMAXmA=' num2str(Fin_MAXmA*0.5,'%10.2f') ' FinalRMSmA=' num2str(Fin_RMSmA*0.5,'%10.2f')]);% DrKamoの回路で実際には半分になる
...
plot(time,(bn*0.2*0.5),'k','linewidth',2);%アイソレータが1mA[10]0の場合 DrKamoの回路で実際には半分になる
hold on;
Tri1=(1:0.2:3.8);
line([Tri1;Tri1],[ones(1,length(Tri1))*(-1);ones(1,length(Tri1))*(-1.5)],'color',[0 0 0],'linewidth',2);
Tri2=(5.5:0.2:8.3);
line([Tri2;Tri2],[ones(1,length(Tri2))*(-1);ones(1,length(Tri2))*(-1.5)],'color',[0 0 0],'linewidth',2);
bTri1=(4.2:0.2:4.8);
line([bTri1;bTri1],[zeros(1,length(bTri1));ones(1,length(bTri1))*(-0.5)],'color',[0.6 0.6 0.6],'linewidth',2);
bTri2=(8.7:0.2:9.3);
line([bTri2;bTri2],[zeros(1,length(bTri2));ones(1,length(bTri2))*(-0.5)],'color',[0.6 0.6 0.6],'linewidth',2);

[row1,ind1] = find(time>=1 & time<1.004);
plot(time(ind1),  (-1)*sin( 2*pi*500*( time(ind1)-1 )) ,'k','linewidth',0.5);
hold off;

set(gca,'xlim',[1-0.001 1+0.004]);set(gca,'ylim',[-1.5 1.5]);
set(gca,'Ygrid','on');set(gca,'Xgrid','on');%grid on;%grid(gca,'minor')
...
[row2,ind2] = find(time>=5.5 & time<5.504);
plot(time(ind2),  (-1)*sin( 2*pi*500*( time(ind2)-5.5 )) ,'k','linewidth',0.5);
hold off;

set(gca,'xlim',[5.5-0.001 5.5+0.004]);set(gca,'ylim',[-1.5 1.5]);
set(gca,'Ygrid','on');set(gca,'Xgrid','on');%grid on;%grid(gca,'minor')
