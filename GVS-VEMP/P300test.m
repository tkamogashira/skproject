...
%%%%%ここから
[y, fs, nbits] = wavread('m2_ga_100ms.wav');%fs=22050, nbits=16だった。
ao=analogoutput('nidaq','Dev1');
...
set(ao,'HwDigitalTriggerSource','PFI0');%PFI0は１３番端子、もうひとつの端子はD GND(１８番）を使う
set(ao,'TriggerCondition','PositiveEdge');
set(ao,'TriggerConditionValue',3);%High電圧は2.2V-5.25Vで、Low電圧は0-0.8Vのようなので、ここでは仮に3Vにした。Neuropackからのトリガをオシロで見て確認する

set(ao,'TriggerFcn',@outsound);

putsample(ao,0);
outputSignal=y;
putdata(ao,outputSignal);%%% wavの音声データを、FsでwavreadしてoutputSignalにする
start(ao)
