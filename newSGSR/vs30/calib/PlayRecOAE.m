function [recSig, ANOM] = PlayRecOAE(playSig, fltIndex, Atten, DoPlot);
% PlayRecOAE - basic play/record function for OAEs using system II (PD1)
% usage:
%   [recSig, ANOM] = PlayRecOAE(playSig, fltIndex, Atten, DoPlot);
% 
...
% may we remove DAMA buffers after D/A and A/D?
DoDeallot = ~iscell(playSig) & ~isempty(playSig);
if iscell(playSig), playSig = playSig{1}; end

persistent Nplay DBNplay DBNrecord DACchannel
ADCchannel = 1; % A/D
N_LAG = 9;

if isempty(playSig), % repeat previous play/rec
else, % new pay/rec action - prepare from scratch
   Nplay = length(playSig);
   if size(playSig)==size(playSig,1), playSig = playSig.'; end % Nchan x Nplay
   Nplay = length(playSig) + N_LAG;
   Nchan = size(playSig,1);
   if Nchan>1, DACchannel = 0; else, DACchannel = 1; end % stereo or left chan
   DBNplay = ML2dama([playSig zeros(Nchan, N_LAG)]);
   ...
   [nStreams, addSimp, specIB] = PD1routing(DACchannel);
   nStreams(2) = 1; % 1-IB, 1 OB
...
if DACchannel==0, 
   s232('dplay', DBNplay(1), DBNplay(2));
else,
   s232('play',DBNplay);
end
