...
DefParam.envrunavunit  = '#';     %must be '#' or 'ms' ...
DefParam.envrunav      = 1;
%Plot parameters ...
...
if strcmpi(Param.envrunavunit, 'ms'), RunAvStr = sprintf('%.2fms', Param.envrunav);
else, RunAvStr = sprintf('%.2f#Per', Param.envrunav); end 
...
case 0, 
    ...
    if strcmpi(Param.envrunavunit, 'ms'), 
        ...
        if (FFT.DF ~= 0), RunAvTsup = FracPer*(1000/FFT.DF); else, RunAvTsup = NaN; end
        RunAvNsup  = round(RunAvTsup/Param.corbinwidth);
    end 
    
    YDiffCorEnv = runav(abs(hilbert(YDiffCor)), RunAvNsup);
