...
[ Thr, dsThr ] = getThr4Cell(dsBB.FileName, dsBB.icell);
if ~iscell(Thr.str) && isnan(Thr.str)
	CellInfo.thrstr = Thr.str;
else
	CellInfo.thrstr = [ ...
		sprintf('Threshold curve:%s <%s>\n', dsBB.FileName, num2str(Thr.seqnr)) ...
		sprintf('CF: %s @ %s\n', Param2Str(Thr.cf, dsThr.xunit, 0), Param2Str(Thr.thr, 'dB', 0)) ...
		sprintf('SR: %s / ', Param2Str(Thr.sr, 'spk/sec', 1)) ...
		sprintf('BW: %s / ', Param2Str(Thr.bw, dsThr.xunit, 1)) ...
...
if ~isa(ParamList{1}, 'dataset') || ~isequal(ParamList{1}.chan, 0)
	error('First argument should be binaural dataset.'); 
end
dsBB = ParamList{1};

%Checking dataset ...
FileFormat = dsBB.FileFormat; 
[CellNr, TestNr, StimType] = UnRavelID(dsBB.SeqID);
if ~(strcmpi(FileFormat, 'IDF/SPK') && ...
		any(strcmpi(StimType, {'BB', 'FS', 'BFS'}))) && ...	                %IDF/SPK datasets ...
...
CellInfo.filename = upper(dsBB.FileName);
CellInfo.iseq    = dsBB.iSeq;
CellInfo.seqid     = dsBB.SeqID;
CellInfo.cellstr  = sprintf('%s <%s> (#%d)', CellInfo.filename, CellInfo.seqid, CellInfo.iseq);
CellInfo.sys      = dsBB.FileFormat;
...
%Nr of indep vars can be more than one in the case of EDF datasets
FileFormat = dsBB.fileformat;
...
if strcmpi(dsBB.FileFormat, 'IDF/SPK') && ...
		any(strcmpi(dsBB.StimType, {'BFS', 'FS'})) && ...
		(dsBB.nrec ~= dsBB.nsub) && ...
		(datenum(dsBB.Time([3, 2, 1, 4, 5, 6])) < datenum(2002, 8, 1))
	
    warning('SGSR:Critical', 'Incomplete FS-dataset collected before August 2002.');
	...
	if all(isnan(S.BeatFreq(iSubSeqs)))
		...
		StimParam.beatcarfreq = abs(S.BeatFreq(iSubSeqs));
	end
...
Beat   = Param.beat; %beat from StimParam
NFreq  = length(Freq);
Period = 1000./Freq;  %Periods in [ms] 

IndepVal = Param.indepval; %indepval from StimParam
...
calcRate = CalcRATE(dsBB, 'anwin', Param.anwin, 'sort', Param.sort, 'isubseqs', Param.isubseqs);
...
try
	...
	[defaultConvention, binParam] = CheckBinParam(dsBB, 'bb');	
...
% 	
	polarPhColor(fliplr(HIST), 'freqs', fliplr(Freq), 'runav', Param.polarrunav, 'periods', fliplr(Period));
end
...
if strcmpi(Type, 'cc') %Superimpose, composite curve and spectrum ...
    ...
    RasHdl = rasplot(dsBB, Param.isubseqs, AxRAS);
    set(RasHdl, 'Color', 'k');
