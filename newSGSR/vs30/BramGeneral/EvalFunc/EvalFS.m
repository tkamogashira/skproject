...
if ~isempty(UD) & ~isempty(UD.CellInfo) & ~isnan(UD.CellInfo.THRSeq)
    dsTHR = dataset(ds1.filename, UD.CellInfo.THRSeq);
    [CellParam(1).CF, CellParam(1).SA, Thr, BW, Q10] = EvalTHR(dsTHR, 'plot', 'no');
else
    [CellParam(1).CF, CellParam(1).SA, Thr, BW, Q10] = deal(NaN); 
end
CF = CellParam(1).CF;
SR = CellParam(1).SA;
Thr1 = lowerFields(CollectInStruct(CF, SR, Thr, BW, Q10));
CellParam(1).CD = greenwood(CellParam(1).CF);
UD = getuserdata(ds2);
if ~isempty(UD) & ~isempty(UD.CellInfo) & ~isnan(UD.CellInfo.THRSeq)
    dsTHR = dataset(ds2.filename, UD.CellInfo.THRSeq);
    [CellParam(2).CF, CellParam(2).SA, Thr, BW, Q10] = EvalTHR(dsTHR, 'plot', 'no');
else
    [CellParam(2).CF, CellParam(2).SA, Thr, BW, Q10] = deal(NaN); 
end
CF = CellParam(2).CF;
SR = CellParam(2).SA;
Thr2 = lowerFields(CollectInStruct(CF, SR, Thr, BW, Q10));
...
if strcmpi(PlotParam.plot, 'YES')
    if any(strcmpi(PlotParam.plotmode, {'ALL', 'CC'})) 
        ...
        PlotCC(CC, SI, CrossCor, FFT, CellInfo, Param);
    end
    if any(strcmpi(PlotParam.plotmode, {'ALL', 'IPC'}))
        ...
        PlotIPC(IPC, IR, BinCycleHist, CellInfo, Param);
    end
...
% PLOTCC %
%--------%
function PlotCC(ComCurve, SuperImpose, CrossCor, FFT, CellInfo, Param)
...
% PLOTIPC %
%---------%
function PlotIPC(IPC, IR, BinCycleHist, CellInfo, Param)
