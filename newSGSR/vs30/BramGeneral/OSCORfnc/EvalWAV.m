function ArgOut = EvalWAV(varargin)
%EVALWAV calculate and plot relevant information on WAV dataset.
%   EVALWAV(ds, ...) calculates and plots relevant information on dataset
...
%------------------------------------parameters-----------------------------------
DefParam.subseqinput = 'modfreq';  %'modfreq' or 'subseq' ...
DefParam.isubseq     = NaN;        %NaN denotes all subsequences ...
DefParam.cowindow    = 0.050;
...
if ~strcmp(DS.FileFormat, 'SGSR'), error('Dataset doesn''t contain SGSR-data.'); end
if ~any(strcmp(DS.StimType, 'WAV')), error('Dataset doesn''t contain WAV responses.'); end
...
try,
    ...
    else, THR = struct('CF', NaN, 'Thr', NaN, 'SA', NaN, 'BW', NaN, 'Q10', NaN); end   
    %Location ...
    if ~isempty(UD.Experiment), Loc = UD.Experiment.ExposedStr;
    else, Loc = ''; end
    %Rate curve information ...
    if isfield(UD.CellInfo, 'RCNTHR'), RCN.Thr = UD.CellInfo.RCNTHR;
    else, RCN = struct([]); end
catch, 
...
%Check if responses were collected from auditory nerve ...
if ~isempty(Loc) & ~strcmpi(Loc, 'AN'),
    error('Responses in dataset were not collected from AN.');
end

%Caching system caches timewarped model-spiketrains and not the actual calculation
%parameters ...
if strncmpi(Param.cache, 'y', 1),
    SearchParam = structcat(struct('filename', DS.FileName, 'seqid', DS.SeqID), ...
        getfields(Param, {'isubseq', 'cowindow', 'timewarp'}));
    Spt = FromCacheFile(mfilename, SearchParam);
    if ~isempty(Spt), %Retrieve data from cache ...
        fprintf('Data retrieved from cache ...\n');
    else, %Perform actual calculation ...
        Spt = CreateModelSpt(DS, Param);
        %Saving calulated data in cache ...
        ToCacheFile(mfilename, +1000, SearchParam, Spt);
        fprintf('Data written to cache ...\n');
    end
    CALC = CalcMTFs(Spt, Param);
else,
    Spt = CreateModelSpt(DS, Param);
    CALC = CalcMTFs(Spt, Param);
end
...
%Assemble data ...
Data = CollectInStruct(DS, THR, Loc, RCN, Param, CALC);

%If requested display information ...
if strncmpi(Param.plot, 'y', 1), PlotOSCOR(Data, Param); end

%If requested return information ...
if (nargout ~= 0), ArgOut = structtemplate(Data, Template, 'reduction', 'off'); end

%----------------------------------local functions------------------------------------
function Param = CheckParam(Param, ds)

if ~ischar(Param.subseqinput) | ~any(strncmpi(Param.subseqinput, {'modfreq', 'subseq'}, 1)),
    error('Value for property ''subseqinput'' must be ''modfreq'' or ''subseq''.');
end
[IDs, ModFreq] = ExtractModFreq(ds);
if isnan(Param.isubseq), Param.isubseq = 5:ds.nsubrecorded;
elseif ~isnumeric(Param.isubseq) | (ndims(Param.isubseq) > 2) | ~any(size(Param.isubseq) == 1),
    error('Value for property ''isubseq'' must be numeric vector.');
elseif strncmpi(Param.subseqinput, 'modfreq', 1),
    if ~all(ismember(Param.isubseq, denan(ModFreq))), error('Wrong value for property ''isubseq''.');
    else, Param.isubseq = find(ismember(ModFreq, Param.isubseq)); end
elseif ~all(ismember(Param.isubseq, find(~isnan(ModFreq)))), 
    error('Wrong value for property ''isubseq''.'); 
end
[Param.modfreq, idx] = sort(ModFreq(Param.isubseq)'); Param.modfreq = Param.modfreq(:);
Param.isubseq = Param.isubseq(idx); Param.ids = IDs(:);
if ~isnumeric(Param.binfreq) | (length(Param.binfreq) ~= 1) | (Param.binfreq == 0),
    error('Invalid value for property ''binfreq''.'); 
end
if (Param.binfreq < 0), Param.calcbinfreq = abs(Param.binfreq)*Param.modfreq;
else, Param.calcbinfreq = repmat(Param.binfreq, length(Param.isubseq), 1); end
if ~isnumeric(Param.cowindow) | (length(Param.cowindow) ~= 1) | (Param.cowindow <= 0), 
    error('Invalid value for property ''cowindow''.'); 
end
if ~any(strncmpi(Param.timewarp, {'y', 'n'}, 1)), 
...
if ~isnumeric(Param.nbin) | (length(Param.nbin) ~= 1) | (Param.nbin <= 0) | rem(Param.nbin, 4), 
    error('Invalid value for property ''nbin''.'); 
end
if ~isnumeric(Param.rcutoffthr) | (length(Param.rcutoffthr) ~= 1) | (Param.rcutoffthr <= 0), 
    error('Invalid value for property ''rcutoffthr''.'); 
end
if ~isnumeric(Param.zcutoffthr) | (length(Param.zcutoffthr) ~= 1) | (Param.zcutoffthr <= 0), 
    error('Invalid value for property ''zcutoffthr''.'); 
end
if ~any(strncmpi(Param.unwrpalg, {'i', 'n'}, 1)), error('Property ''unwrpalg'' must be ''iter'' or ''normal''.'); end
if ~isnumeric(Param.delayacc) | (length(Param.delayacc) ~= 1) | (Param.delayacc <= 0), 
    error('Invalid value for property ''delayacc''.'); 
end
if ~isnumeric(Param.mxwrapstp) | (length(Param.mxwrapstp) ~= 1) | (Param.mxwrapstp <= 0), 
    error('Invalid value for property ''mxwrapstp''.'); 
end
if ~any(strncmpi(Param.cache, {'y', 'n'}, 1)), error('Property ''cache'' must be ''yes'' or ''no''.'); end
if ~any(strncmpi(Param.plot, {'y', 'n'}, 1)), error('Property ''plot'' must be ''yes'' or ''no''.'); end

%-------------------------------------------------------------------------------------
function [IDs, ModFreq] = ExtractModFreq(ds)

WAVFileNames = ds.WAVdetails.wavFileNames;
N = length(WAVFileNames);
IDs = repmat({''}, N, 1); ModFreq = repmat(NaN, N, 1);
for n = 1:N,
    FileName = WAVFileNames{n};
    IDs{n} = FileName(max(find(FileName == '\'))+1:find(FileName == '.')-1);
    [Value, Count] = sscanf(IDs{n}, 'AB%d', 1);
    if (Count == 1), ModFreq(n) = Value; end
end

%-------------------------------------------------------------------------------------
function Spt = CreateModelSpt(ds, Param)

SptAp = ds.spt(find(strcmpi(Param.ids, 'A')), :);
SptAn = ds.spt(find(strcmpi(Param.ids, '-A')), :);
SptBp = ds.spt(find(strcmpi(Param.ids, 'B')), :);
SptBn = ds.spt(find(strcmpi(Param.ids, '-B')), :);
SptAB = ds.spt(Param.isubseq, :);
...
Spt = [SptApAB; SptAnAB; SptBpAB; SptBnAB];

%Timewarping ...
if strncmpi(Param.timewarp, 'y', 1), 
    warning('Timewarping data. This may take a while.');
    Spt = TimeWarp(Spt, repmat(Param.modfreq, 4, 1)); 
end
...
%-----------------------------------------------------------------------------
function Spt = TimeWarp(Spt, ModFreq)

try StimDirName = [getfield(getdirinfo('oscor'), 'MadStimDir'), '64K\'];
catch, error('Couldn''t get directory information'); end

for SubSeqNr = 1:size(Spt, 1),
    ModSin = GetModWave(StimDirName, 'sin', ModFreq(SubSeqNr));
    ModCos = GetModWave(StimDirName, 'cos', ModFreq(SubSeqNr));
    Spt(SubSeqNr, :) = SubSeqTWarp(Spt(SubSeqNr, :), ModFreq(SubSeqNr), ModSin, ModCos);
end
...
%-----------------------------------------------------------------------------
function SpkTr = SubSeqTWarp(SpkTr, ModFreq, ModSin, ModCos);

t = ModSin(:,1); tmax = max(t);
NCyc = unwrap(atan2(ModSin(:,2), ModCos(:,2)))/(2*pi);
p = polyfit(t, NCyc, 1);

N = size(SpkTr, 2);
for i = 1:N,
    SpkTr{i} = [interp1(t, NCyc, SpkTr{i}(find(SpkTr{i} <= tmax)), 'linear'), ...
        polyval(p, SpkTr{i}(find(SpkTr{i} > tmax)))]*(1000/ModFreq);
end

%-----------------------------------------------------------------------------
function Data = CalcMTFs(Spt, Param)
...
H = CalcPRDH(Spt, 'anwin', Param.anwin, 'nbin', Param.nbin, 'binfreq', repmat(Param.calcbinfreq, 4, 1));

%Combine cyclehistograms ...
PrdH = H.hist(1:Nsub);
for n = 1:Nsub,
    PrdH(n).n = H.hist(n).n + ShiftPRDH(H.hist(n+Nsub).n, mod(0.5*abs(Param.binfreq), 1)) + ...
        ShiftPRDH(H.hist(n+2*Nsub).n, mod(0.75*abs(Param.binfreq), 1)) + ShiftPRDH(H.hist(n+3*Nsub).n, mod(0.25*abs(Param.binfreq), 1));
    PrdH(n).nspk = sum(cat(2, H.hist(n+Nsub.*(0:3)).nspk));
    NCyc = PrdH(n).ncyc; BinDur = PrdH(n).bindur;
    ...
    PrdH(n).ph = atan2(yCo, xCo)/2/pi; if (PrdH(n).ph < 0), PrdH(n).ph = PrdH(n).ph + 1; end
...
Z  = N .* (R.^2);
Rate = 1000 * N /diff(Param.anwin) /Nrep /4;

MaxR = max(R(find(Z >= Param.zcutoffthr)));
if isempty(MaxR), [MaxR, RSatMax, BVar, COVar] = deal(NaN);
else
    RSatMax = RS(find(R == MaxR));         %Cannot use index returned by MAX because intermediate
    BVar = Param.modfreq(find(R == MaxR)); %insignificant points can introduce a shift ...
    
    RCutOff = MaxR/(10^(Param.rcutoffthr/10));
    COVar = Intersection(Param.modfreq, R, RCutOff);
end

HSVar = Intersection(Param.modfreq, Z, Param.zcutoffthr);
[Ph, Delay] = local_unwrap(WPh, Z, Param.calcbinfreq, Param);

IndepVal = Param.modfreq;
Data = CollectInStruct(IndepVal, X, Y, R, MaxR, RSatMax, BVar, COVar, Z, HSVar, WPh, Ph, Delay, N, Rate);
...
%-----------------------------------------------------------------------------
function Xi = Intersection(X, Y, Yc)

idx = min(find(Y < Yc));
if ~isempty(idx) & (idx > 1),
    %Inverse interpolation ...
    Xi = interp1(Y(idx([1 1])-[1, 0]), X(idx([1 1])-[1, 0]), Yc, 'linear');
else, Xi = NaN; end
...
if strncmpi(Param.unwrpalg, 'i', 1), %BRAM's algoritme ...iteratie...
    ...
    Nsig = min(find(Z < Param.zcutoffthr)) - 1; %hoogste significante datapunt...
    if isempty(Nsig), Nsig = length(find(~isnan(Ph))); end
    inc = 1:Nsig;
    
    while (p(1) <  -Param.delayacc) | (p(1) > Param.delayacc)
        n = find( (diff(Ph(inc)) > -Param.mxwrapstp) & (diff(Ph(inc)) < +Param.mxwrapstp) );
        idx = min(find(diff(n) > 1));
        if isempty(idx), n = length(inc); else n = n(idx)+1; end
        p = polyfit(Freq(1:n), Ph(1:n),1); Delay = Delay + p(1);
        ncycles = p(1)./Period;
        Ph  = Ph - mod(ncycles, 1);
        %Alles binnen een cyclus terugbrengen
        Ph(find(Ph > (Center+0.5))) = Ph(find(Ph > (Center+0.5))) -1;
        Ph(find(Ph < (Center-0.5))) = Ph(find(Ph < (Center-0.5))) +1;
    end
elseif strncmpi(Param.unwrpalg, 'n', 1), %MARCEL's algoritme ...
    ...
    Ph(find(Ph > (Center+0.5))) = Ph(find(Ph > (Center+0.5))) -1;
    Ph(find(Ph < (Center-0.5))) = Ph(find(Ph < (Center-0.5))) +1;
else, error('Invalid value for property unwrpalg, should iter or nl.'); end
...
set(Hdl.STB_Stim_RepDur, 'String', ['StimDur: ' int2str(min(Data.DS.burstdur)) 'ms / RepDur: ' int2str(Data.DS.repdur) 'ms']);
set(Hdl.STB_ITD_SPL, 'String', 'SPL: N/A');
...
xlabel('Modulation Freq. (Hz)'); ylabel('Magnitude');
axis([Data.CALC.IndepVal(1) Data.CALC.IndepVal(end) 0 1]);
...
axis([Data.CALC.IndepVal(1) Data.CALC.IndepVal(end) 0 800]);
title('Rayleigh Significance');
xlabel('Modulation Freq. (Hz)'); ylabel('Z = n * R^2');
...
axis([Data.CALC.IndepVal(1) Data.CALC.IndepVal(end) Data.CALC.Ph(1)-0.5 Data.CALC.Ph(1)+0.5]);
...
%-------------------------------------------------------------------------------------
function PlotHists(Hdl, CalcData, Param)

%If more than 15 cyclehistograms need to be plotted, than only 15 of them are plotted ...
N = length(Param.isubseq); if (N <= 15), Nrs = 1:N; else Nrs = round(1:N/15:N); end
...
for i = 1:min([15 N]),
    ...
    txt = {['BF:' num2str(Param.calcbinfreq(n)) 'Hz']; ...
