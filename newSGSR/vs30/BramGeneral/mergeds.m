...
%   - Added support for EDF format datasets 
...
if (nargin == 1) && ischar(varargin{1}) && strcmpi(varargin{1}, 'factory')
    ...
    [DSarray, SubSeqs, Param] = ParseArgs(DefParam, varargin{:});
end
...
if isempty(SubSeqs)
    [Sizes, Data] = MergeReps(DSarray);
else
    [Sizes, Data] = MergeSubSeqs(DSarray, SubSeqs);
end
...
Settings.SessionInfo  = cellfun( ( @(ds) (ds.session) ), num2cell(DSarray), ...
    'UniformOutput', false);
Settings.RecordParams = cellfun( ( @(ds) (ds.recparam) ), num2cell(DSarray), ...
    'UniformOutput', false);

ds = dataset(CollectInStruct(ID, Sizes, Data, Stimulus, Settings), 'convert');

%by abel: merge EDF datasets to an EDF format
if strcmpi({ID.OrigID.FileFormat}, 'EDF');
	ds = ds2edf(ds);
end
...
FirstPropIdx = find(cellfun('isclass', varargin, 'char'),1);
if isempty(FirstPropIdx)
    FirstPropIdx = NArgs+1;
end
...
NDS = length(DSIdx); % Number of datasets
SubSeqsIdx = find(cellfun('isclass', varargin(1:FirstPropIdx-1), 'double'));
NSubSeqs = length(SubSeqsIdx); % Number of subseq ranges entered

try
    DSarray = [varargin{DSIdx}];
catch
    error('Datasets of different stimulus types can never be merged.');
end

%DS = MERGEDS(DS1, SubSeqs1, DS2, SubSeqs2, ..., DSN, SubSeqsN)
if isequal(NDS, NSubSeqs) && isequal(union(DSIdx, SubSeqsIdx), 1:(2*NDS)) ...
        && isequal(DSIdx+1, SubSeqsIdx)
    % conditions check that: * there is a subseq range for every dataset
    %                        * the datasets and subseqs are the first
    %                           2*NDS arguments
    %                        * every dataset is followed by a subseq entry
    ReqInput = varargin(SubSeqsIdx);
    SubSeqs = cell(0);
    if strcmpi(Param.subseqinput, 'indepval')
        for n = 1:NDS, 
            if ~all(ismember(ReqInput{n}, ApplyTolerance(DSarray(n).xval(1:DSarray(n).nrec), Param.numtol)))
                ...
                SubSeqs{n} = find(ismember(ApplyTolerance(DSarray(n).xval(1:DSarray(n).nrec), Param.numtol), ReqInput{n}));
            end
        end
    else
        for n = 1:NDS
            if ~all(ismember(ReqInput{n}, 1:DSarray(n).nrec))
                error('Not all requested subsequences for a dataset exist or were recorded.');
            end
        end
        SubSeqs = ReqInput;
...
elseif isequal(NSubSeqs, 0) && isempty(SubSeqsIdx) && all(ismember(DSIdx, 1:NDS))
    for n = 1:NDS
        IndepVals{n} = deNaN(sort(DSarray(n).indepval));
    end
    if isequal(IndepVals{:})
        SubSeqs = cell(0);
    else
        for n = 1:NDS
            SubSeqs{n} = 1:DSarray(n).nrec;
        end
    end
else
    error('Syntax error');
...
if ~isnumeric(Param.iseq) || (length(Param.iseq) ~= 1) || (Param.iseq < 0)
    error('Invalid value for property iseq.');
end
if ~any(strcmpi(Param.sortsubseqs, {'yes', 'no'}))
    error('Property sortsubseqs must be ''yes'' or ''no''.');
end
if ~any(strcmpi(Param.subseqinput, {'indepval', 'subseq'}))
    error('Property subseqinput must be ''indepval'' or ''subseq''.');
end
if ~isnumeric(Param.numtol) || (length(Param.numtol) ~= 1) || (Param.numtol <= 0)
    error('Invalid value for property numtol.');
end
...
if ~isequal(FileName{:})
    error('Datasets originate from different experiments.');
else
	%By abel: use original filename to remain compatible with getThr4Cell
    %FileName = [FileName{1} '-MERGE'];
	FileName = FileName{1};
end
...
if strcmpi(class(DSarray), 'EDFdataset')
    ...
    if ~isequal(SchName{:})
        error('Datasets have different schema type.');
    else
        StimType = upper(DSarray(1).StimType);
    end
else
    StimType = upper({DSarray.StimType});
    if ~isequal(StimType{:})
        error('Datasets have different stimulus types.');
    else
        StimType = StimType{1};
    end
end
...
Time = round(datevec(now));
Time([1, 3]) = Time([3, 1]);
if inLeuven
    Place = ['Leuven/' compuname]; 
elseif inUtrecht
    Place = ['Utrecht/' compuname]; 
else
    Place = ['**/' compuname];
end
...
OrigID = cellfun(( @(ds) (ds.ID)), num2cell(DSarray));

ID = CollectInStruct(FileName, FileFormat, FullFileName, iSeq, StimType, ...
    iCell, SeqID, Time, Place, Experimenter, OrigID);

%---------------------------------------------------------------------
function [Sizes, Data] = MergeSubSeqs(DSarray, SubSeqs)

%Data = cat(1, DSarray.Data);
Data = cellfun(( @(ds) (ds.Data)),num2cell(DSarray));

Nreps = cellfun('size', {Data.SpikeTimes}, 2);
if (length(unique(Nreps)) ~= 1)
    warning('Reducing number of repetitions for some datasets.');
end
Nrep = min(Nreps);
...
NDS = length(DSarray);
SpikeTimes = cell(0);
%SpikeTimes are saved sequentially, Sorting based on IndepVal is accomplished in MergeIndepVar() 
for n = 1:NDS
    SpikeTimes = [SpikeTimes; Data(n).SpikeTimes(SubSeqs{n}, 1:Nrep)];
end
...
if ~all(cellfun('isempty', {Data.OtherData}))
    warning('Some of the datasets contain data other then spiketimes.');
end
OtherData = cellfun(( @(data) (data.OtherData)), num2cell(Data), ...
    'UniformOutput', false);
...
if (length(unique(cat(2, DSarray.nsub))) ~= 1)
    error('Number of subsequences aren''t equal for all datasets.');
else
    Nsub = DSarray(1).nsub;
end
...
IndepVar = descstruct(cellfun(( @(ds) (ds.Stimulus)),num2cell(DSarray)), ...
    'IndepVar');
Name = {IndepVar.Name};

%By abel: For EDF datasets with mutiple indepvars we need to get the
%indepvar struct by ds.indepvar
if strcmpi({IndepVar.Name}, 'SubSequence')
	IndepVar = cellfun(( @(ds) (ds.indepvar)),num2cell(DSarray));
end
Name = {IndepVar.Name};


if ~isequal(Name{:})
    error('Datasets have different independent variables.'); 
else
    Name = Name{1};
end
...
if isempty(SubSeqs)
    Values = IndepVar(1).Values;
else
    NDS = length(DSarray);
    Values = [];
    for n = 1:NDS
        Values = [Values; reshape(IndepVar(n).Values(SubSeqs{n}), ...
            length(SubSeqs{n}), 1)];
    end
end
uValues = unique(Values);
if ~isequal(sort(Values), uValues)
    warning('Duplicate values for independent variable.');
elseif (length(unique(diff(uValues))) ~= 1)
    warning('Values of independent variable in merged datasets don''t have a fixed increment.');
end

if isempty(SubSeqs) || strcmpi(Param.sortsubseqs, 'yes')
    [Values, SIdx] = sort(Values); 
else
    SIdx = 1:length(Values);
end
...
if (length(unique(cat(2, DSarray.dachan))) ~= 1)
    error('Number of active channels is different for datasets.');
else
    ActiveChan = DSarray(1).dachan;
end

RepDurs = {DSarray.repdur};
if ~isequal(RepDurs{:})
    error('Repetition duration is different for datasets.');
else
    RepDur = DSarray(1).RepDur;
end

BurstDurs = {DSarray.burstdur};
if ~isequal(BurstDurs{:})
    error('Stimulus duration is different for datasets.');
else
    BurstDur = DSarray(1).BurstDur;
end

CarFreqs     = cellfun(( @(ds) (ds.carfreq)), num2cell(DSarray), ...
    'UniformOutput', false);
ModFreqs     = cellfun(( @(ds) (ds.modfreq)), num2cell(DSarray), ...
    'UniformOutput', false);
if isempty(SubSeqs)
    if isEqualIncNaNs(CarFreqs{:})
        ...
        error('Carrier frequency is different for datasets.');
    end
    ...
    if isEqualIncNaNs(ModFreqs{:})
        ...
        error('Modulation frequency is different for datasets.');
    end
else
    NDS = length(SubSeqs);
    ...
    if any(cellfun('size', CarFreqs, 1) > 1)
        CarFreq = [];
        for n = 1:NDS
            CarFreq = [CarFreq; CarFreqs{n}(SubSeqs{n}, :)];
        end
        CarFreq = CarFreq(SIdx, :);
    elseif isEqualIncNaNs(CarFreqs{:})
        CarFreq = CarFreqs{1};
    else
        error('Carrier frequency is different for datasets.');
    end
    ...
    if any(cellfun('size', ModFreqs, 1) > 1)
        ModFreq = [];
        for n = 1:NDS
            ModFreq = [ModFreq; ModFreqs{n}(SubSeqs{n}, :)];
        end
        ModFreq = ModFreq(SIdx, :);
    elseif isEqualIncNaNs(ModFreqs{:})
        ModFreq = ModFreqs{1};
    else
        error('Modulation frequency is different for datasets.');
    end
...
StimParam = descstruct(cellfun(( @(ds) (ds.Stimulus)), num2cell(DSarray)), ...
    'StimParam');
[DiffFNames, DiffVal] = StructArrayCmp(StimParam);
NFields = length(DiffFNames);
NDS = length(DSarray);
if (NFields > 0)
    Nindent  = 5;
    ...
    for n = 1:(NDS-1)
        warnmsg = [warnmsg, Val2Str(DiffVal(:, n)), repmat(', ', NFields, 1)];
    end
...
if ~isempty(C) && ~isequal(C{:})
   ...
   for n = 1:NFields
      ...
      if isstruct(Vals{1})
          if (length(Vals{1}) > 1)
              warning(['Dataset contains structure-array in' ...
                  'StimParam-field. Not comparing these fields.']);
          else
              [FNadd, Vadd] = StructArrayCmp(cat(2, Vals{:})); %Recursion ...
              FN = [FN(:); FNadd];
              V = [V; Vadd];
          end   
      elseif iscell(Vals{1}) && (length(Vals{1}) == 2) ...
              && all(cellfun('isclass', Vals{1}, 'struct'))
          Arg = [];
          for i = 1:length(Vals)
              Arg = [Arg, Vals{i}{1}];
          end
...
for n = 1:N
    ...
    switch ClassType
        case 'double'
            Str = strvcat(Str, mat2str(Vals{n}));
        otherwise
            Str = strvcat(Str, sprintf('<%s>', ClassType));
    end
end
...
if any(~cellfun('isclass', varargin, 'double'))
    boolean = isequal(varargin{:});
elseif ~all(cellfun('isclass', varargin, 'double'))
    boolean = false;
else
    Sz = size(varargin{1});
    ...
    while boolean && (n <= nargin)
        ...
        curNotNaNValues = curArg(~isnan(curArg(:)));
