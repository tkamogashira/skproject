...
%   S = SPECTANA(X, Y, ...) analyses spectrum of digital signal given by vectors
%   X and Y, where X is a linear spaced time-axis (in millisec) and Y is the
%   amplitude of the signal. This function returns a structure S with fields :
%
...
%   To view list of all possible properties and their default value, use 'list'
%   as only property.
%
%   The old interface is still implemented and is invoked if there is more than 
%   one output argument: [XS, YS, YSRunAv, DF] = SPECTANA(X, Y) analyses
%   spectrum of digital signal given by vectors X and Y, where X is a linear
%   spaced time-axis(in millisec) and Y is de amplitude of the signal.
%   This function returns the spectrum as 2 vectors: XS is the
%   frequency-axis(in Hz), YS is de ampiltude of each given frequency in the
%   signal given in dB(maximum A is reference, thus maximum is at 0 dB).
%   The dominant frequency in the signal is given in DF.
%
%   [XS, YS, YSRunAv, DF] = SPECTANA(X, Y, RunAvRange) does the same analysis,
%   but uses a running average of range RunAvRange in Hz, which is returned as
%   the vector YSRunAv, to calculated the DF.
...
if nargout > 1 %OLD VERSION ...
    ...
    if ~any(size(X) == 1) || ~any(size(Y) == 1) || ~isequal(length(X), length(Y))
        error('First two inputs should be vectors of same size.');
    end
    if nargin == 2
        RunAvRange = 0;
    end
    ...
    if (DF < 10)
        %Waarschijnlijk DC aanwezig in signaal ... vanaf 50Hz zoeken naar
        %maximum ...
        DF = getmaxloc(XS, YSRunAv, 0, [50, max(XS)/2]);
    end
    
    if ~iscol
        [XS, YS, YSRunAv] = deal(XS', YS', YSRunAv');
    end
    ...
    if ~any(size(T) == 1) || ~any(size(S) == 1) || ~isequal(length(T), length(S))
        error('First two inputs should be vectors of same size.');
    end
    
    Param = checkproplist(DefParam, varargin{:});
    
    if ~any(strncmpi(Param.runavunit, {'#', 'h'}, 1))
        error('Invalid value for property runavunit.');
    end
    if ~isnumeric(Param.runavrange) || (length(Param.runavrange) ~= 1) || ...
            (Param.runavrange < 0)
        error('Invalid value for property runavrange.');
    end
    if ~isnumeric(Param.timeunit) || (length(Param.timeunit) ~= 1) || ...
            (Param.timeunit <= 0)
        error('Invalid value for property timeunit.');
    end
    if ~isnumeric(Param.bwcutoff) || (length(Param.bwcutoff) ~= 1) || ...
            (Param.bwcutoff >= 0)
        error('Invalid value for property bwcutoff.');
    end
    if ~isnumeric(Param.gdrange) || (length(Param.gdrange) ~= 1) || ...
            (Param.gdrange <= 0)
        error('Invalid value for property gdrange.');
    end
    ...
    Zidx = find(T >= 0, 1 ); %nul op de tijdsas ...
    ...
    if strncmpi(Param.runavunit, 'h', 1)
        RunAvN = Param.runavrange / df;
    else
        RunAvN = Param.runavrange;
    end
    ...
    if (DF < 10)
        %if DC in signal then discard first 50Hz of spectrum ...
        DF = getmaxloc(f, A, RunAvN, [50, max(f)/2]);
    end 
