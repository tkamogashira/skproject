...
if (NParam == 1) & ischar(ParamList{1}) & strcmpi(ParamList{1}, 'factory') %#ok<AND2> %Display list of properties ...
...
if ismember(NParam, Sidx)
    Sidx = Sidx(1:end-1);
end

NS = length(Sidx);
ArgList = ParamList(1:NS*3);
PropList = ParamList(NS*3+1:end);
...
for n = 1:NS
    ...
    if isempty(SName)
        SName = sprintf('(Arg #%d)', n);
    end
    T{n} = Struct2Table(S, SName);
    if ~CheckFieldExpr(XFields{n}, T{n}.FNames)
        error(sprintf('''%s'' is not a valid expression.', XFields{n}));
    end
    if ~CheckFieldExpr(YFields{n}, T{n}.FNames)
        error(sprintf('''%s'' is not a valid expression.', YFields{n}));
    end
end
...
elseif (iscell(Param.cellidfields) & (length(Param.cellidfields) == NS)) & ...
    ...
    error('Invalid value for property cellidfields.');
end
%The property execevalfnc is checked independently ...
if isempty(Param.execevalfnc)
    Param.execevalfnc = repmat({''}, 1, NS);
else
    if ischar(Param.execevalfnc)
        Param.execevalfnc = repmat({Param.execevalfnc}, 1, NS);
    end
    if ~iscellstr(Param.execevalfnc) | (length(Param.execevalfnc) ~= NS)
        error('Property execevalfnc must be character string or a cell-array of strings.');
    end
end
...
for n = 1:NS
    if ~all(ismember(Param.cellidfields{n}, T{n}.FNames))
...
for n = 1:NS
    if ~all(ismember(Param.totalidfields, T{n}.FNames))
...
if ~iscellstr(Param.totalidfields)
    error('Invalid value for property totalidfields.');
end
if ~ischar(Param.info) | ~any(strncmpi(Param.info, {'y', 'n'}, 1))
    error('Wrong value for property info.');
end
if ~ischar(Param.fit) | ~any(strncmpi(Param.fit, {'n', 'l'}, 1))
    error('Wrong value for property fit.');
end
if ~ischar(Param.contour) | ~any(strncmpi(Param.contour, {'y', 'n'}, 1))
    error('Wrong value for property contour.');
end
if ~ischar(Param.gutter) | ~any(strncmpi(Param.gutter, {'y', 'n'}, 1))
    error('Wrong value for property gutter.');
end
if ~ischar(Param.hist) | ~any(strncmpi(Param.hist, {'y', 'n'}, 1))
    error('Wrong value for property hist.');
end
if ~ischar(Param.histstyle) | ~any(strncmpi(Param.histstyle, {'f', 'o'}, 1))
    error('Wrong value for property histstyle.');
end
if ~ischar(Param.histbarcolor) | ~any(strncmpi(Param.histbarcolor, {'t', 'f'}, 1))
    error('Wrong value for property histbarcolor.');
end
if ~isnumeric(Param.histbarwidth) | (length(Param.histbarwidth) ~= 1) | (Param.histbarwidth <= 0)
    error('Wrong value for property histbarwidth.');
end
if ~isnumeric(Param.histnbin) | (length(Param.histnbin) ~= 1) | mod(Param.histnbin, 1)
    error('Wrong value for property histnbin.');
end
if ~ischar(Param.histyunit) | ~any(strcmpi(Param.histyunit, {'#', '%'}))
    error('Value for property histyunit must be ''#'' or ''%''.');
end
...
if ~isnumeric(Param.xlim) | ~isequal(sort(size(Param.xlim)), [1, 2]) | (Param.xlim(1) >= Param.xlim(2))
    error('Wrong value for property xlim.');
end
if ~isnumeric(Param.ylim) | ~isequal(sort(size(Param.ylim)), [1, 2]) | (Param.ylim(1) >= Param.ylim(2))
    error('Wrong value for property ylim.');
end

if strncmpi(Param.gutter, 'y', 1) & strncmpi(Param.hist, 'y', 1)
    error('Property gutter and hist cannot have both the value ''yes''.');
end
...
if ~isempty(DolSidx)
    ...
    FN = cell(1, NFields);
    for n = 1:NFields
        FN{n} = Expr(DolSidx(n, 1)+1:DolSidx(n, 2)-1);
    end
else
    FN = {Expr};
end

if ~all(ismember(FN, FNames))
    return;
else
    boolean = logical(1);
end;
...
%Reorganize data ...
for n = 1:NTables
    P(n) = AssemblePlotData(T{n}, XFields{n}, YFields{n}, Param.cellidfields{n}, Param.execevalfnc{n}, Param);
end
LegNames = ExtractLegNames(P, XFields, YFields);
[P.LegendName] = deal(LegNames{:});
...
for n = 1:NPlots
    ...
    LgdHdl = [LgdHdl, min(LnHdls)];
    PHdls{n} = LnHdls;

    MaxX = max([MaxX; max(P(n).Xplot(:))]);
    MinX = min([MinX; min(P(n).Xplot(:))]);

    MaxY = max([MaxY; max(P(n).Yplot(:))]);
    MinY = min([MinY; min(P(n).Yplot(:))]);
end
...
elseif strncmpi(Param.gutter, 'y', 1)
    ...
    axes(XHistHdl);
    line(AxLim(1:2), [0 0], 'LineStyle', '-', 'Color', 'k', 'Marker', 'none');
    axes(YHistHdl);
    line(AxLim(1:2), [0 0], 'LineStyle', '-', 'Color', 'k', 'Marker', 'none');
...
[Info.NPoints, Info.NCells] = deal(0);
Data = [];
for n = 1:NPlots
...
Hdl = legend(LgdHdl, cat(1, {P.LegendName}));
set(findobj(Hdl, 'type', 'text'), 'fontsize', 8);
axes(AxHdl); %Setting the axis object containing the scatterplot as the current axis ...
axis(AxLim);
xlabel(GetAxisLbl(XFields), 'fontsize', 8);
ylabel(GetAxisLbl(YFields), 'fontsize', 8);
...
Col = GetTableColumn(T, GroupFields{:});
if isnumeric(Col)
    Col = num2cell(Col);
end
...
for n = 1:length(idx)
    ...
    NElem = length(exidx);
    Xplot(1:NElem, n) = XVec;
    Yplot(1:NElem, n) = YVec;
end
...
if isempty(ExecStat)
    ...
    if ~isempty(idx)
        ...
        [IDFNames, IDs] = deal(cell(0));
    end
else
    [isOK, FNames] = CheckFieldExpr(ExecStat, T.FNames);
    if ~isOK
        error(sprintf('''%s'' is not a valid statement.', Param.execevalfnc{n}));
    end
...
if strncmpi(Param.contour, 'y', 1)
    incidx = find(all(~isnan([Xorig, Yorig]), 2));
    Ninc = length(incidx);
    if (Ninc >= 3)
...
for n = 1:N
    ...
    if (length(idx) > 1)
        NewLegendNames{n} = [LegendNames{n}, ' (', XFields{n}, ',', YFields{n}, ')'];
    end
end
...
if strncmpi(Param.histbarcolor, 'f', 1)
    FaceColor = EdgeColor;
else
    FaceColor = 'none';
end
...
DolSidx = find(Expr == '$');
if isempty(DolSidx)
    V = GetTableColumn(T, Expr);
else
    DolSidx = reshape(DolSidx, 2, length(DolSidx)/2)';

    NFields = size(DolSidx, 1); FNames = cell(1, NFields);
    for n = 1:NFields
        FNames{n} = Expr(DolSidx(n, 1)+1:DolSidx(n, 2)-1);
    end
    for n = 1:NFields
        Expr = strrep(Expr, ['$' FNames{n} '$'], '%s');
    end
    ...
    D = [];
    ColNr = 1;
    VNames = cell(1, NFields);
    for n = 1:NFields
        Col = GetTableColumn(T, FNames{n});
        if ~isnumeric(Col)
            error(sprintf('Field ''%s'' cannot be used in expressions.', FNames{n}));
        end
        NCols = size(Col, 2);
        idx = ColNr:(ColNr+NCols-1);
        D(:, idx) = Col;
        VNames{n} = ['D(:,' mat2str(idx) ')'];
        ColNr = ColNr+NCols;
    end
...
if ~isnumeric(M) | ~isnumeric(ColNr) | (length(ColNr) ~= 1)
    error('Wrong syntax for GETCOLUMN.');
end
if (ColNr < 1) | (ColNr > size(M, 2))
    error('Requested column number doesn''t exist.');
else
    C = M(:, ColNr);
end
...
idx = find(~isinf(Param.xlim));
LimVec(idx)   = Param.xlim(idx);
idx = find(~isinf(Param.ylim));
LimVec(idx+2) = Param.ylim(idx);
...
C = vectorzip(Fields, repmat({' and '}, 1, NFields));
C(end) = [];
Str = cat(2, C{:});
...
if strcmpi(Flag, 'datapoint')
    ...
    if isempty(UD.P(Pidx).ExecStat)
        if ~isempty(UD.P(Pidx).IDFNames)
            ...
            Txt = sprintf('\\bf\\fontsize{9}Not a single valid fieldname in the ''totalIDfields''-\nproperty for the table to which the datapoint belongs!');
        end

        Hdl = msgbox(Txt, upper(mfilename), struct('WindowStyle', 'non-modal', 'Interpreter', 'tex'));
    else
        ExecStat(UD.P(Pidx).ExecStat, UD.P(Pidx).IDs(RowIdx, :));
    end
...
if ~isempty(DolSidx)
    ...
    NFields = size(DolSidx, 1); FNames = cell(1, NFields);
    for n = 1:NFields
        FNames{n} = Stat(DolSidx(n, 1)+1:DolSidx(n, 2)-1);
    end
    for n = 1:NFields
        if ischar(Args{n})
            Stat = strrep(Stat, ['$' FNames{n} '$'], ['''', Args{n}, '''']);
        else
            Stat = strrep(Stat, ['$' FNames{n} '$'], mat2str(Args{n}));
        end
    end
end
