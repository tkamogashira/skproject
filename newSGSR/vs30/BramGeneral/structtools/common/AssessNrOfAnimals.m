...
if ~isstruct(S)
    error('First argument should be a structure-array.');
end
[Data, FNames] = destruct(S);
if ~ischar(IdFName) || ~ismember(IdFName, FNames)
    error('Second argument should be valid fieldname.');
end
N = length(S);
if (nargin == 2)
    IncRowNrs = 1:N;
elseif ~isnumeric(IncRowNrs) || ~all(ismember(IncRowNrs, 1:N))
    error(['Optional third argument should be numerical vector with row ' ...
        'numbers of structure-array.']);
end
...
%animal ...
FileNames = char(Data(IncRowNrs, ismember(FNames, IdFName)));
FileNames = unique(char2num(FileNames(:, 2:end), 1), 'rows');
if (length(FileNames) == 1) && isnan(FileNames)
    Nr = 0;
else
    Nr = length(FileNames);
end
