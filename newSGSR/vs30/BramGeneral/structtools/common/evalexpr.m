...
if ~iscell(Expr) || (size(Expr, 1) ~= 1)
    error('First argument should be cell-array with semi-parsed expression.'); 
end
if ~iscell(Data)
    error('Second argument should be cell-array with flattened structure-array.'); 
end

%Parse expression ...
idxFields = find(cellfun('isclass', Expr, 'double')); 
FieldNrs = cat(1, Expr{idxFields}); 
NFields = length(FieldNrs);

idx = find(all(cellfun('isclass', Data(:, FieldNrs), 'double'), 1));
idxNumFields = idxFields(idx); 
NumFieldNrs = FieldNrs(idx);
...
if any(testIdx(:))
    error('Numerical values in a structure-array can only be row- or columnvectors.'); 
end    
...
OtherFieldNrs = FieldNrs(~ismember(FieldNrs, NumFieldNrs));
...
if (nargout == 0) %Execute statement ...
    ...
    if (size(Data, 1) ~= 1)
        error('For statement execution the structure-array must only have one row.'); 
    end
    %Adjust expression, all fields are replaced by content cell-array references,
    %including character strings ...
    for n = 1:NFields
        Expr{idxFields(n)} = sprintf('(Data{%d})', FieldNrs(n)); 
    end
    eval(cat(2, Expr{:}));
    ...
    isVectorizable = (size(Data, 1) ~= 1); 
    n = 1; %Optimistic approach ...
    ...
    %processed row by row ...
        ...
        NRow = size(Data, 1); 
        Value = cell(NRow, 1); %Pre-allocation ...
        if (NRow == 1)
            %Adjust expression, all fields are replaced by content cell-array references,
