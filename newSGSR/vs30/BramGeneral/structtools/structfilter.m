...
if ~ischar(Expr)
    error('Second argument should be character string with conditional expression.'); 
end
...
%edit P:
% original: idx = find(EvalExpr(ParseExpr(Expr, FNames), Data));
data = EvalExpr(ParseExpr(Expr, FNames), Data);
if (iscell(data))
    data = cell2mat(data);
end
