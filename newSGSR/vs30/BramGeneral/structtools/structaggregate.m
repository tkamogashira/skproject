...
if (nargin == 1) && ischar(varargin{1}) && strcmpi(varargin{1}, 'factory')
    ...
    [Data, FieldNames, Param] = ParseArgs(DefParam, varargin{:});
end

%Grouping ...
if ~isempty(Param.groupfnames)
    N = length(Param.groupfnames);
    ColumnIdx = zeros(1, N);
    for n = 1:N
        ColumnIdx(n) = find(ismember(FieldNames, Param.groupfnames{n}));
    end
    [GroupData, GroupIdx] = findgroups(Data(:, ColumnIdx), Param.groupintervals);
    NGroups = length(GroupIdx);
else
    GroupIdx{1} = 1:size(Data, 1);
    GroupData = {};
    NGroups = 1;
end

%Apply aggregate functions per group ...
NFncs = numel(Param.aggrfncs);
...
ResultData(cellfun('isclass', ResultData, 'cell') & cellfun('isempty', ResultData)) = {[]};
...
AggrFncs = cellstr(varargin{2});
NAggrFncs = numel(AggrFncs);
for n = 1:NAggrFncs
    AggrFncs{n} = parseExpr(AggrFncs{n}, FieldNames);
end
if (NArgs >= 4) && ((ischar(varargin{3}) && ~ismember(varargin{3}, fieldnames(DefParam))) ...
        || iscellstr(varargin{3})) && (isnumeric(varargin{4}) || iscell(varargin{4}))
    GroupFNames = cellstr(varargin{3});
    NGroups = length(GroupFNames);
    if ~all(ismember(GroupFNames, FieldNames))
        error('The field ''%s'' to group by doesn''t exist.', GroupFNames{1});
    end
    if isnumeric(varargin{4})
        GroupIntervals = varargin(4);
    else
        GroupIntervals = varargin{4};
    end
    NIntervals = length(GroupIntervals);
    if (NIntervals ~= NGroups)
        error(['Number of supplied intervals should be equal to the number of ' ...
            'supplied grouping fieldnames.']);
    end
    for n = 1:NIntervals
        if (size(GroupIntervals{n}, 1) == 1) && (size(GroupIntervals{n}, 2) ~= 0) %Also applicable to two-element rowvectors ...
            ...
            GroupIntervals{n} = reshape(mmrepeat( ...
                GroupIntervals{n}, [1, repmat(2, 1, N-2), 1]), 2, (2*(N-2)+2)/2)';
        elseif ~(size(GroupIntervals{n}, 1) ~= 0) && (size(GroupIntervals{n}, 2) == 2) ...
                && ~isempty(GroupIntervals{n})
            error('Invalid format for grouping intervals.'); 
        end
    end    
    ParamIdx = 5;
elseif (NArgs >= 3) && ((ischar(varargin{3}) && ...
        ~ismember(varargin{3}, fieldnames(DefParam))) || iscellstr(varargin{3}))
    GroupFNames = cellstr(varargin{3});
    NGroups = length(GroupFNames);
    GroupIntervals = repmat({[]}, 1, NGroups);
    if ~all(ismember(GroupFNames, FieldNames))
        error('One of the fields to group by doesn''t exist.');
    end
    ParamIdx = 4;
else
...
elseif ischar(Param.aggrfnames) && (NAggrFncs == 1)
    Param.aggrfnames = cellstr(Param.aggrfnames);
elseif ~iscellstr(Param.aggrfnames) || (length(Param.aggrfnames) ~= NAggrFncs)
    error(['Property ''aggrfnames'' must be character string or cell-array of ' ...
        'strings with same number of elements as aggregational functions.']);
end
if isempty(Param.aggrfnames)
    Param.aggrfnames = cellstr([repmat('aggr', NAggrFncs, 1), num2str((1:NAggrFncs)')])';
elseif ischar(Param.aggrfnames) && (NAggrFncs == 1)
...
if ischar(Param.intexpr)
    Param.intexpr = repmat({Param.intexpr}, 1, NGroups);
elseif iscellstr(Param.intexpr) && (length(Param.intexpr) == 1)
    Param.intexpr = repmat(Param.intexpr, 1, NGroups);
elseif ~iscellstr(Param.intexpr) || (length(Param.intexpr) ~= NGroups)
    error(['Property ''intexpr'' must be character string or a cell-array of ' ...
        'strings with the same number of elements as grouping fieldnames.']);
end
ResultFNames = [GroupFNames, Param.aggrfnames];
for n = 1:NGroups
    if isempty(Param.intexpr{n})
        Param.intexpr{n} = {n};
    else
        Param.intexpr{n} = parseExpr(Param.intexpr{n}, ResultFNames);
    end
end
