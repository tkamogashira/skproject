...
returned, minus one if no left most entry is present.*/
{
    ...
    while (i <= j){
        /*Compare doubles within relative precision of floating point representation*/
        if (fabs(dist = Data[m = (i+j)/2]-Scalar) <= fabs(Scalar*DBL_EPSILON)){
            return m;
        }
        else if (dist > 0){
            j = m-1;
...
long Partition(double* Data, long i, long j, double* P)
{
    ...
    Swap(Data+i, Data+m);
    if (P != NULL){
        Swap(P+i, P+m); /*Swap pivot element with first element*/
    }
    for (k = i+1, m = i; k <= j; k++){
        if (Data[k] < Data[i])
        {
    ...
    Swap(Data+i, Data+m);
    if (P != NULL){
        Swap(P+i, P+m); /*Put pivot element back*/
    }
...
/*Using quicksort algorithm by C.A. Hoare*/
{
    if (i >= j){
        ...
        QuickSort(Data, i, m-1, P);
        QuickSort(Data, m+1, j, P);
    }
}
...
/*Concatenate two vectors. The supplied vectors must already be initialised!*/
{
    ...
    if (V2.n == 0){
        return; /*Adding the empty vector*/
    }
...
/*Display the contents of a vector on the MATLAB command window.*/
{
    ...
    mexPrintf("\tdata    : [");
    for (i = 0; i < V.n; i++){
        mexPrintf(" %.3f", V.data[i]); mexPrintf(" ]\n");
    }
}
...
is copied into the newly created rowvector.*/
{
    ...
    if (!mxIsNumeric(A) || (mxGetM(A) != 1)){
        ...
        V = VectorPtrInit(mxGetN(A), mxGetPr(A));
    }
