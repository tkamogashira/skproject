function [RAPStat, PlotStruct, ErrTxt] = CalcRAPIsih(RAPStat)
...
%     function. The mandatory text on a plot is the number of subsequences included
...
%Checking of a dataset is specified and if ISI histogram can be calculated for the
%current dataset ...
...
elseif isTHRdata(RAPStat.GenParam.DS) || isCALIBdata(RAPStat.GenParam.DS), 
    ErrTxt = 'ISI histogram cannot be plotted for current dataset stimulus type'; 
    return;
end
...
ViewPort   = GetRAPHistWindow(RAPStat, 'isih');
if any(ViewPort < 0) || (diff(ViewPort) <= 0)
    ErrTxt = 'Invalid limits for abcissa';
    return;
end
...
if isnan(NBin)
    ...
    %number of bins ...
    Dur = abs(diff(ViewPort));
    NBin = floor(Dur/BinWidth);
...
if (RunAv > NBin),
    ErrTxt = 'Number of points for running average cannot be larger than number of bins';
    return;
end
CalcData = CalcISIH(ds, 'isubseqs', iSubSeqs, 'ireps', iReps, 'anwin', AnWin, ...
...
for n = 1:NPlots,
    ...
    if isRAPStatDef(RAPStat, 'PlotParam.Axis.X.Label.String'), PlotStruct(n).Axis.X.Label.String = 'ISI (ms)'; end
    ...
    if (n == 1) && isRAPStatDef(RAPStat, 'PlotParam.Axis.Title.Label'), PlotStruct(n).Title.Label = 'ISI histogram';
    ...
    if strcmpi(RAPStat.PlotParam.Axis.Text.CalcData, 'on'),
        ...
        PlotStruct(n).Text(end).Label      = sprintf('\\itReFrac\\rm \\leq %.2f ms', CalcData.hist(n).minisi);
        PlotStruct(n).Text(end).FontName   = RAPStat.PlotParam.Axis.Text.FontName;
        PlotStruct(n).Text(end).FontSize   = RAPStat.PlotParam.Axis.Text.FontSize;
        PlotStruct(n).Text(end).FontWeight = 'normal';
        PlotStruct(n).Text(end).FontAngle  = 'normal';
...
%Setting calculated data information in the RAP status variable... For a ISI
...
RAPStat.CalcData.ISI.ReFrac    = CalcData.hist(end).minisi;    %Refractory period in ms ... 