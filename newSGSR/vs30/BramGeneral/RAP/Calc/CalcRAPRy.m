function [RAPStat, PlotStruct, ErrTxt] = CalcRAPRy(RAPStat)
...
elseif isTHRdata(RAPStat.GenParam.DS) | isCALIBdata(RAPStat.GenParam.DS), 
    ErrTxt = 'Ry curve cannot be plotted for current dataset stimulus type'; 
    return;
end
...
PlotStruct.Plot.YData = CalcData.curve.z;
PlotStruct.Plot.FaceColor = RAPStat.PlotParam.Ry.LineColor;
PlotStruct.Plot.EdgeColor = [];
PlotStruct.Plot.LineStyle = RAPStat.PlotParam.Ry.LineStyle;
PlotStruct.Plot.LineWidth = RAPStat.PlotParam.Ry.LineWidth;
PlotStruct.Plot.Marker    = RAPStat.PlotParam.Ry.Marker;
...
if isRAPStatDef(RAPStat, 'PlotParam.Axis.Y.Label.String'), PlotStruct.Axis.Y.Label.String = 'Z'; end
...
if isRAPStatDef(RAPStat, 'PlotParam.Axis.Title.Label'), PlotStruct.Title.Label = 'Rayleigh statistics'; end
...
if strcmpi(RAPStat.PlotParam.Axis.Text.CalcData, 'on'),
    ...
    PlotStruct.Text(end).Label      = {sprintf('\\itMaxZ:\\rm %.2f', CalcData.curve.maxz), ...
            sprintf('\\itBestIndepVal:\\rm %.0f (%s)', CalcData.curve.valatmaxz, UnitStr)};%, ...
...
for n = 1:NDots,
    ...
    PlotElem(n).Y         = CalcData.curve.z(CalcData.curve.isign(n));
    PlotElem(n).FaceColor = RAPStat.PlotParam.Ry.LineColor;
    PlotElem(n).EdgeColor = [];
    PlotElem(n).LineStyle = [];
    PlotElem(n).LineWidth = [];
    PlotElem(n).Marker    = RAPStat.PlotParam.Ry.Marker;
end
...
RAPStat.CalcData.SR.MaxRY      = CalcData.curve.maxz;
RAPStat.CalcData.SR.BestVal   = CalcData.curve.valatmaxz;
