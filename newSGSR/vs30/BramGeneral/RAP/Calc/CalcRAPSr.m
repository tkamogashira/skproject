function [RAPStat, PlotStruct, ErrTxt] = CalcRAPSr(RAPStat)
...
elseif isTHRdata(RAPStat.GenParam.DS) | isCALIBdata(RAPStat.GenParam.DS), 
    ErrTxt = 'Sr curve cannot be plotted for current dataset stimulus type'; 
    return;
end
...
CalcData = CalcSr(ds, 'isubseqs', iSubSeqs, 'ireps', iReps, 'anwin', AnWin, 'minisi', MinISI, ...
...
if strcmp(RAPStat.PlotParam.Sr.YAxisUnit, 'rate'), 
   PlotStruct.Plot.YData = CalcData.curve.sr;
   YUnitStr = 'Rate';
elseif strcmp(RAPStat.PlotParam.Sr.YAxisUnit, 'count'), 
   PlotStruct.Plot.YData = CalcData.curve.srcount; 
   YUnitStr = 'Count';
else,
    ErrTxt = 'YAxisUnit invalid!';
    return;
end

PlotStruct.Plot.FaceColor = RAPStat.PlotParam.Sr.LineColor;
PlotStruct.Plot.EdgeColor = [];
PlotStruct.Plot.LineStyle = RAPStat.PlotParam.Sr.LineStyle;
PlotStruct.Plot.LineWidth = RAPStat.PlotParam.Sr.LineWidth;
PlotStruct.Plot.Marker    = RAPStat.PlotParam.Sr.Marker;
...
PlotStruct.Axis.Y.Limits = RAPStat.PlotParam.Axis.Y.Limits;
if isRAPStatDef(RAPStat, 'PlotParam.Axis.Y.Label.String'), 
    if strcmp(RAPStat.PlotParam.Sr.YAxisUnit, 'rate'), 
        PlotStruct.Axis.Y.Label.String = 'SR (Rate)'; 
    elseif strcmp(RAPStat.PlotParam.Sr.YAxisUnit, 'count'), 
        PlotStruct.Axis.Y.Label.String = 'SR (Count)'; 
    end
end
...
if isRAPStatDef(RAPStat, 'PlotParam.Axis.Title.Label'), PlotStruct.Title.Label = 'Sync Rate'; end
...
if strcmpi(RAPStat.PlotParam.Axis.Text.CalcData, 'on'),
    ...
    PlotStruct.Text(end).Label      = {sprintf('\\itMaxSR:\\rm %.2f (%s)', CalcData.curve.maxsr, YUnitStr), ...
...
for n = 1:NDots,
    ...
    if strcmp(RAPStat.PlotParam.Sr.YAxisUnit, 'rate'), 
        PlotElem(n).Y = CalcData.curve.sr(CalcData.curve.isign(n));
    elseif strcmp(RAPStat.PlotParam.Sr.YAxisUnit, 'count'), 
        PlotElem(n).Y = CalcData.curve.srcount(CalcData.curve.isign(n)); 
    end

    PlotElem(n).FaceColor = RAPStat.PlotParam.Sr.LineColor;
    PlotElem(n).EdgeColor = [];
    PlotElem(n).LineStyle = [];
    PlotElem(n).LineWidth = [];
    PlotElem(n).Marker    = RAPStat.PlotParam.Sr.Marker;
end
...
% PlotElem(NDots+1).LineWidth = RAPStat.PlotParam.Sr.LineWidth;
...
% PlotElem(NDots+2).LineWidth = RAPStat.PlotParam.Sr.LineWidth;
% PlotElem(NDots+2).Marker    = 'none';

if NDots > 0, PlotStruct.Add = PlotElem; end;

%Setting calculated data information in the RAP status variable... For a sync rate
%curve this is MaxSR, BestVal ...
RAPStat.CalcData.XData         = PlotStruct.Plot.XData;
RAPStat.CalcData.YData         = PlotStruct.Plot.YData;
RAPStat.CalcData.SR.MaxSR      = CalcData.curve.maxsr;
RAPStat.CalcData.SR.BestVal   = CalcData.curve.valatmax;
%RAPStat.CalcData.VSM.CutOffR   = CalcData.curve.cutoffr;
%RAPStat.CalcData.VSM.CutOffVal = CalcData.curve.cutoffval;
RAPStat.CalcData.SR.RaySign   = CalcData.curve.raysign;
