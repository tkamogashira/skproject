function [RAPStat, LineNr, ErrTxt] = RAPCmdNX(RAPStat, LineNr, NXToken, ErrToken)
...
if isempty(CurSeqNr), idx = 1; %No dataset specified ...
else, idx = find(ismember(SeqNrs, CurSeqNr)) + 1; end
if (idx > NSeqs) & isempty(ErrLine), ErrTxt = 'No more datasets in datafile'; return;
elseif (idx > NSeqs), LineNr = ErrLine; return; end
