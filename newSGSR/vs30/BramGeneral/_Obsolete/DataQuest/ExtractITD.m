...
if any(strcmpi(ds.SchName, {'CALIB', 'SCH005'}))
    ITD = NaN;
elseif strcmpi(ds.SchName, 'SCH008')
    ITD = diff(ds.StimParam.delay, 1, 2);
...
if any(strcmpi(ds.StimType, {'THR', 'NRHO', 'ARMIN'}))
    ITD = diff(ds.StimParam.delay, 1, 2);
elseif any(strcmpi(ds.StimType, {'BN', 'PS', 'BERT', 'WAV'}))
    ITD = NaN;
else
    ITD = Try2ExtractITD(ds); 
end
...
if any(strcmpi(ds.StimType, {'NTD', 'CTD'}))
    ITD = ds.indepval;
elseif any(strcmpi(ds.StimType, {'NSPL', 'SPL', 'FM', 'IID', 'LMS', 'IMS', 'BMS', 'CFS', 'CSPL', 'FS', 'FSLOG'}))
    ...
    ITD = diff(Delay, 1 , 2);
elseif any(strcmpi(ds.StimType, {'BB', 'BFS', 'ICI'}))
    ITD = NaN;
else
    ITD = Try2ExtractITD(ds); 
end
...
if all(isnan(ITD))
    ...
    if ~all(isnan(Delay)) & (size(Delay, 2) == 2)
        ITD = diff(Delay, 1, 2); 
    end    
end
