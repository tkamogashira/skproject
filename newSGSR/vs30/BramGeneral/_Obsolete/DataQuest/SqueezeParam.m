...
if ischar(P) | (isnumeric(P) & (length(P) == 1))
    return;
else
    [Nrow, Ncol] = size(P);
    if ~any(Ncol == [1, 2])
        error('Stimulus parameter can only have one or two columns.'); 
    end
    ...
    if iscellstr(P)
        ...
        if (Ncol == 2)
            [C1, idx1] = deNaN(P(:, 1)); 
            [C2, idx2] = deNaN(P(:, 2));
            if isequal(idx1, idx2) & isequal(C1, C2)
                Ncol = 1; P = P(:, 1); 
            end
        end
        if (Ncol == 1)
            if all(isnan(P))
                P = NaN;
            elseif (length(unique(deNaN(P))) == 1)
                P = deNaN(P); P = P(1); 
            end
