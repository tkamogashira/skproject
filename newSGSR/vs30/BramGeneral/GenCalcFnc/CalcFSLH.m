function S = CalcFSLH(ArgIn, varargin)
%CALCFSLH    calculate FSL Histogram
%   S = CALCFSLH(DS) calculates First Spiketime Latency Histograms for
%   the subsequences of dataset DS. A structure S is returned containing
%   the calculated data.
%
%   S = CALCFSLH(Spt) calculates First Spiketime Latency Histograms for
%   the subsequences of cell-array of spiketrains. Attention! The property
...
for n = 1:NSubSeqs, Hist(n) = CalcFSLHistogram(Spt(Param.isubseqs(n), Param.ireps{n}), Param); end    
...
%-----------------------------------------------------------------
function Hist = CalcFSLHistogram(Spt, Param)
...
%First spiketimes from each repetition must be gathered with a for loop ...
NRep = length(Spt); FirstSpt = []; for n = 1:NRep, FirstSpt = [FirstSpt min(Spt{n})]; end
...
if isempty(FirstSpt), FirstSpt = Inf; end; %To avoid crash dump but still use histc ...
N = histc(FirstSpt, Edges); N(end) = []; %Remove last garbage bin ...
...
Hist = collectInStruct(BinCenters, N, Rate, Frac, BinWidth);
