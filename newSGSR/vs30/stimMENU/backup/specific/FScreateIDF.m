function idfSeq = FScreateIDF(hifreq, lofreq, deltafreq,...
...
if isequal(StepUnit,'log'),
   % logarithmic freq sweep - delegate to FSLOGcreateIDF
   idfSeq = FSLOGcreateIDF(hifreq, lofreq, deltafreq,...
      modfreq, modpercent, SPL, delay, ...
      interval, duration, rise, fall, repcount, order,...
      activeChan, limitChan, StepUnit);
   return;
