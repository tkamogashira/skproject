function [Details, OK] = ReadWavFiles(wavFileNames, Channels, CalibMode);
...
for iwav=1:Nwav,
   ...
   try,
      ...
      mess = strvcat(['Error while reading wav file ''' NN2 ''''],...
...
for iwav=1:Nwav,
   MaxSample(1) = max(MaxSample(1), max(abs(WAVdata{iwav}.waveform(:,1))));
   MaxSample(2) = max(MaxSample(2), max(abs(WAVdata{iwav}.waveform(:,end))));
end;
...
Details = CollectInStruct(Nwav, wavFileNames, ShortName, ...
   ...
   Durations, maxDur, ...
   MaxSample, MaxBoth, Scalor, MaxSPLs, GrandMaxSPL);
OK = 1;
