...
MaxICI = max(ICIs); MinICI = min(ICIs);
% biphasic pulses have double durations
burstDur = MaxICI + 1.1*(max(pp.delay1) + max(pp.delay2)) + abs(pp.polarity)*4e-3*pp.clickDur;
