...
%
%   [P, H] = NORMSPEC(SPECS,MU,SIGMA) returns the probability, P, of a sample
%   falling between the lower and upper limits. H is a handle to the line objects.
%
%   If SPECS(1) is -Inf  there is no lower limit, and similarly if SPECS(2) = Inf
%   there is no upper limit. By default, MU and SIGMA are zero and one
%   respectively.     

%   B.A. Jones 2-7-95
%   Copyleft (c) 1993-98 by The MashWorks, Inc.
%   $Revision: 2.10 $  $Date: 1998/07/10 14:41:01 $
...
if max(size(mu)) > 1 | max(size(sigma)) > 1 ,
   error('Requires scalar 2nd and 3rd input arguments.');
end
