...
%   $Revision: 2.16 $  $Date: 1998/09/30 19:12:43 $
%   B.A. Jones 4-21-95.
...
if ~strcmp(action,'start')   
  ...
  texthandle = get(fact_fig,'UserData');
  ...
  if ~isempty(data_fig);
     trial_data = get(data_fig,'UserData');
     p1_popup   = findobj(data_fig,'Tag','popup1');
  end

  if ~isempty(doe_fig);
     doe_data   = get(doe_fig,'UserData'); 
  
     p2_popup   = findobj(doe_fig,'Tag','popup2');
     doe_button = findobj(doe_fig,'Tag','doe_button');
  end
  plat = computer;
...
set(0,'Units','pixels')
ss = get(0,'ScreenSize');
fact_fig = figure('Units','pixels','Position',[10 ss(4)-210+fpadj 480 200]);
set(fact_fig,'Color',fcolor,'Backingstore','off','Resize','off',...
    'Name','Reaction Simulator','Tag','control','BusyAction','cancel');

p1_field=uicontrol('Style','edit','Units','pixels','Position',fieldp1,...
         'BackgroundColor','white','String',num2str(p1),'Tag','p1f',...
       'Callback','rsmdemo(''setp1field'')');
         
p1_slider=uicontrol('Style','slider','Units','pixels','Position',slidep1,...
        'Value',p1,'Max',p1hi,'Min',p1lo,...
        'Callback','rsmdemo(''setp1slider'')','Tag','p1s');

p1_text=uicontrol('Style','text','Units','pixels','Position',textp1,...
        'BackgroundColor',fcolor,...
        'ForegroundColor','white','String','Hydrogen');

p2_field=uicontrol('Style','edit', 'Units','pixels','Position',fieldp2,...
       'BackgroundColor','white','String',num2str(p2),'Tag','p2f',...
      'Callback','rsmdemo(''setp2field'')');

         
p2_slider=uicontrol('Style','slider','Units','pixels','Position',slidep2,...
        'Value',p2,'Max',p2hi,'Min',p2lo,...
        'Callback','rsmdemo(''setp2slider'')','Tag','p2s');
                           
p2_text=uicontrol('Style','text','Units','pixels','Position',textp2,...
        'BackgroundColor',fcolor,...
        'ForegroundColor','white','String','n-Pentane');

p3_field=uicontrol('Style','edit','Units','pixels','Position',fieldp3,...
         'BackgroundColor','white','String',num2str(p3),'Tag','p3f',...
       'Callback','rsmdemo(''setp3field'')');
         
p3_slider=uicontrol('Style','slider','Units','pixels','Position',slidep3,...
       'Value',p3,'Max',p3hi,'Min',p3lo,...
        'Callback','rsmdemo(''setp3slider'')','Tag','p3s');
                           
p3_text=uicontrol('Style','text','Units','pixels','Position',textp3,...
        'BackgroundColor',fcolor,...
        'ForegroundColor','white','String','Isopentane');

out_field=uicontrol('Style','text','Units','pixels',...
          'BackgroundColor','white','Position',outfieldp,'Tag','of');

out_text=uicontrol('Style','text','Units','pixels','Position',textout,...
        'BackgroundColor',fcolor,'ForegroundColor','white','String','Reaction Rate');

runs_field=uicontrol('Style','text','Units','pixels','String',num2str(13),...
         'BackgroundColor','white','Position',[25 150 70 20],'Tag','rf');

runs_text=uicontrol('Style','text','Units','pixels','Position',[25 170 75 20],...
        'BackgroundColor',fcolor,'ForegroundColor','white','String','Runs Left');

reset_button = uicontrol('Style','Pushbutton','Units','pixels',...
          'Position',[25 125 75 20],'Callback','rsmdemo(''reset'')',...
          'Visible','off','Tag','rs','String','Reset');

run_button = uicontrol('Style','Pushbutton','Units','pixels',...
          'Position',[25 80 75 20],'Tag','rb','Callback','rsmdemo(''run'')',...
		  'String','Run');

output_button = uicontrol('Style','Pushbutton','Units','pixels',...
          'Position',[25 55 75 20],'Callback','rsmdemo(''output'')','String','Export');

help_button = uicontrol('Style','Pushbutton','Units','pixels',...
          'Position',[375 170 100 20],'Callback','rsmdemo(''help'')',...
          'String','Help');

close_button = uicontrol('Style','Pushbutton','Units','pixels','Position',[25 10 75 20],...
          'Callback','rsmdemo(''close'')','String','Close');

% Create Trial and Error Data View
colheads = str2mat('Run #','Hydrogen','n-Pentane','Isopentane','Reaction Rate');
data_fig = figure('Units','pixels','Position',[5 ss(4)-470+fpadj 300 200],'BusyAction','cancel');
...
analyze_button = uicontrol('Style','Pushbutton','Units','pixels','Position',[2 2 100 20],...
             'Callback','rsmdemo(''analyze'')','String','Analyze');

p1_popup       = uicontrol(data_fig,'Style','Popup','String',...
              'Plot|Hydrogen vs. Rate|n-Pentane vs. Rate|Isopentane vs. Rate',...
              'Units','pixels','Position',[120 2 140 20],...
              'CallBack','rsmdemo(''plot1'')','Tag','popup1');

% Create Designed Experiment Data View
doe_fig = figure('Units','pixels','Position',[315 ss(4)-470+fpadj 355 200],'BusyAction','cancel');
...
doe_button = uicontrol('Style','Pushbutton','Units','pixels','Position',[1 178 150 20],...
             'Callback','rsmdemo(''doe'')',...
             'String','Do Experiment','Tag','doe_button');

rsm_button = uicontrol('Style','Pushbutton','Units','pixels','Position',[1 2 110 20],...
             'Callback','rsmdemo(''fit1'')','String','Response Surface');

nlin_button = uicontrol('Style','Pushbutton','Units','pixels','Position',[115 2 110 20],...
             'Callback','rsmdemo(''fit2'')','String','Nonlinear Model');

p2_popup    = uicontrol(doe_fig,'Style','Popup','String',...
              'Plot|Hydrogen vs. Rate|n-Pentane vs. Rate|Isopentane vs. Rate',...
              'Units','pixels','Position',[230 2 120 20],...
              'CallBack','rsmdemo(''plot2'')','Tag','popup2');

set(doe_fig,'HandleVisibility','callback')
set(data_fig,'HandleVisibility','callback')
set(fact_fig,'UserData',0,'HandleVisibility','callback')
...
elseif strcmp(action,'output')
   ...
   elseif ~isempty(data1) & isempty(data2)
      entrystr = 'reactants,rate';
      promptstr = 'Enter names for the variables or use the supplied defaults.';
   elseif isempty(data1) & ~isempty(data2)
      entrystr = 'doe_reactants,doe_rate';
      promptstr = 'Enter names for the variables or use the supplied defaults.';
   elseif ~isempty(data1) & ~isempty(data2)
      entrystr = 'reactants,rate,doe_reactants,doe_rate';
      promptstr = 'Enter names for the variables or use the supplied defaults.';
   end
   ...
   ok_button = uicontrol('Style','Pushbutton','Units','pixels','Position',[180 5 40 20],...
               'Callback','rsmdemo(''toworkspace'');close(gcbf)',...
               'String','OK');

   set(fact_fig,'UserData',texthandle);
...
elseif strcmp(action,'toworkspace')
   ...
   elseif ~isempty(data1) & isempty(data2)
      ...
      y1 = data1(:,4);
      cpos=findstr(s,',');
	  assignin('base',s(1:cpos(1)-1),x1);
	  assignin('base',s((cpos(1)+1):end),y1);
   elseif isempty(data1) & ~isempty(data2)
      x2 = data2(:,1:3);
      y2 = data2(:,4);
      cpos=findstr(s,',');
	  assignin('base',s(1:cpos(1)-1),x2);
	  assignin('base',s((cpos(1)+1):end),y2);
   elseif ~isempty(data1) & ~isempty(data2)
      x1 = data1(:,1:3);
      y1 = data1(:,4);
      x2 = data2(:,1:3);
      ...
      cpos=findstr(s,',');
	  assignin('base',s(1:cpos(1)-1),x1);
	  assignin('base',s((cpos(1)+1):(cpos(2)-1)),y1);
	  assignin('base',s((cpos(2)+1):(cpos(3)-1)),x2);
	  assignin('base',s((cpos(3)+1):end),y2);
   end
end
