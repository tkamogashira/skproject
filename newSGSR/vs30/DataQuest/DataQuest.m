...
%   exists for the current datafile directory, use DATAQUEST INIT to
%   create one.
...
if ~exist(DBFileName, 'file'), %Initialization ...
    ...
    if ~exist('DBSchedule', 'var') | ~exist('DBParam', 'var'), 
        ...
        if ishandle(PBHdl), delete(PBHdl); end; error(ErrMsg);
    end
    ...
    elseif ~isequal(DBParam.maxnentries, Param.maxnentries),
        ...
        LogFile('add', WarnMsg); warning(WarnMsg);
...
idx = find(~[DBSchedule.processed]); N = length(idx);
for n = 1:N,
    if ~ishandle(PBHdl), break;
    else, ProgBar(PBHdl, [(n-1)/N, 0], {sprintf('Processing %s ...', upper(DBSchedule(idx(n)).name)), ''}); end
    DBSchedule(idx(n)) = ProcessScheduleEntry(DBSchedule(idx(n)), PBHdl, (n-1)/N, 1/N, Param);
end
if ishandle(PBHdl), ProgBar(PBHdl, [1, 1], {'Flushing memory buffer ...', ''}); end;
Buffer('flush');

%Create diskmap for searching the database. Attention! This must be done after
%flushing the buffer ...
if ishandle(PBHdl), ProgBar(PBHdl, [1, 1], {'Creating map of disk ...', ''}); end;
DBDiskMap = CreateDiskMap(Param.datadir);

%Remove dialog box with progression bar ...
if ishandle(PBHdl), delete(PBHdl); end
    ...
    if strcmpi(Entry.type, 'zip'),
        ...
        if ~ismember(ZipFileName, ExtractedArchives),
            ...
            if Status, 
                ...
                warning(WarnMsg); LogFile('add', WarnMsg);
                return;
            else, ExtractedArchives = [ExtractedArchives, {ZipFileName}]; end
        end
    else, FileName = fullfile(Entry.path, Entry.name); end %Datafile already extracted ...
    
    try, 
        LUT = log2lut(FileName);
        if isInvalidLUT(LUT), error('To catch block ...'); end
    catch,
