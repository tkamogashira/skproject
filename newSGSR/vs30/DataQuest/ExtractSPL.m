...
%Extract SPL ...
if strcmpi(ds.FileFormat, 'EDF'), SPL = ExtractSPL4EDF(ds, Nchan); %EDF datasets ...
elseif strcmpi(ds.FileFormat, 'SGSR'), SPL = ExtractSPL4SGSR(ds, Nchan); %SGSR datasets ...
elseif strcmpi(ds.FileFormat, 'IDF/SPK'), SPL = ExtractSPL4IDF_SPK(ds, Nchan); %IDF/SPK datasets ...
elseif strcmpi(ds.FileFormat, 'MDF'), %MDF datasets ...
    ...
    else, SPL = Try2ExtractVF(ds, 'spl'); end    
else, SPL = Try2ExtractVF(ds, 'spl'); end %Unknown dataset format ...
...
if strcmpi(ds.StimType, 'THR'), SPL = NaN;
...
elseif any(strcmpi(ds.StimType, {'NRHO', 'ARMIN', 'BN', 'BERT'})), SPL = ds.StimParam.SPL;
...
if any(strcmpi(ds.StimType, {'ITD', 'FS', 'FSLOG', 'BFS', 'FM', 'LMS', 'BB', 'CFS', 'CTD'})),
    ...
    if (Nchan > 1), SPL(2) = ds.StimParam.indiv.stim{2}.spl; end
elseif strcmpi(ds.StimType, 'IMS'), %????
    if isequal(ds.StimParam.indiv.stim{1}.lospl, ds.StimParam.indiv.stim{1}.hispl) & ...
        ...
        if (Nchan > 1), SPL(2) = ds.StimParam.indiv.stim{2}.lospl; end
    else, SPL = NaN; end
elseif any(strcmpi(ds.StimType, {'SPL', 'NSPL'})), SPL = ds.indepval;
elseif strcmpi(ds.StimType, 'NTD'), SPL = ds.StimParam.SPL; 
%If dataset doesn't belong to any of the above then try to retrieve SPL using the virtual
%field spl of a dataset ...
else, SPL = Try2ExtractVF(ds, 'spl'); end
...
if any(strcmpi(ds.SchName, {'CALIB', 'SCH005', 'SCH008'})), SPL = NaN;
elseif any(strcmpi(ds.SchName, {'SCH006', 'SCH012', 'SCH016'})), SPL = ds.StimParam.SPL;
else, SPL = Try2ExtractVF(ds, 'spl'); end
