...
if ~ishandle(FigHdl) || ~strcmpi(get(FigHdl, 'Type'), 'figure')
    error('First argument should be handle of figure object.'); 
end
if ~ishandle(AxHdl) || ~strcmpi(get(AxHdl, 'Type'), 'axes')
    error('Second argument should be handle of axes object.'); 
end
...
elseif (nargin == 3) || ischar(varargin{1})
    ...
    switch LocStr
    case 'll'
        N = 3; %Axes object in lower left corner ...
    case 'ur'
        N = 2; %Axes object in upper right corner ...
    case 'ul'
        N = 1; %Axes object in upper left corner ...
    case 'lr'
        N = 4; %Axes object in lower right corner ...
    end
    Pidx = 2;
elseif (nargin == 5) || isnumeric(varargin{1})
    [NX, NY, N] = deal(varargin{1:3});
    if (length(NX) ~= 1) || (NX <= 0) || (mod(NX, 1) ~= 0)
        error('Grid elements for abcis should be positive integer.'); 
    end
    if (length(NY) ~= 1) || (NY <= 0) || (mod(NY, 1) ~= 0)
        error('Grid elements for ordinate should be positive integer.'); 
    end
    if (length(N) ~= 1) || (N <= 0) || (mod(N, 1) ~= 0)
        error('Axes position should be positive integer.'); 
    end
    if (N > (NX*NY))
