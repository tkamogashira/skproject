function List = genWFList(varargin)
% genWFList  This function has been replaced by genCorrList

error('genWFList is obsolete. This function has been replaced by genCorrList.');

% genCorrList(varargin);


% 
% %genWFList  generate list for creation of correlograms.
% %   List = genWFList(List, FileName, iSeqs/SeqIDs, iSubSeqs/IndepVals) adds
...
% %   be specified.
% %
% %   An example with genWFList (see kevCorr for a real life example):
% %       List = genWFList(struct([]), 'A0242', [-140, -141], (40:10:80)', ...
...
% %
% %       List = genWFList(struct([]), 'A0242', 341, [+1, -1], ... 
% %           'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');
% %       List = genWFList(List, 'A0242', [336; 337], [+1, -1], ... 
% %           'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');
...
% Data = Data(:, [1 2 4 3 5]);
% if isnumeric(Props.discernvalue)
%     DiscernValues = Props.discernvalue;
% else
...
%     end
% catch
%     error(['Property ''discernvalue'' must be numeric scalar, numeric vector with the same number of\n', ...
...
%     end
% catch
%     error('First argument should be valid structure-array.'); 
% end
% 
% %----------------------------------------------------------------------------
...
% 
% if ischar(FileNames)
%     FileNames = repmat({FileNames}, NEntries, 2); 
...
% else
%     error('Datafile must be specified by their filename.'); 
% end
% 
% %----------------------------------------------------------------------------
...
% 
% if any(isnan(iSeqs))
%     error('One of the given sequences doesn''t exist.'); 
% end
% 
% %----------------------------------------------------------------------------
...
%     for n = 1:Nrec
%         iSeq(n) = GetiSeq4SeqID(FileName{n}, SeqID(n)); 
%     end %Recursion ...   
...
% end
% 
% if any(isnan(iSubSeqs))
%     error('One of the given subsequences doesn''t exist.'); 
% end
% 
% %----------------------------------------------------------------------------
