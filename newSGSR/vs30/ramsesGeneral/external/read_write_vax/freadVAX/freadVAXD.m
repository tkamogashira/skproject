function [numVax count] = freadVAXD(fid, number, method)
...
switch method   
    case {'float32', 'single'}
      [rawUINT32 count] = fread(fid,number,'uint32=>uint32');
      numVax = uint32le_to_VAXF( rawUINT32 );
      
    case {'float64', 'double'}
      [rawUINT32 count] = fread(fid,2*number,'uint32=>uint32');%read 2 32bit numbers
    ...
    otherwise
      [numVax count] = fread(fid, number, method);
