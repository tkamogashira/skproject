function IDFdisp(idf, i_seq, disp_all)
...
if ischar(idf) || isnumeric(idf) % filename or testset index -> read idf file
   idf = idfread(idf);
end
if isinf(i_seq) % last one
   i_seq = length(idf.sequence);
end
