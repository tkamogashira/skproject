...
% 
% 
% create a signal with a mistuned harmonic
% s=signal(0.1,16000);
% clear p
% p{1}='fundamental';
% p{2}=100;
% p{3}='type';
% p{4}='mistunedharmonic';
% p{5}='changeby';
% p{6}=10;
% p{7}='min_fre';
% p{8}=500;
...
% p{11}='harmonicnumber';
% p{12}='12';
% s=genharmonics(s,p{1},p{2},p{3},p{4},p{5},p{6},p{7},p{8},p{9},p{10},p{11},p{12})

%  create a harmonic signal with lots of harmonics
% s=signal(0.1,16000);
% clear p
% p{1}='fundamental';
% p{2}=250;
...
% p{5}='max_fre';
% p{6}=5001;
% s=genharmonics(s,p{1},p{2},p{3},p{4},p{5},p{6})
% 
...
% defaultvalues:
if strcmp(str_filterprop,'') && isempty(str_max_fre{1})
...
% if isempty(str_max_fre) 
if isempty(str_max_fre{1}) 
