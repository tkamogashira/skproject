            ...
            if (BBICselectWithCF(m).ThrCF>=(k-1)*50)&(BBICselectWithCF(m).ThrCF<k*50)
                ...
                if length(MM1)>=2
                    ...
                    for g=1:length(MM1)-1
                        [bump,idx]=max(abs(Dif(MM1(g)+1:MM1(g+1))));
                        bumpidx=MM1(g)+idx;
                        bumpM=[bumpM [bump;bumpidx]];
                        
                    end;
                    bumpM(:,1)=[];
                    [maxbump,maxbumpidx]=max(abs(bumpM(1,:)));
                    plot(BBICselectWithCF(m).sigpX(bumpidx),BBICselectWithCF(m).sigpY(bumpidx),'m*','MarkerSize',6);hold on
                end;
            end;
        end;
    end;
