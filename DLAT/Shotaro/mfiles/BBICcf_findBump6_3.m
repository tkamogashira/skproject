        ...
        if BBICselectWithCF(m).ThrCF~=[]
            ...
            if (BBICselectWithCF(m).ThrCF>=(k-1)*200)&(BBICselectWithCF(m).ThrCF<k*200)
                ...
                if length(MM1)>=2
                    ...
                    Low=BBICselectWithCF(m).RateBF*(2^(-1/3));
                    High=BBICselectWithCF(m).RateBF*(2^(1/3));
...
figure
for x=1:SIZE(2)
    if isempty(BBICselectWithCF_S(x).bumpfreq)==0&isempty(BBICselectWithCF_S(x).RateBF)==0
        plot(BBICselectWithCF_S(x).CD,BBICselectWithCF_S(x).bumpfreq-BBICselectWithCF_S(x).RateBF,'bo');grid on;hold on
        plot(BBICselectWithCF_S(x).CD,abs(BBICselectWithCF_S(x).bumpfreq-BBICselectWithCF_S(x).RateBF),'r*','MarkerSize',4);
    end;
end;
hold off
