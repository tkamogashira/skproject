...
FullFileName = parseEDFFileName(varargin{1});
%if(exist(FullFileName, 'file'))
    %[ner,ntype]=audfileformat(FullFileName);
    %if isequal(ntype, 21)
        %d = HarrisDataset(FullFileName, varargin{2});
        %return;
        %end
        %end
...
if isIDF, ConstrMode = 'readFarm';
else, ConstrMode = 'readSGSR';
end
...
case 'readSGSR', % an SGSR filename and sequence number is given
   ...
   if dd.Header.SessionInfo.requestID,
      ...
      try, iCell = str2num(strtok(SeqID,'-')); end;
   end
   ...
   try Experimenter = dd.Header.SessionInfo.Experimenter; catch Experimenter = ''; end
...
case 'readFarm', % an IDF/SPK filename and sequence number is given
   ...
   try, [dum, RecordParams] = log2lut(fn); catch, RecordParams = []; end
   % independent variable
   if isempty(IndepVar),
      SMS = idf2sms(IDF);
      PI = SMS.PRP.plotInfo;
...
ToCacheFile('DataSet', Ncache, RetrieveParam, d);
...
if InterruptedAndStupid,
   warning('Incomplete data collection *and* exotic convention of UET var storage: extraction of independent variable may be corrupted.');
end
UU(find(UU<-1e20))=0; % PDP-11  value for NaN is something like -1e30
s1 = std(UU(:,1));
s2 = std(UU(:,2));
