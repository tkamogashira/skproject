...
for k=1:30
    ...
    for m=1:SIZE(2)
        ...
        if (BBICselectBB3(m).CP < -0.5)|(BBICselectBB3(m).CP >= 0.5)
            ...
            size1=size(BBICselectBB3(m).allY);
            BBICselectBB3(m).allYr=BBICselectBB3(m).allY+ones(1,size1(2))*Diff;
            ...
            BBICselectBB3(m).allYr=BBICselectBB3(m).allY;
            BBICselectBB3(m).lineYr=BBICselectBB3(m).lineY;
        end;
        
        %making a vector of difference between curve and line with CD&CP
        LINEY=BBICselectBB3(m).allX.*BBICselectBB3(m).CD/1000+BBICselectBB3(m).CPr;
        Dif=BBICselectBB3(m).allY-LINEY;
        %display(m)
        %display(Dif)
        ...
        if (BBICselectBB3(m).CD>=(k-13)*0.2)&(BBICselectBB3(m).CD<(k-12)*0.2)
            ...
            if BBICselectBB3(m).Mserr>=0.001            
                if length(MM1)>=2
                    ...
                    plot(BBICselectBB3(m).allX,BBICselectBB3(m).allYr,'r');axis([0 3000 -2 2]);grid on;hold on
                    ...
                    plot(BBICselectBB3(m).allX(maxbumpidx),BBICselectBB3(m).allY(maxbumpidx),'r*','MarkerSize',6);hold on
                    BBICselectBB3_S(m).bumpfreq=BBICselectBB3(m).allX(maxbumpidx);
                    N=[N m];
                else
                    plot(BBICselectBB3(m).allX,BBICselectBB3(m).allYr,'m');axis([0 3000 -2 2]);grid on;hold on
                    %plot(BBICselectBB3(m).lineX,BBICselectBB3(m).lineYr,'k--');
                ...
                if length(MM1)>=2
                    ...
                    %High=BBICselectWithCF(m).DomF*(2^(1/3));
                    %if Low<=BBICselectWithCF(m).sigpX(maxbumpidx)&BBICselectWithCF(m).sigpX(maxbumpidx)<=High
                    plot(BBICselectBB3(m).allX,BBICselectBB3(m).allYr,'b');axis([0 3000 -2 2]);grid on;hold on
                    %plot(BBICselectBB3(m).lineX,BBICselectBB3(m).lineYr,'k--');
                    plot(BBICselectBB3(m).VSBF,-1.2,'b.','MarkerSize',8);
                    plot(BBICselectBB3(m).RateBF,-0.7,'b.','MarkerSize',8);
                    %plot(BBICselectBB3(m).ThrCF,BBICselectBB3(m).ThrCF*BBICselectBB3(m).CD/1000+BBICselectBB3(m).CPr,'r.','MarkerSize',8);
                        
                    plot(BBICselectBB3(m).allX(maxbumpidx),BBICselectBB3(m).allY(maxbumpidx),'b*','MarkerSize',6);hold on
                    BBICselectBB3_S(m).bumpfreq=BBICselectBB3(m).allX(maxbumpidx);
                    N=[N m];
                else
                    plot(BBICselectBB3(m).allX,BBICselectBB3(m).allYr,'c');axis([0 3000 -2 2]);grid on;hold on
                    %plot(BBICselectBB3(m).lineX,BBICselectBB3(m).lineYr,'k--');
