...
for m=1:(SIZE(2)-1)
    if strcmp(CFcombiselectS(m).Fiber2,CFcombiselectS(m+1).Fiber2)==0
        ...
        for n=1:Size(2)
            ...
            R=13;C=3;
            subplot(R,C,GroupN);plot(CF2group(n).DeltaCF,CF2group(n).CD,'k*','MarkerSize',8);axis([0 0.4 -2 2]);hold on
            subplot(R,C,GroupN);plot(CF2group(n).DeltaCF,CF2group(n).BestITD,'r.','MarkerSize',8);hold on
            subplot(R,C,GroupN);plot(CF2group(n).DeltaCF,CF2group(n).CPr,'go','MarkerSize',4);hold on
