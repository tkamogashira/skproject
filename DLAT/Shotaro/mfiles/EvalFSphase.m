function [ArgOut,BF,BestITD,BP,CD,CP,CF1,CF2,IPCx,IPCy,IRx,IRy] = EvalFSphase(varargin)
...
if CalcParam.corbinwidth==0
    ArgOut=[];BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];IRx=[];IRy=[];return;
end
...
if isempty(ComSub)
    fprintf('No common subsequences.');ArgOut=[];BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];IRx=[];IRy=[];return; 
elseif NSub == 1
    fprintf('Only one common subsequence.');ArgOut=[];BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];IRx=[];IRy=[];return;
else
...
if strcmpi(PlotParam.plot, 'YES')
    ...
    if any(strcmpi(PlotParam.plotmode, {'ALL', 'IPC'}))
        ...
        [CD,CP,CF1,CF2,IPCx,IPCy,IRx,IRy] = PlotIPC(IPC, IR, BinCycleHist, CellInfo, Param);
    end
    ...
    if any(strcmpi(PlotParam.plotmode, {'ALL', 'PRA'}))
        ...
        PlotPRA(MRA, IRA, MPC, IPC, IR, MR, MSR, ISR, PD, CellInfo, Param);
    end
end
...
if CheckStimParam(ds1, ds2)
    ...
    DataFile=[];Cell1=[];Cell2=[];ds1=[];ds2=[];CalcParam.corbinwidth=0;PlotParam=[];ArgOut=[];BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;return
end
...
% PLOTIPC %
%---------%
function [CD,CP,CF1,CF2,IPCx,IPCy,IRx,IRy] = PlotIPC(IPC, IR, BinCycleHist, CellInfo, Param)
...
%IPCx&IPCy assign
IRx=IR.X;IRy=IR.Y;
assignin('base','IRx',IRx);assignin('base','IRy',IRy);         
%Niet significante waarden donker plotten
...
% PLOTPRA %
%---------%
function PlotPRA(MRA, IRA, MPC, IPC, IR, MR, MSR, ISR, PD, CellInfo, Param);
