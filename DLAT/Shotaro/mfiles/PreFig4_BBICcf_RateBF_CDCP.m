...
for k=1:44
    ...
    xlabel([num2str((k-1)*50) '<=RateBF<' num2str(k*50)],'FontSize',6,'Units','normalized','Position',[0.5,1]);
    for m=1:SIZE(2)
        if BBICselectWithCF(m).ThrCF~=[]
            if (BBICselectWithCF(m).RateBF>=(k-1)*50)&(BBICselectWithCF(m).RateBF<k*50)
