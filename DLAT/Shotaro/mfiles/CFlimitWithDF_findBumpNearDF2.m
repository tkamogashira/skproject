    ...
    if strcmp(CFcombiselectWithDFS(m).Fiber2,CFcombiselectWithDFS(m+1).Fiber2)==0
        ...
        for n=1:Size(2)
            ...
            if length(MM1)>=2
                ...
                Low=CF2group(n).DF2*(2^(-1/3));
                High=CF2group(n).DF2*(2^(1/3));
...
assignin('base','CFcombiselectWithDFS_NearDF2',CFcombiselectWithDFS);
...
for m=1:(SIZE(2)-1)
    if strcmp(CFcombiselectWithDFS(m).Fiber2,CFcombiselectWithDFS(m+1).Fiber2)==0
        ...
        for n=1:Size(2)
            ...
            plot(x,y-yk2,'b');grid on;hold on;ylim([-0.5 0.5])
