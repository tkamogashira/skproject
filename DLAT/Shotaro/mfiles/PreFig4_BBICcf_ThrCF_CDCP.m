...
for k=1:44
    ...
    xlabel([num2str((k-1)*50) '<=ThrCF<' num2str(k*50)],'FontSize',6,'Units','normalized','Position',[0.5,1]);
    for m=1:SIZE(2)
        if BBICselectWithCF(m).ThrCF~=[]
            if (BBICselectWithCF(m).ThrCF>=(k-1)*50)&(BBICselectWithCF(m).ThrCF<k*50)
                plot(BBICselectWithCF(m).CPr,BBICselectWithCF(m).CD,'.','MarkerSize',20);axis([-0.5 0.5 -2 2]);grid on;hold on
