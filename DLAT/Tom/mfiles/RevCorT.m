...
% make magnitude- and phase-frequency plot
figure;
gainaxis = subplot(1,2,1);
plot(freq, Gain);
ylim([-20 0]);
title('Gain');
phaseaxis = subplot(1,2,2);
plot(freq, Phase);
title('Phase');
% add cell information
    %Calculate sizes of gain axis ...
    XRng = XLim(gainaxis); YRng = YLim(gainaxis);
    Xsize = abs(diff(XRng)); Ysize = abs(diff(YRng));
    %Find coordinates of a point above and left of the higher right corner ...
    [Xhrc, Yhrc] = deal(XRng(2) - 0.4*Xsize, YRng(2) + 0.08*Ysize);
    %compute cell information
    g = getCF4Cell(DS.filename, DS.id.iCell);
    CF = g.thr.cf;
    cellinfo = strcat(DS.filename,'-',DS.seqid,' - CF = ',num2str(CF), ' Hz - SPL = ',num2str(DS.spl),' dB');
subplot(gainaxis);    
text(Xhrc, Yhrc, cellinfo);