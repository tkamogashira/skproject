function staggerWFplot(DLAT, intensity, normto, sf, datafilenames)
...
for i=1:nd
    ...
    if ~isempty(find(D(i).xval==intensity))&~isempty(find(D(i).xval==normto))
        ...
        if D(i).difcor == 1, line(D(i).lag(idx,:), D(i).normco(idx,:), 'LineStyle', '-', 'Marker', 'none', 'Color', [0 0 1]);
        elseif D(i).difcor == 0, line(D(i).lag(idx,:), D(i).normco(idx,:), 'LineStyle', '-', 'Marker', 'none', 'Color', [1 0 0]);
