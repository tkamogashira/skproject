...
%
% two types of delay have to be discerned: fine structure-delay (computed by Tf, by limiting the correlation lag to +/- 400 microsec) ... 
% and enveloppe-delay (computed by T). Each fiber has two rows in struct array DLAT: 
% the first contains in field yval the envelope-delay, the second contains fine structure-delay in field yval.
%
...
% TF 09/09/2005
%
% Split monkey data into seperate m-file
% PM 10/10/2006
%
% M0627 added, fine-structure data (Tf) removed.
% PM 11/10/2006
...
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
% ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;close all; 
...
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
    %ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
    close all; 
...
%M0541, M0541B, M0541C
%Fiber 29
List = GenWFList(struct([]), 'M0541', [240;241;242;243], [+1, -1], 'discernvalue','getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit','dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 20    
%Tf = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 20   
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 
...
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','yes','smoothing',smoothing,'spline',spline); % reference = ds 74                       
...
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','yes','smoothing',smoothing,'spline',spline); % reference = ds 120                       
...
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','yes','smoothing',smoothing,'spline',spline); % reference = ds 150                       
...
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 11                       
...
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 18                       
...
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 3                       
...
T = genwfplotTF(List, 1, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 23                       
...
%M0627B------------------------------------------------------------------------------------------------------------------------------------
%Fiber28
List = GenWFList(struct([]), 'M0627B', [2;3;4;5], [+1, -1], 'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 2                       
%Tf = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 2                    
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 

%Fiber29
List = GenWFList(struct([]), 'M0627B', [13;14;15;16], [+1, -1], 'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 13                       
%Tf = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 13                    
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 

%Fiber33
List = GenWFList(struct([]), 'M0627B', [29;30], [+1, -1], 'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 2, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 29                      
%Tf = genwfplotTF(List, 2, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 29                    
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 

%Fiber38
List = GenWFList(struct([]), 'M0627B', [38;39;40;41;42;43;44], [+1, -1], 'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 5, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 38                       
%Tf = genwfplotTF(List, 5, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 38                    
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 

%Fiber41
List = GenWFList(struct([]), 'M0627B', [52;53;54;55;56;57], [+1, -1], 'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 52                       
%Tf = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 52                    
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 

%Fiber48
List = GenWFList(struct([]), 'M0627B', [84;85;86;87], [+1, -1], 'discernvalue', 'getfield(dataset($filename$, $iseqp$), ''spl'')');               
T = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline); % reference = ds 84                       
%Tf = genwfplotTF(List, 3, 'discernfieldlabel', 'Intensity', 'discernfieldunit', 'dB SPL','plot','no','smoothing',smoothing,'spline',spline,'cormaxlag',0.4); % reference = ds 84                    
D = [D; ExtractPSentryTF(T, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];
%    ExtractPSentryTF(Tf, XFieldName, YFieldName, lagfn, normcofn, ppnm, spd, spnm)];%groupplot(D(numel(D)),'xval', 'yval','xlim',[30 90],'ylim',[-1 0.7]);pause;
close all; 
...
%-----------------------------------------------------------------
DLATMONK = D; clear('D');
save('psLATmonk', 'DLATMONK');
%-----------------------------------------------------------------
Ddifenv=structfilter(DLATMONK, '($difcor$ == 1)&($envdelay$ == 1)');
Ddiffine=structfilter(DLATMONK, '($difcor$ == 1)&($envdelay$ == 0)');
Dnondifenv=structfilter(DLATMONK, '($difcor$ == 0)&($envdelay$ == 1)');
Dnondiffine=structfilter(DLATMONK, '($difcor$ == 0)&($envdelay$ == 0)');
