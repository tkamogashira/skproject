...
for n = 1:((comp/2)-0.5)
    freq = lowlim + ((n)*(bandlim/(comp/2))) + randn;
    F = [F freq]; 
end  
...
for n = 0:11:(lowlim)  %filterprincipe: alle freq. onder bepaalde lowlim krijgen amplitude 0
    A = [A 0];
end
...
if shiftarg == 1
    Rtau = Ltau;
    S = [-shiftpara:((2*shiftpara)/(100-1)):shiftpara];
    Rtau = Rtau + S;
end
