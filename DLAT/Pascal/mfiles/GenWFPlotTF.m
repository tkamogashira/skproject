function ArgOut = GenWFPlotTF(varargin)
...
if (nargin == 1) & ischar(varargin{1}) & strcmpi(varargin{1}, 'factory'),
    ...
    disp(DefProps);
    return;
else, [List, RefRow, Props] = ParseArgs(DefProps, varargin{:}); end

%Calculate auto- or crosscorrelograms ...
[CorFncs, RefRow] = List2CorFncs(List, RefRow, Props);

%Display crosscorrelograms if requested...
if strcmpi(Props.plot, 'yes'), DisplayCorFncs(CorFncs, RefRow, Props); end
...
List = CheckAndTransformList(List, Props);

%----------------------------------------------------------------------------
function List = CheckAndTransformList(List, Props)
...
if ~isempty(Props.rightyexpr), 
    try, Expr = ParseExpr(Props.rightyexpr, FNames);
    catch, error('Property ''rightyexpr'' must be a valid expression.'); end
    RightYValues = EvalExpr(Expr, Data);
else, RightYValues = repmat(NaN, N, 1); end
...
List = structsort(List, Props.discernfieldname, 'mode', 'desc');
...
if (~isnan(CorFncs(1).ds1.iseqn))&(~isnan(CorFncs(1).ds1.iseqp)), difcor='yes';
else, difcor='no';
end
try, g=num2str(getCFList(CorFncs)); %try-catch used to handle fibers for which there is no thr-curve
catch, g='(no cf available)';
end
