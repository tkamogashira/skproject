%Arrange Phase for structure
function NewStruct=Arrange_Phase(Struct)
L=size(Struct);
for n=1:L(2)
    ...
    if Struct(n).CF1>Struct(n).CF2
        ...
        if (NewStruct(n).BP>=0)&(NewStruct(n).BP<1)
            NewStruct(n).BPm=NewStruct(n).BP;
        else
            NewStruct(n).BPm=NewStruct(n).BP-floor(NewStruct(n).BP);
        end
        NewStruct(n).CD=Struct(n).CD*(-1);
        NewStruct(n).CP=1-(Struct(n).CP);
        if (NewStruct(n).CP>=0)&(NewStruct(n).CP<1)
            NewStruct(n).CPm=NewStruct(n).CP;
        else
            NewStruct(n).CPm=NewStruct(n).CP-floor(NewStruct(n).CP);
        end
    else
