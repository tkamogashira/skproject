%CFmismatch
function CFcombi = CFmismatchNew(varargin);


for p=1:(length(varargin)-1)
    [ArgOut,BF,BestITD,BP,CD,CP,CF1,CF2,IPCx,IPCy,ISRx,ISRy]=EvalFSphase2R(varargin{p+1},varargin{1}, 'plotmode', 'ALL');
    Fiber1=[num2str(getfield(varargin{p+1},'ID','FileName')),' ',num2str(getfield(varargin{p+1},'ID','SeqID'))];
    Fiber2=[num2str(getfield(varargin{1},'ID','FileName')),' ',num2str(getfield(varargin{1},'ID','SeqID'))];
    ds1.filename=[Fiber1,' ', Fiber2];
    ds1.icell=[num2str(getfield(varargin{p+1},'ID','FileName')),' ',num2str(getfield(varargin{1},'ID','FileName'))];
...
SIZE=size(CFcombi);
for n=1:SIZE(2)
    x=CFcombi(n).IPCx;y=CFcombi(n).IPCy;
    X=CFcombi(n).ISRx;Y=CFcombi(n).ISRy;
