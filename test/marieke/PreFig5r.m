%x: DeltaCF y: CD(*), BestITD(o), CP(green o) I have to add grid and adjust size of each panel
...
for m=1:(SIZE(2)-1)
    if strcmp(CFcombiselectS(m).Fiber2,CFcombiselectS(m+1).Fiber2)==0
        ...
        NN=ceil(GroupN/10)-1;
        axes('Position',[(GroupN-NN*10-1)*(1/10) NN*(1/4) 1/10 1/4],'FontSize',8);hold on;
        for n=1:Size(2)
            ...
            %R=13;C=3;
            plot(CF2group(n).DeltaCF,CF2group(n).CD,'k*','MarkerSize',8);axis([0 0.4 -2 2]);grid on;hold on
            plot(CF2group(n).DeltaCF,CF2group(n).BestITD,'r.','MarkerSize',8);hold on
            plot(CF2group(n).DeltaCF,CF2group(n).CPr,'go','MarkerSize',4);hold on
            %subplot(R,C,GroupN);plot(x,y);axis([0 4000 -3 3]);hold on
            %subplot(R,C,GroupN);plot(X(YMi),y(YMi),'ro');hold on
            %subplot(R,C,GroupN);plot(xb,yb,'m*','MarkerSize',10);hold on
            %subplot(R,C,GroupN);plot(x1,y1,'k<');plot(x2,y2,'g>');hold on
            xk=(0:10:max(x));
