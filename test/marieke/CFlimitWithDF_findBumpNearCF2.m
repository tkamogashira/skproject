        ...
        for n=1:Size(2)
            ...
            if length(MM1)>=2
                ...
                if Low<=x(maxbumpidx)&x(maxbumpidx)<=High
                    ...
                    CFcombiselectWithDFS(startN-1+n).bumpdif=Dif(maxbumpidx);
                    N=[N startN-1+n];
                end;
...
assignin('base','CFcombiselectWithDFS_NearCF2',CFcombiselectWithDFS);
...
for m=1:SIZE(2)
    ...
    line([CFcombiselectWithDFS(m).ISRx(MaxISRI) CFcombiselectWithDFS(m).CF2],[CFcombiselectWithDFS(m).ISRy(MaxISRI) CFcombiselectWithDFS(m).ISRy(MaxISRI)],'Color','r');hold on;grid on
    
    plot(CFcombiselectWithDFS(m).CF1,CFcombiselectWithDFS(m).ISRy(MaxISRI),'c*');hold on;grid on
    line([CFcombiselectWithDFS(m).ISRx(MaxISRI) CFcombiselectWithDFS(m).CF1],[CFcombiselectWithDFS(m).ISRy(MaxISRI) CFcombiselectWithDFS(m).ISRy(MaxISRI)],'Color','c');hold on;grid on
    
    plot(CFcombiselectWithDFS(m).bumpfreq,CFcombiselectWithDFS(m).ISRy(MaxISRI),'gd','MarkerSize',12);hold on;grid on
    line([CFcombiselectWithDFS(m).ISRx(MaxISRI) CFcombiselectWithDFS(m).bumpfreq],[CFcombiselectWithDFS(m).ISRy(MaxISRI) CFcombiselectWithDFS(m).ISRy(MaxISRI)],'Color','c');hold on;grid on
