...
SIZE=size(CFcombiselectNoLimitWithDFS);
startN=1;GroupN=0;
for m=1:(SIZE(2)-1)
    if strcmp(CFcombiselectNoLimitWithDFS(m).Fiber2,CFcombiselectNoLimitWithDFS(m+1).Fiber2)==0
        CF2group=CFcombiselectNoLimitWithDFS(startN:m);
        startN=m+1;
        GroupN=GroupN+1;
        Size=size(CF2group);
        %assignin('base',['CF2group' num2str(GroupN)],CF2group);
        NN=ceil(GroupN/10)-1;
        axes('Position',[(GroupN-NN*10-1)*(1/10)+0.01 NN*(1/5)+0.01 1/10-0.01 1/5-0.01],'FontSize',8);hold on;
        for n=1:Size(2)
