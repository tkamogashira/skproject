function [BF,BestITD,BP,CD,CP,CF1,CF2,IPCx,IPCy,ISRx,ISRy] = EvalFSphase2Rnew(varargin)
...
[DataFile,DataFile2 Cell1, Cell2, ds1, ds2, CalcParam, PlotParam] = EvalParam(varargin, DefParam);
if CalcParam.corbinwidth==0
    BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];ISRx=[];ISRy=[];return;
end
...
if isempty(ComSub)
    fprintf('No common subsequences.');BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];ISRx=[];ISRy=[];return; 
elseif NSub == 1
    fprintf('Only one common subsequence.');BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];ISRx=[];ISRy=[];return;
else
...
CellInfo = CollectInStruct(DataFile,DataFile2, Cell1, Cell2);
...
% EVALPARAM %
%-----------%
function [DataFile,DataFile2 Cell1, Cell2, ds1, ds2, CalcParam, PlotParam] = EvalParam(ParamList, DefParam)
%EVALFS(ds1, ds2, ...)
...
%if ~strcmp(ds1.FileName, ds2.FileName), error('Datasets are from different experiment.'); end
DataFile = ds1.FileName;
DataFile2 = ds2.FileName;
...
if CheckStimParam(ds1, ds2)
    ...
    DataFile=[];Cell1=[];Cell2=[];ds1=[];ds2=[];CalcParam.corbinwidth=0;PlotParam=[];BF=0;BestITD=0;BP=0;CD=0;CP=0;CF1=0;CF2=0;IPCx=[];IPCy=[];ISRx=[];ISRy=[];return
end
...
% PLOTIPC %
%---------%
function [CD,CP,CF1,CF2,IPCx,IPCy] = PlotIPC(IPC, IR, BinCycleHist, CellInfo, Param,DDSS1,DDSS2)
...
disp(Param.CellParam(1).CF)
disp(Param.CellParam(2).CF)
disp(CellInfo(1).DataFile)
disp(CellInfo(1).Cell1.CellNr)
disp(CellInfo(1).DataFile2)
disp(CellInfo(1).Cell2.CellNr)
[CF1,SR1]=getCF4Cell(CellInfo(1).DataFile,CellInfo(1).Cell1.CellNr)
[CF2,SR2]=getCF4Cell(CellInfo(1).DataFile2,CellInfo(1).Cell2.CellNr)
