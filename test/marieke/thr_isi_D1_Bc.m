...
for u=1:length(D1_Bc)
    
    for x=1:length(Dthr186)
        if strcmp(Dthr186(x).ds.filename,D1_Bc(u).thrfile)==1 & strcmp(Dthr186(x).ds.seqid,D1_Bc(u).thrds)==1
            xxx=x;
        end;
    end;
    D1_Bc(u).depth=Dthr186(xxx).depth;
    
end;


for u=1:length(D1_Bc)
    ds = dataset(D1_Bc(u).ds1.filename, D1_Bc(u).ds1.seqid);
    ...
    axes(Title)
    text(0.5,0.5,[D1_Bc(u).ds1.filename ' ' D1_Bc(u).ds1.seqid])
    %print
    close
    clear k;
    
    %aa=D1_Bc(u).thrfile;bb=D1_Bc(u).thrds;
    for x=1:length(Dthr186)
        if strcmp(Dthr186(x).ds.filename,D1_Bc(u).thrfile)==1 & strcmp(Dthr186(x).ds.seqid,D1_Bc(u).thrds)==1
            xxx=x;
        end;
    ...
    if Dthr186(xxx).tag==0
        ds2 = dataset(D1_Bc(u).thrfile,D1_Bc(u).thrds);
        evalTHRs(ds2)
        figure(FigHdl)
    else
        ...
        THRtext=strvcat(['ThrData lost'],['CF: ' num2str(D1_Bc(u).CF) 'Hz'],['THR: ' num2str(D1_Bc(u).THR) 'dB']);
    ...
    axes(AxHdl4)
    text(0.1,0.95,[D1_Bc(u).ds1.filename ' ' D1_Bc(u).ds1.seqid ' depth: ' num2str(D1_Bc(u).depth)])
    if D1_Bc(u).approach==2
        text(0.1,0.8,'Midline approach')
    else
        text(0.1,0.8,'Transbulla approach')
    end;
    if D1_Bc(u).RecSide==1
        text(0.1,0.7,'RecSide: Left')
    else
        text(0.1,0.7,'RecSide: Right')
    end;
    if D1_Bc(u).StimSide==1
        text(0.1,0.6,'StimSide: Left')
    else
        text(0.1,0.6,'StimSide: Right')
    end;
    text(0.1,0.4,'Subjective evaluation of ISI:')
    text(0.1,0.3,[D1_Bc(u).Evalisi])
    axes(AxHdl5)
    LIST=['Relevant dataset:'];
    for v=1:length(F)
        if strcmp(F(v).thrfile,D1_Bc(u).thrfile)==1&...
                strcmp(F(v).thrds,D1_Bc(u).thrds)==1
            name=F(v).ds1.seqid;
    ...
    for v=1:length(D1_A)
        if strcmp(D1_A(v).thrfile,D1_Bc(u).thrfile)==1&...
                strcmp(D1_A(v).thrds,D1_Bc(u).thrds)==1
