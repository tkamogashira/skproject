...
for k=1:20
    ...
    for m=1:SIZE(2)
        ...
        if (CFcombiselectALL_30(m).CF2>=(k-1)*200)&(CFcombiselectALL_30(m).CF2<k*200)
            N=N+1;
            x=CFcombiselectALL_30(m).IPCx;y=CFcombiselectALL_30(m).IPCy;X=CFcombiselectALL_30(m).ISRx;Y=CFcombiselectALL_30(m).ISRy;
            if (CFcombiselectALL_30(m).CP < -0.5)|(CFcombiselectALL_30(m).CP >= 0.5)
                y=y+(CFcombiselectALL_30(m).CPr-CFcombiselectALL_30(m).CP);
            end;
            if CFcombiselectALL_30(m).pLinReg < 0.005
                plot(x,y,'b');hold on;
            elseif CFcombiselectALL_30(m).pLinReg >= 0.005
                plot(x,y,'r');hold on;    
            end;
            %ylim([-2.5 2.5]);%axis([0 4000 -2.5 2.5]);
            %plot(bumpx,bumpy,'r*','MarkerSize',4);hold on
            [YM,YMi]=max(Y);YY=CFcombiselectALL_30(m).CPr+(CFcombiselectALL_30(m).CD/1000)*X(YMi);
            %plot(X(YMi),YY,'ko','MarkerSize',6);hold on
            
            xb=CFcombiselectALL_30(m).BF;yb=CFcombiselectALL_30(m).BPr;
            %plot(xb,yb,'y*','MarkerSize',5);hold on
            
            x1=CFcombiselectALL_30(m).CF1;y1=CFcombiselectALL_30(m).CF1p;
            x2=CFcombiselectALL_30(m).CF2;y2=CFcombiselectALL_30(m).CF2p;
            ...
            xk=(0:10:max(x));
            yk=CFcombiselectALL_30(m).CPr+(CFcombiselectALL_30(m).CD/1000)*xk;
            ...
            ykd=CFcombiselectALL_30(m).CPr+(CFcombiselectALL_30(m).CD/1000)*x;
            Dif=y-ykd;
            ...
            if length(MM1)>=2
                ...
                bumpM(:,1)=[];
                if CFcombiselectALL_30(m).CD>=0
                    [p,q]=max(bumpM(1,:));maxbumpidx=bumpM(2,q);
                else
                    [p,q]=min(bumpM(1,:));maxbumpidx=bumpM(2,q);
                end;
                Low=CFcombiselectALL_30(m).CF2*(2^(-1/3));
                High=CFcombiselectALL_30(m).CF2*(2^(1/3));
                if Low<=x(maxbumpidx)&x(maxbumpidx)<=High
                    plot(x(maxbumpidx),y(maxbumpidx),'r*','MarkerSize',8);hold on
                    CFcombiselectALL_30(m).bumpfreq=x(maxbumpidx);
                    CFcombiselectALL_30(m).bumpdif=Dif(maxbumpidx);
...
assignin('base','CFcombiselectALL_30_bump',CFcombiselectALL_30);
...
for k=1:20
    ...
    for m=1:SIZE(2)
        ...
        if (CFcombiselectALL_30(m).CF2>=(k-1)*200)&(CFcombiselectALL_30(m).CF2<k*200)
            ...
            if CFcombiselectALL_30(m).pLinReg < 0.005
                plot(CFcombiselectALL_30(m).coch, CFcombiselectALL_30(m).CD, 'k.','markersize',6);hold on;
            elseif CFcombiselectALL_30(m).pLinReg >= 0.005
                plot(CFcombiselectALL_30(m).coch, CFcombiselectALL_30(m).CD, 'ko','markersize',6);hold on;
            end;
        end;
...
for k=1:20
    ...
    for m=1:SIZE(2)
        ...
        if (CFcombiselectALL_30(m).CF2>=(k-1)*200)&(CFcombiselectALL_30(m).CF2<k*200)
            ...
            plot(CFcombiselectALL_30(m).CF2,0,'g.','MarkerSize',10);hold on
            
            xk=(0:10:max(x));
            %yk=CFcombiselectALL_30(m).CPr+(CFcombiselectALL_30(m).CD/1000)*xk;
            %line(xk,yk,'LineStyle', '--', 'Color', 'k', 'Marker', 'none');hold on
            yk2=CFcombiselectALL_30(m).CPr+(CFcombiselectALL_30(m).CD/1000)*x;
