...
for n=1:length(CFcombiselectALL_40)
    x=CFcombiselectALL_40(n).IPCx;y=CFcombiselectALL_40(n).IPCy;
...
for n=1:length(CFcombiselectALL_40)
    ...
    xdif=x-ones(1,length(x))*CFcombiselectALL_40(n).CF2;
    Lind=min(find(xdif>0));Rind=max(find(xdif<0));
    CFcombiselectALL_40(n).CF2ph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselectALL_40(n).CF2-x(Lind))/(x(Rind)-x(Lind));%Phase at CF2 on real curve
    
    x2=CFcombiselectALL_40(n).CF2;y2=CFcombiselectALL_40(n).CF2ph;
    
    plot(x,yn*1000./x);hold on;
    plot(x2,y2*1000/x2,'>','MarkerSize',5,'Color','r','MarkerFaceColor','r');hold on;
    
    %xind=find(x>=CFcombiselectALL_40(n).CF2*(2^(0)) & x<=CFcombiselectALL_40(n).CF2*(2^(1/3)));%Window at CF2
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    %line(x(xind),(x(xind).^(-1))*1000*CFcombiselectALL_40(n).CPrr+ones(1,length(x(xind)))*CFcombiselectALL_40(n).CD,'LineStyle', '-', 'Color', 'r', 'Marker', 'none');hold on;
    
    %x2=CFcombiselectALL_40(n).CF2;y2=CFcombiselectALL_40(n).CF2p;
...
for n=1:length(CFcombiselectALL_40)
    x=CFcombiselectALL_40(n).IPCx;y=CFcombiselectALL_40(n).IPCy;
    Y=x*(CFcombiselectALL_40(n).CD/1000)+ones(1,length(x))*CFcombiselectALL_40(n).CP;
...
for n=1:length(CFcombiselectALL_40)
    ...
    xind=find(x>=CFcombiselectALL_40(n).CF2*(2^(-1/3)) & x<=CFcombiselectALL_40(n).CF2*(2^(0)));%Window at CF2
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
...
for n=1:length(CFcombiselectALL_40)
    ...
    xind=find(x>=CFcombiselectALL_40(n).CF2*(2^(-1/3)) & x<=CFcombiselectALL_40(n).CF2*(2^(1/3)));%Window at CF2
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
