%CFmismatch
function CFcombiselect = CFmismatchN5_A0241(varargin);
global THR_A0241
N=length(varargin);
M=(1:1:N);
S=[varargin{1}];
...
for q=1:N
    ...
    for p=1:(N-1)
        ...
        for y=1:length(THR_A0241)
            if strncmp(THR_A0241(y).filename,num2str(getfield(A(p+1),'ID','FileName')),5)==1&strncmp(THR_A0241(y).seqid,num2str(getfield(A(p+1),'ID','SeqID')),5)==1
                dt1=dataset(THR_A0241(y).thrfile,THR_A0241(y).thrds);
                T1= evalTHR(dt1, 'plot', 'no');
                CF1=T1.thr.cf;
                clear dt1;clear T1;
            elseif strncmp(THR_A0241(y).filename,num2str(getfield(A(1),'ID','FileName')),5)==1&strncmp(THR_A0241(y).seqid,num2str(getfield(A(1),'ID','SeqID')),5)==1
                dt2=dataset(THR_A0241(y).thrfile,THR_A0241(y).thrds);
        ...
        if (DeltaCF>=-1/3)&(DeltaCF<=1/3)&(CF1<=CF2)
            ...
            [BF,BestITD,BP,CD,CP,cf1,cf2,IPCx,IPCy,ISRx,ISRy]=EvalFSphase2R_A0241(A(p+1),A(1), 'plotmode', 'ALL');
