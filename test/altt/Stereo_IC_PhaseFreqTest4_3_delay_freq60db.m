...
BBICselectWithCF_60=structfilter(BBICselectWithCF,'$spl$==60');
echo off;

for n=1:length(BBICselectWithCF_60)
    x=BBICselectWithCF_60(n).sigpX;y=BBICselectWithCF_60(n).sigpY;
...
for n=1:length(BBICselectWithCF_60)
    ...
    xdif=x-ones(1,length(x))*BBICselectWithCF_60(n).DomF;
    Lind=min(find(xdif>0));Rind=max(find(xdif<0));
    BBICselectWithCF_60(n).DomFph=yn(Lind)+(yn(Rind)-yn(Lind))*(BBICselectWithCF_60(n).DomF-x(Lind))/(x(Rind)-x(Lind));%Phase at DomF on real curve
    
    x2=BBICselectWithCF_60(n).DomF;y2=BBICselectWithCF_60(n).DomFph;
    
    plot(x,yn*1000./x);hold on;
    plot(x2,y2*1000/x2,'>','MarkerSize',5,'Color','r','MarkerFaceColor','r');hold on;
    
    %xind=find(x>=BBICselectWithCF_60(n).CF2*(2^(0)) & x<=BBICselectWithCF_60(n).CF2*(2^(1/3)));%Window at CF2
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    %line(x(xind),(x(xind).^(-1))*1000*BBICselectWithCF_60(n).CPrr+ones(1,length(x(xind)))*BBICselectWithCF_60(n).CD,'LineStyle', '-', 'Color', 'r', 'Marker', 'none');hold on;
    
    %x2=BBICselectWithCF_60(n).CF2;y2=BBICselectWithCF_60(n).CF2p;
...
for n=1:length(BBICselectWithCF_60)
    x=BBICselectWithCF_60(n).sigpX;y=BBICselectWithCF_60(n).sigpY;
    Y=x*(BBICselectWithCF_60(n).CD/1000)+ones(1,length(x))*BBICselectWithCF_60(n).CP;
...
for n=1:length(BBICselectWithCF_60)
    ...
    xind=find(x>=BBICselectWithCF_60(n).DomF*(2^(0)) & x<=BBICselectWithCF_60(n).DomF*(2^(1/3)));%Window at DomF
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
...
for n=1:length(BBICselectWithCF_60)
    ...
    xind=find(x>=BBICselectWithCF_60(n).DomF*(2^(-1/3)) & x<=BBICselectWithCF_60(n).DomF*(2^(0)));%Window at DomF
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
    
    %x2=BBICselectWithCF_60(n).DomF;y2=BBICselectWithCF_60(n).DomFp;
    %plot(x2,y2*1000/x2,'>','MarkerSize',5,'Color','r','MarkerFaceColor','r');
end;
f=(100:1:4000);
plot(f,f.^(-1)*1000,'g');plot(f,f.^(-1)*(-1000),'g');plot(f,f.^(-1)*500,'g');plot(f,f.^(-1)*(-500),'g');plot(f,f*0,'g');axis([0 4000 0 4]);
...
for n=1:length(BBICselectWithCF_60)
    ...
    xind=find(x>=BBICselectWithCF_60(n).DomF*(2^(-1/3)) & x<=BBICselectWithCF_60(n).DomF*(2^(1/3)));%Window at DomF
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
