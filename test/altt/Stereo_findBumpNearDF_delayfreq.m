...
for k=1:40
    ...
    for n=1:SIZE(2)
        if (CFcombiselect8121single(n).BF>=(k-1)*100)&(CFcombiselect8121single(n).BF<k*100)
            ...
            xdif=x-ones(1,length(x))*CFcombiselect8121single(n).BF;
            Lind=min(find(xdif>0));Rind=max(find(xdif<0));
            CFcombiselect8121single(n).BFph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselect8121single(n).BF-x(Lind))/(x(Rind)-x(Lind));%Phase at DF on real curve
    
            x2=CFcombiselect8121single(n).BF;y2=CFcombiselect8121single(n).BFph;
    
            %plot(x,yn*1000./x);hold on;
            plot(x2,y2*1000/x2,'>','MarkerSize',5,'Color','g','MarkerFaceColor','g');hold on;
    
            xind=find(x>=CFcombiselect8121single(n).BF*(2^(-1/3)) & x<=CFcombiselect8121single(n).BF*(2^(1/3)));%Window at BF
            %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
            %plot(xplus,yplus*1000./xplus);hold on
            line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',1,'MarkerEdgeColor','b','Color','b');hold on;
