...
for n=1
    ...
    for s=1:length(x)
        ...
        aaa=colormap(jet);
        CC=aaa(index,:);
        plot(x(s),y(s),'o','MarkerFaceColor',CC,'MarkerEdgeColor',CC,'MarkerSize',15);hold on;
        text('string',num2str(z),'position',[x(s),y(s)],'FontSize',8);
        clear z;
    end;    
    [YM,YMi]=max(Y);
    %CFcombiselect242(n).BPr=CFcombiselect242(n).CPr+(CFcombiselect242(n).CD/1000)*CFcombiselect242(n).BF;
    ...
    plot(x,y,'-','LineWidth',2, 'Color',[0.7 0.7 0.7]);hold on;
    %plot(X(YMi),y(YMi),'o','LineWidth',2,'MarkerSize',10,'Color',[0.7 0.7 0.7],'MarkerFaceColor',[1 1 1]);
    plot(xb,yb,'ko','MarkerSize',15);
    plot(x1,y1,'k<','MarkerSize',12);%CFc
    plot(x2,y2,'>','MarkerSize',12,'Color','k','MarkerFaceColor','k');%CFi
    xk=(0:10:max(x));
    yk=CFcombiselect242(n).CPr+(CFcombiselect242(n).CD/1000)*xk;
    line(xk,yk,'LineStyle', '-', 'Color', 'k', 'Marker', 'none');
    x3=(xb-100:10:xb+100);
...
xlabel('Tone frequency (Hz)');ylabel('Interaural phase (cycles)');
