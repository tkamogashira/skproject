...
for n=1:length(CFcombiselectALL_30)
...
for n=1:length(CFcombiselectALL_30)
    ...
    CFcombiselectALL_30(n).CF2ph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselectALL_30(n).CF2-x(Lind))/(x(Rind)-x(Lind));%Phase at CF2 on real curve
    ...
    %xind=find(x>=CFcombiselectALL_30(n).CF2*(2^(0)) & x<=CFcombiselectALL_30(n).CF2*(2^(1/3)));%Window at CF2
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    %line(x(xind),(x(xind).^(-1))*1000*CFcombiselectALL_30(n).CPrr+ones(1,length(x(xind)))*CFcombiselectALL_30(n).CD,'LineStyle', '-', 'Color', 'r', 'Marker', 'none');hold on;
    
    %x2=CFcombiselectALL_30(n).CF2;y2=CFcombiselectALL_30(n).CF2p;
...
for n=1:length(CFcombiselectALL_30)
    x=CFcombiselectALL_30(n).IPCx;y=CFcombiselectALL_30(n).IPCy;
    Y=x*(CFcombiselectALL_30(n).CD/1000)+ones(1,length(x))*CFcombiselectALL_30(n).CP;
...
for n=1:length(CFcombiselectALL_30)
    ...
    xind=find(x>=CFcombiselectALL_30(n).CF2*(2^(-1/3)) & x<=CFcombiselectALL_30(n).CF2*(2^(0)));%Window at CF2
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
