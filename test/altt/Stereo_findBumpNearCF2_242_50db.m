...
for k=1:40
    ...
    for n=1:SIZE(2)
        if (CFcombiselect242(n).CF2>=(k-1)*100)&(CFcombiselect242(n).CF2<k*100)
            ...
            [YM,YMi]=max(Y);YY=CFcombiselect242(n).CPr+(CFcombiselect242(n).CD/1000)*X(YMi);
            ...
            %x3=(xb-100:10:xb+100);
            %line(x3,(x3-xb)*(CFcombiselect242(n).BestITD)/1000+yb,'LineStyle', '-', 'Color', 'm', 'Marker', 'none');
