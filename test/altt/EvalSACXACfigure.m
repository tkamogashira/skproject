...
%   T = EVALSACXAC(ds1, SubSeqs1, ds2, SubSeqs2) calculates the SAC, XAC, SUM-
%   and DIFCOR from the specified subsequences. If subsequences are given as
%   vectors, for each corresponding subsequence pair a calculation is performed.
%   E.g.:
%           ds1 = dataset('A0242', -135);
%           ds2 = dataset('A0242', -136);
%           EvalSACXAC(ds1, 50:10:80, ds2, 50:10:80);
%
%   T = EVALSACXAC(ds, SubSeqs) where SubSeqs is a two-element vector, specifying
%   the subsequence pair used for the calculation.
%   E.g.:
...
elseif (nargin == 1) & ischar(varargin{1}) & strcmpi(varargin{1}, 'clrcache')
    ...
    [ds1, ds2, Spt1, Spt2, Info, StimParam, Param] = ParseArgs(DefParam, varargin{:}); 
end

%Retrieving data from SGSR server ...
try,
    UD = getuserdata(Info(1).ds1.filename, Info(1).ds1.icell); if isempty(UD), error('To catch block ...'); end
...
for n = 1:NCalc,
    if (NCalc > 1) & strcmpi(Param.verbose, 'yes'), fprintf('Calculating correlograms %d/%d ...\n', n, NCalc); end    
    if strcmpi(Param.cache, 'yes'),
        SearchParam = structcat( ...
            ...
            getfields(Param, {'anwin', 'corbinwidth', 'cormaxlag', 'envrunavunit', 'envrunav', 'diffftrunav', 'sumfftrunav', 'cutofflevel', 'calcdf'})); %Calculation parameters ...
        ...
        if isempty(CacheData), 
            ...
            else, CalcData(n) = CalcDifCor(Spt1(n, :), Spt2(n, :), Thr, Param); end
...
for n = 1:NCalc,
    ...
    if ~isempty(Thr), FilterCF = DetermineCalcDF(Param.effsplcf, Thr.cf, CalcData(n).diff.fft.df, CalcData(n).ac.fft.df);
    else, FilterCF = DetermineCalcDF(Param.effsplcf, NaN, CalcData(n).diff.fft.df, CalcData(n).ac.fft.df); end
    ...
    try, 
        ...
        if isequal(length(unique(EffSPL)), 1), StimParam(n).effspl1 = EffSPL(1); 
        elseif isequal(length(EffSPL), ds1.nsub), StimParam(n).effspl1 = EffSPL(Info(n).ds1.isubseq); end
    catch, warning(lasterr); StimParam(n).effspl1 = NaN; end
    try, 
        EffSPL = CalcEffSPL(ds2, 'filtercf', FilterCF, 'filterbw', FilterBW, 'channel', ds2.chan);
        if isequal(length(unique(EffSPL)), 1), StimParam(n).effspl2 = EffSPL(1); 
        elseif isequal(length(EffSPL), ds2.nsub), StimParam(n).effspl2 = EffSPL(Info(n).ds2.isubseq); end
    catch, warning(lasterr); StimParam(n).effspl2 = NaN; end
    ...
    elseif ~isnan(StimParam(n).effspl1), StimParam(n).effspl = StimParam(n).effspl1;
    else, StimParam(n).effspl = StimParam(n).effspl2; end
...
if strcmpi(Param.plot, 'yes'), 
    for n = 1:NCalc, PlotDifCor(Info(n), StimParam(n), Thr, Rcn, Param, CalcData(n)); end
end
...
if (nargout > 0), 
    ...
    for n = 1:NCalc, ArgOut(n) = structtemplate(structcat(CalcData(n), struct('stim', StimParam(n))), Template); end
end
...
if (Nds == 1), %T = EVALSACXAC(ds, SubSeqs)
    ...
    else, error('Second argument should be a scalar or two-element numeric vector.'); end
        
    ParamIdx = 3;
elseif (Nds == 2), %T = EVALSACXAC(ds1, SubSeqs1, ds2, SubSeqs2)
    if (length(varargin) < 4), error('Wrong number of input arguments.'); end
    
    ds1 = varargin{1}; if ~isa(ds1, 'dataset'), error('First argument should be dataset.'); end
    ds2 = varargin{3}; if ~isa(ds2, 'dataset'), error('Third argument should be dataset.'); end
    if ~isequal(ds1.filename, ds2.filename) | ~isequal(ds1.icell, ds2.icell), error('Datasets weren''t collected from the same cell.'); end
    
    if ~isnumeric(varargin{2}) | ~any(size(varargin{2}) == 1), error('Second argument should be a numeric vector.'); end
    if ~isnumeric(varargin{4}) | ~any(size(varargin{4}) == 1), error('Forth argument should be a numeric vector.'); end
    if ~isequal(length(varargin{2}), length(varargin{4})), error('Subsequence vectors for datasets should be of same length.'); end
...
if strcmpi(Param.subseqinput, 'subseq'), 
    ...
    if ~all(ismember(IndepVal1, ds1.indepval)), error('One of the values for the independent variable of first dataset doesn''t exist.'); end
    if ~all(ismember(IndepVal2, ds2.indepval)), error('One of the values for the independent variable of second dataset doesn''t exist.'); end
...
for n = 1:N, StimParam(n) = GetStimParam(ds1, iSubSeq1(n), ds2, iSubSeq2(n)); end
...
if ~isnumeric(Param.acfftrunav) | (length(Param.acfftrunav) ~= 1) | (Param.acfftrunav < 0), error('Invalid value for property acfftrunav.'); end
...
if ~isnumeric(Param.sumfftrunav) | (length(Param.sumfftrunav) ~= 1) | (Param.sumfftrunav < 0), error('Invalid value for property sumfftrunav.'); end
if ~isnumeric(Param.cutofflevel) | (length(Param.cutofflevel) ~= 1) | ~ismember(Param.cutofflevel, [3, 6]), error('Invalid value for property cutofflevel.'); end
...
spl1 = GetSPL(ds1); spl1 = CombineSPLs(spl1(iSubSeq1, 1), spl1(iSubSeq1, end));
spl2 = GetSPL(ds2); spl2 = ComBineSPLs(spl2(iSubSeq2, 1), spl2(iSubSeq2, end));
...
if ischar(Spt2) & strcmpi(Spt2, 'nodiag'),
    ...
    if ~isempty(Thr), DomFreq = DetermineCalcDF(Param.calcdf, Thr.cf, NaN, FFTsac.DF);
    else, DomFreq = DetermineCalcDF(Param.calcdf, NaN, NaN, FFTsac.DF); end
    ...
    %Calculating the ratio of the secundary over the primary peak for the SAC and the half height width
    %on the peak (relative to asymptote one)...
    ...
    [Ypp, T, NC] = SPTCORR(anwin(Spt1, Param.anwin), 'nodiag', Param.cormaxlag, Param.corbinwidth, WinDur); %SAC ...
    spkrate1 = NC.Rate1;
    nspike1 = NC.Nspike1;
    Ypp = ApplyNorm(Ypp, NC);
    %Correlation of noise token A- responses of a cell with the responses of that same cell to that same noise
    %token.
    [Ynn, T, NC] = SPTCORR(anwin(Spt2, Param.anwin), 'nodiag', Param.cormaxlag, Param.corbinwidth, WinDur); %SAC ...\
    spkrate2 = NC.Rate2;
    nspike2 = NC.Nspike2;
    ...
    [Ypn, T, NC] = SPTCORR(anwin(Spt1, Param.anwin), anwin(Spt2, Param.anwin), Param.cormaxlag, Param.corbinwidth, WinDur); %XAC ...
    Ypn = ApplyNorm(Ypn, NC);
    %Correlation of noise token A- responses of a cell with the responses of that same cell to a different noise
    %token, in this case A+.
    [Ynp, T, NC] = SPTCORR(anwin(Spt2, Param.anwin), anwin(Spt1, Param.anwin), Param.cormaxlag, Param.corbinwidth, WinDur); %XAC ...
    ...
    %Determine which dominant frequency to be used in the calculation ...
    if ~isempty(Thr), DomFreq = DetermineCalcDF(Param.calcdf, Thr.cf, FFTdif.DF, FFTsac.DF);
    else, DomFreq = DetermineCalcDF(Param.calcdf, NaN, FFTdif.DF, FFTsac.DF); end
    ...
    SumHHWx = cintersect(T, Ysumcor, HalfMaxSum); SumHHW = abs(diff(SumHHWx));
    
    %Performing spectrum analysis on the SUMCOR. Because a sumcor has a prominent DC component in comparison with
    %other correlograms, detrending is necessary to get a representative dominant frequency ...
    FFTsum = spectana(T, detrend(Ysumcor, 'constant'), 'RunAvUnit', 'Hz', 'RunAvRange', Param.sumfftrunav);
    FFTsum.Magn.dB = FFTsac.Magn.dB/2;
    
    %Extracting Best and CutOff Frequecy from spectrun of SUMCOR, because the spectrum can be considered as that
    %of a low pass filter ...
    [MaxP, PCutOff, BestFreq, CutOffFreq] = AssessLowPassFilter(FFTsum.Freq, FFTsum.Magn.P(2, :), FFTsum.Magn.dB(2, :), Param.cutofflevel);
    
    %Reorganizing calculated data ...
    CalcData.delay           = T;
    CalcData.ac.normco       = [Ysac; Yxac];
    CalcData.ac.max          = max(Ysac);
    CalcData.ac.saczero      = Ysac(find(T == 0));
    CalcData.ac.xaczero      = Yxac(find(T == 0));
...
idx = min(find((MagndB < -CutOffLevel) & (Freq > BestFreq)));
if ~isempty(idx) & (idx ~= 1), CutOffFreq = interp1(MagndB([idx-1, idx]), Freq([idx-1, idx]), -CutOffLevel);
else, CutOffFreq = NaN; end    
...
if isequal(Info.ds1.iseq, Info.ds2.iseq) & isequal(Info.ds1.isubseq, Info.ds2.isubseq),
    ...
    IDStr   = sprintf('%s <%d-%d>@%.0f%s', Info.ds1.filename, CellNr, TestNr, Info.ds1.indepval, Info.ds1.indepunit);
    ...
    IDStr   = sprintf('%s <%d-%d>@%.0f%s & <%d-%d>@%.0f%s', Info.ds1.filename, CellNr1, TestNr1, Info.ds1.indepval, Info.ds1.indepunit, ...
        CellNr2, TestNr2, Info.ds2.indepval, Info.ds2.indepunit);
    NormStr = sprintf('Norm. Count\n(N_{rep}*(N_{rep}-1)*r^2*\\Delta\\tau*D)'); %Normalisation Coincidence count ...
end
...
if ~isempty(Rcn),
    text(0.0, 0.5, char(Param.str, '', Rcn.str), 'Units', 'normalized', 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', ...
        'FontWeight', 'demi', 'FontSize', 9);
else,
    text(0.0, 0.5, Param.str, 'Units', 'normalized', 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', ...
        'FontWeight', 'demi', 'FontSize', 9);
end    
if ~isempty(Thr),
    text(0.625, 0.5, char(Thr.str, '', StimParam.str), 'Units', 'normalized', 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', ...
        'FontWeight', 'demi', 'FontSize', 9);
else,
    text(0.625, 0.5, char(sprintf('THR data not present'), '', StimParam.str), 'Units', 'normalized', 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', ...
        'FontWeight', 'demi', 'FontSize', 9);
end    

%Plotting SAC and XAC ...
X = CalcData.delay; Y = CalcData.ac.normco; 
[MinX, MaxX, XTicks] = GetAxisLim('X', X, Param.corxrange, Param.corxstep);
AxCOR = axes('Position', [0.10 0.63 0.2 0.165], 'Box', 'off', 'TickDir', 'out');
if isequal(Info.ds1.iseq, Info.ds2.iseq) & isequal(Info.ds1.isubseq, Info.ds2.isubseq),
    LnHdl = line(X, Y, 'LineStyle', '-', 'Marker', 'none', 'LineWidth', 2, 'Color', 'b');
    assignin('base',[Info.ds1.filename '_' num2str(CellNr1) '_' num2str(TestNr1) 'sacxacX'],X);
    assignin('base',[Info.ds1.filename '_' num2str(CellNr1) '_' num2str(TestNr1) 'sacxacY'],Y);
    title('SAC', 'FontSize', 12);
    TxtStr =  {sprintf('Max: %.2f', CalcData.ac.max), sprintf('PeakRatio: %.2f', CalcData.ac.peakratio), ...
...
xlabel('Delay(ms)'); ylabel(NormStr, 'Units', 'normalized', 'Position', [-0.09, 0.5, 0]);
...
%Plotting DIFCOR ...
if isequal(Info.ds1.iseq, Info.ds2.iseq) & isequal(Info.ds1.isubseq, Info.ds2.isubseq),
    AxDIF = CreateEmptyAxis([0.10 0.365 0.2 0.165]);
else,
    X = CalcData.delay; Y = CalcData.diff.normco; 
    AxDIF = axes('Position', [0.10 0.365 0.2 0.165], 'Box', 'off', 'TickDir', 'out');
    LnHdl = line(X, Y);
    assignin('base',[Info.ds1.filename '_' num2str(CellNr1) '_' num2str(TestNr1) 'difcorX'],X);
    assignin('base',[Info.ds1.filename '_' num2str(CellNr1) '_' num2str(TestNr1) 'difcorY'],Y);
    ...
    text(MinX, MaxY, {sprintf('Max: %.2f', CalcData.diff.max), sprintf('PeakRatio: %.2f', CalcData.diff.peakratio), ...
            sprintf('HHW: %.2fms', CalcData.diff.hhw)},'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', 9);
end

%Plotting SUMCOR ...
if isequal(Info.ds1.iseq, Info.ds2.iseq) & isequal(Info.ds1.isubseq, Info.ds2.isubseq),
    AxSUM = CreateEmptyAxis([0.575 0.365 0.2 0.165]);    
else,
    X = CalcData.delay; Y = CalcData.sum.normco; 
    AxSUM = axes('Position', [0.575 0.365 0.2 0.165], 'Box', 'off', 'TickDir', 'out');
    LnHdl = line(X, Y); 
...
if isequal(Info.ds1.iseq, Info.ds2.iseq) & isequal(Info.ds1.isubseq, Info.ds2.isubseq), 
    FFT = CalcData.ac.fft; TitleStr = 'DFT on SAC';
else, FFT = CalcData.diff.fft; TitleStr = 'DFT on DIFCOR'; end
X = FFT.freq; 
...
AxDFT = axes('Position', [0.10 0.10 0.2 0.165], 'Box', 'off', 'TickDir', 'out','XScale','log');
LnHdl = line(X, Y);
assignin('base',[Info.ds1.filename '_' num2str(CellNr1) '_' num2str(TestNr1) 'dftX'],X);
assignin('base',[Info.ds1.filename '_' num2str(CellNr1) '_' num2str(TestNr1) 'dftY'],Y);
...
if isequal(Info.ds1.iseq, Info.ds2.iseq) & isequal(Info.ds1.isubseq, Info.ds2.isubseq),
    AxSDFT = CreateEmptyAxis([0.575 0.10 0.2 0.165]);    
else,
    ...
    [MinY, MaxY] = GetAxisLim('Y', Y, Param.fftyrange);
    AxSDFT = axes('Position', [0.575 0.10 0.2 0.165], 'Box', 'off', 'TickDir', 'out','XScale','log');
    LnHdl = line(X, Y);
    set(LnHdl(2), 'LineStyle', '-', 'Color', 'b', 'LineWidth', 0.5);
    set(LnHdl(1), 'LineStyle', ':', 'Color', 'k', 'LineWidth', 0.5);
    ...
    YRange = get(AxSDFT, 'YLim'); MinY = YRange(1); MaxY = YRange(2);
