...
for k=1:40
    ...
    for n=1:SIZE(2)
        if (CFcombiselect241(n).CF2>=(k-1)*100)&(CFcombiselect241(n).CF2<k*100)
            ...
            [YM,YMi]=max(Y);YY=CFcombiselect241(n).CPr+(CFcombiselect241(n).CD/1000)*X(YMi);
            ...
            %x3=(xb-100:10:xb+100);
            %line(x3,(x3-xb)*(CFcombiselect241(n).BestITD)/1000+yb,'LineStyle', '-', 'Color', 'm', 'Marker', 'none');
            clear Dif;clear MM1;clear bump;clear DifM;
            ykd=CFcombiselect241(n).CPr+(CFcombiselect241(n).CD/1000)*x;
            Dif=y-ykd;
            ...
            if length(MM1)>=2
                ...
                bumpM(:,1)=[];
                if CFcombiselect241(n).CD>=0
                    [p,q]=max(bumpM(1,:));maxbumpidx=bumpM(2,q);
                else
                    [p,q]=min(bumpM(1,:));maxbumpidx=bumpM(2,q);
                end;
                Low=CFcombiselect241(n).CF2*(2^(-1/3));
                High=CFcombiselect241(n).CF2*(2^(1/3));
...
for k=1:40
    ...
    for n=1:SIZE(2)
        if (CFcombiselect241(n).CF2>=(k-1)*100)&(CFcombiselect241(n).CF2<k*100)
            ...
            x=CFcombiselect241(n).IPCx;y=CFcombiselect241(n).IPCy;
            ylast=y(length(y));
            ...
            xdif=x-ones(1,length(x))*CFcombiselect241(n).CF2;
            Lind=min(find(xdif>0));Rind=max(find(xdif<0));
            CFcombiselect241(n).CF2ph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselect241(n).CF2-x(Lind))/(x(Rind)-x(Lind));%Phase at CF2 on real curve
    
            x2=CFcombiselect241(n).CF2;y2=CFcombiselect241(n).CF2ph;
    
            %plot(x,yn*1000./x);hold on;
            plot(x2,y2*1000/x2,'>','MarkerSize',5,'Color','g','MarkerFaceColor','g');hold on;
    
            %xind=find(x>=CFcombiselect241(n).CF2*(2^(-1/3)) & x<=CFcombiselect241(n).CF2*(2^(1/3)));%Window at CF2
