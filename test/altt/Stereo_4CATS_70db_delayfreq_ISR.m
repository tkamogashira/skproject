...
for k=1:40
    ...
    for n=1:SIZE(2)
        if (CFcombiselectALL_70(n).CF2>=(k-1)*100)&(CFcombiselectALL_70(n).CF2<k*100)
            
            x=CFcombiselectALL_70(n).IPCx;y=CFcombiselectALL_70(n).IPCy;
            difvec=abs(x-ones(1,length(x))*CFcombiselectALL_70(n).CF2);
            ...
            for qq=1:length(CFcombiselectALL_70(n).ISRy)
                z=CFcombiselectALL_70(n).ISRy(qq);
            ...
            CFcombiselectALL_70(n).CF2ph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselectALL_70(n).CF2-x(Lind))/(x(Rind)-x(Lind));
            ...
            ykd=CFcombiselectALL_70(n).CPr+(CFcombiselectALL_70(n).CD/1000)*x;
            Dif=y-ykd;
            ...
            if length(MM1)>=2
                ...
                bumpM(:,1)=[];
                if CFcombiselectALL_70(n).CD>=0
                    [p,q]=max(bumpM(1,:));maxbumpidx=bumpM(2,q);
                else
                    [p,q]=min(bumpM(1,:));maxbumpidx=bumpM(2,q);
                end;
                Low=CFcombiselectALL_70(n).CF2*(2^(-1/3));
                High=CFcombiselectALL_70(n).CF2*(2^(1/3));
