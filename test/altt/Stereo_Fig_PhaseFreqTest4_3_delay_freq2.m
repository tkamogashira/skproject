...
for n=1:length(CFcombiselectALL)
    ...
    x2=CFcombiselectALL(n).BF;y2=CFcombiselectALL(n).BFph;
...
for n=1:length(CFcombiselectALL)
    ...
    Lind=min(find(xdif>0));Rind=max(find(xdif<0));
    if isempty(Lind)==0 & isempty(Rind)==0
    CFcombiselectALL(n).BFph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselectALL(n).BF-x(Lind))/(x(Rind)-x(Lind));%Phase at DF on real curve
    ...
    xind=find(x>=CFcombiselectALL(n).BF*(2^(0)) & x<=CFcombiselectALL(n).BF*(2^(1/3)));%Window at DF
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
...
for n=1:length(CFcombiselectALL)
    ...
    xind=find(x>=CFcombiselectALL(n).BF*(2^(-1/3)) & x<=CFcombiselectALL(n).BF*(2^(0)));%Window at BF
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
...
for n=1:length(CFcombiselectALL)
    ...
    xind=find(x>=CFcombiselectALL(n).BF*(2^(-1/3)) & x<=CFcombiselectALL(n).BF*(2^(1/3)));%Window at BF
    %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
    %plot(xplus,yplus*1000./xplus);hold on
    line(x(xind),yn(xind)*1000./x(xind),'Marker','o','MarkerSize',5,'MarkerEdgeColor','b','Color','c');hold on;
