...
for k=1:40
    ...
    for m=1:SIZE(2)
        ...
        %if BBICselectWithCF(m).ThrCF>0
            ...
            if (BBICselectWithCF(m).DomF>=(k-1)*100)&(BBICselectWithCF(m).DomF<k*100)
                ...
                if length(MM1)>=2
                    ...
                    xdif=x-ones(1,length(x))*BBICselectWithCF(m).DomF;
                    Lind=min(find(xdif>0));Rind=max(find(xdif<0));
                    if isempty(Lind)==0 & isempty(Rind)==0
                        BBICselectWithCF(m).DomFph=y(Lind)+(y(Rind)-y(Lind))*(BBICselectWithCF(m).DomF-x(Lind))/(x(Rind)-x(Lind));%Phase at DF on real curve
                        x2=BBICselectWithCF(m).DomF;y2=BBICselectWithCF(m).DomFph;
                        plot(x2,y2,'>','MarkerSize',5,'Color','g','MarkerFaceColor','g');hold on;
                    end;
                    line(x,y,'Marker','o','MarkerSize',1,'MarkerEdgeColor','b','Color','b');grid on;hold on;axis([0 3000 -1.2 1.2]);
        ...
        %if BBICselectWithCF(m).ThrCF>0
            ...
            if (BBICselectWithCF(m).DomF>=(k-1)*100)&(BBICselectWithCF(m).DomF<k*100)
                ...
                if length(MM1)>=2
                    ...
                    Lind=min(find(xdif>0));Rind=max(find(xdif<0));
                    if isempty(Lind)==0 & isempty(Rind)==0
                        BBICselectWithCF(m).DomFph=yn(Lind)+(yn(Rind)-yn(Lind))*(BBICselectWithCF(m).DomF-x(Lind))/(x(Rind)-x(Lind));%Phase at DF on real curve
                        x2=BBICselectWithCF(m).DomF;y2=BBICselectWithCF(m).DomFph;
