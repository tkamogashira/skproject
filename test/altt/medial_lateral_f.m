...
for n=1:length(guinan)
    ...
    plot(guinan(n).MLindex,guinan(n).ep_minus_vp_on_dp_minus_vp,'k*','markersize',6);hold on;
    end;
end;
...
axes(HdlA);
xlabel('medial \leftarrow Normalized ML \rightarrow lateral','fontsize',10);
ylabel('ventral \leftarrow Normalized VD  \rightarrow dorsal','fontsize',10);
title('Contralateral fibers','fontsize',10);
...
structplot(ipsi8831120_cf200Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    ipsi8902122_cf1498Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    ipsi8705624_cf1903Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    ipsi9004315_cf2300Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    ipsi8900739_cf2470Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    ipsi8900743_cf2694Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    ipsi880113_cf5388Hz,'MLindex','ep_minus_vp_on_dp_minus_vp',...
    'info','no','xlim',[0 1],'ylim',[0 1.1],...
    'markers',{'^f','vw'},...
...
xlabel('medial \leftarrow Normalized ML \rightarrow lateral','fontsize',10);
ylabel('ventral \leftarrow Normalized VD  \rightarrow dorsal','fontsize',10);
title('Ipsilateral fibers','fontsize',10);
%Hdls=get(HdlA,'children')
box off;
...
HdlA=copyobj(HdlA,FigHdl);%HdlB=copyobj(HdlB,FigHdl);
set(HdlA,'units','centimeters','position',[2 1 10 6],'tickdir','out','fontsize',8);
axes(HdlA);
xlabel('medial \leftarrow Normalized ML \rightarrow lateral','fontsize',10);
ylabel('ventral \leftarrow Normalized VD  \rightarrow dorsal','fontsize',10);
title('Ipsilateral fibers','fontsize',10);
%Hdls=get(HdlA,'children')
box off;
HdlB2=copyobj(HdlB,FigHdl)
set(HdlB2,'units','centimeters','position',[12.5 1 6 6]);
axes(HdlB2);
