SIZE=size(CFcombiselect4CATS_bump30db);
for k=1:40
    NN=ceil(k/10)-1;
    ...
    for n=1:SIZE(2)
        if (CFcombiselect4CATS_bump30db(n).CF2>=(k-1)*100)&(CFcombiselect4CATS_bump30db(n).CF2<k*100)
            
            x=CFcombiselect4CATS_bump30db(n).IPCx;y=CFcombiselect4CATS_bump30db(n).IPCy;
            difvec=abs(x-ones(1,length(x))*CFcombiselect4CATS_bump30db(n).CF2);
            ...
            for qq=1:length(CFcombiselect4CATS_bump30db(n).ISRy)
                z=CFcombiselect4CATS_bump30db(n).ISRy(qq);
            ...
            xdif=x-ones(1,length(x))*CFcombiselect4CATS_bump30db(n).CF2;
            Lind=min(find(xdif>0));Rind=max(find(xdif<0));
            CFcombiselect4CATS_bump30db(n).CF2ph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselect4CATS_bump30db(n).CF2-x(Lind))/(x(Rind)-x(Lind));
            x2=CFcombiselect4CATS_bump30db(n).CF2;y2=CFcombiselect4CATS_bump30db(n).CF2ph;
            ...
            %xind=find(x>=CFcombiselect4CATS_bump30db(n).CF2*(2^(-1/3)) & x<=CFcombiselect4CATS_bump30db(n).CF2*(2^(1/3)));%Window at CF2
            %xplus=[x2 x(xind)];yplus=[y2 y(xind)];
            %plot(xplus,yplus*1000./xplus);hold on
            
            %find bump
            clear Dif;clear MM1;clear bump;clear DifM;
            ykd=CFcombiselect4CATS_bump30db(n).CPr+(CFcombiselect4CATS_bump30db(n).CD/1000)*x;
            Dif=y-ykd;
            ...
            if length(MM1)>=2
                ...
                bumpM(:,1)=[];
                if CFcombiselect4CATS_bump30db(n).CD>=0
                    [p,q]=max(bumpM(1,:));maxbumpidx=bumpM(2,q);
                else
                    [p,q]=min(bumpM(1,:));maxbumpidx=bumpM(2,q);
                end;
                Low=CFcombiselect4CATS_bump30db(n).CF2*(2^(-1/3));
                High=CFcombiselect4CATS_bump30db(n).CF2*(2^(1/3));
