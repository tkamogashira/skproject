...
for n=1:SIZE(2)
            ...
            difvec=abs(x-ones(1,length(x))*CFcombiselectALL_40(n).CF2);
            ...
            for qq=1:length(CFcombiselectALL_40(n).ISRy)
                z=CFcombiselectALL_40(n).ISRy(qq);
                ...
                %low=CFcombiselectALL_40(n).CF2*(2^(-1/3));
                %high=CFcombiselectALL_40(n).CF2*(2^(1/3));
                if z>0.2
            ...
            CFcombiselectALL_40(n).CF2ph=yn(Lind)+(yn(Rind)-yn(Lind))*(CFcombiselectALL_40(n).CF2-x(Lind))/(x(Rind)-x(Lind));
            ...
            ykd=CFcombiselectALL_40(n).CPr+(CFcombiselectALL_40(n).CD/1000)*x;
            Dif=y-ykd;
    ...
    hold off;title('4CATS 40dB Interaural SyncRate > 0.2');
