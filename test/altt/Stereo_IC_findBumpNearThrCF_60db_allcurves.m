...
for k=1:40
    ...
    for m=1:SIZE(2)
        ...
        if BBICselectWithCF_60db(m).ThrCF>0
            ...
            if (BBICselectWithCF_60db(m).ThrCF>=(k-1)*100)&(BBICselectWithCF_60db(m).ThrCF<k*100)
                ...
                if length(MM1)>=2
                    ...
                    Low=BBICselectWithCF_60db(m).ThrCF*(2^(-1/3));
                    High=BBICselectWithCF_60db(m).ThrCF*(2^(1/3));
                    ...
                    if Low<=BBICselectWithCF_60db(m).sigpX(maxbumpidx)&BBICselectWithCF_60db(m).sigpX(maxbumpidx)<=High
                        ...
                        %plot(BBICselectWithCF_60db(m).ThrCF,BBICselectWithCF_60db(m).ThrCF*BBICselectWithCF_60db(m).CD/1000+BBICselectWithCF_60db(m).CPr,'r.','MarkerSize',8);
    ...
    for m=1:SIZE(2)
        ...
        if BBICselectWithCF_60db(m).ThrCF>0
            ...
            if (BBICselectWithCF_60db(m).ThrCF>=(k-1)*100)&(BBICselectWithCF_60db(m).ThrCF<k*100)
                ...
                Lind=min(find(xdif>0));Rind=max(find(xdif<0));
                if isempty(Lind)==0 & isempty(Rind)==0
                    BBICselectWithCF_60db(m).ThrCFph=yn(Lind)+(yn(Rind)-yn(Lind))*(BBICselectWithCF_60db(m).ThrCF-x(Lind))/(x(Rind)-x(Lind));%Phase at ThrCF on real curve
                    x2=BBICselectWithCF_60db(m).ThrCF;y2=BBICselectWithCF_60db(m).ThrCFph;
