function varargout = EvalTHRs(varargin)
...
if ~isnumeric(Param.thr) | (length(Param.thr) ~= 1) | (Param.thr <= 0), error('Value for property ''thr'' must be positive scalar.'); end
if ~any(strcmpi(Param.tailcutoff, {'yes', 'no'})), error('Value for property ''tailcutoff'' must be ''yes'' or ''no''.'); end
if ~any(strcmpi(Param.fit, {'none', 'runav', 'poly', 'spline'})), error('Value for property ''fit'' must be ''none'', ''runav'', ''poly'' or ''spline''.'); end
if ~isnumeric(Param.smplfactor) | (length(Param.smplfactor) ~= 1) | (Param.smplfactor <= 0) | (mod(Param.smplfactor, 1) ~= 0), error('Value for property ''smplfactor'' must be positive integer.'); end
if ~isnumeric(Param.runav) | (length(Param.runav) ~= 1) | (Param.runav < 0) | (mod(Param.runav, 1) ~= 0), error('Value for property ''runav'' must be positive integer.'); end
%Only checking first character in value of property 'plot' for backward compatibility ...
if ~any(strncmpi(Param.plot, {'y', 'n'}, 1)), error('Value for property ''plot'' must be ''yes'' or ''no''.'); end
if ~any(strcmpi(Param.xscale, {'auto', 'lin', 'log'})), error('Value for property ''xscale'' must be ''auto'', ''lin'' or ''log''.'); end
if ~any(strcmpi(Param.xunit, {'auto', 'hz', 'khz'})), error('Value for property ''xunit'' must be ''auto'', ''hz'' or ''khz''.'); end
...
if isempty(diffs), StartIdx = 1;
else, 
    constIdx = find(diffs == 0);
    if isempty(constIdx), StartIdx = 1;
    else, StartIdx = max(constIdx) + 1; end
end
diffs = diff(Thr(minIdx+1:end));
if isempty(diffs), EndIdx = length(Thr);
else, 
    constIdx = find(diffs == 0);
    if isempty(constIdx), EndIdx = length(Thr);
    else, EndIdx = minIdx + min(constIdx); end
end
%Reduce curve ...
...
if strcmpi(Param.xunit, 'hz') | (strcmpi(Param.xunit, 'auto') & (CalcData.thr.cf <= 1000)),    
    ...
    if ~strcmpi(Param.fit, 'none'), [Xfit, Yfit] = deal(CalcData.fit.freq, CalcData.fit.thr); end
    [CForig, BWforig] = deal(CalcData.thr.cf, CalcData.thr.bwf);
    if ~strcmpi(Param.fit, 'none'), [CFfit, BWffit] = deal(CalcData.fit.cf, CalcData.fit.bwf); end
    FreqUnit = 'Hz';
else,
    [Xorig, Yorig] = deal(CalcData.thr.freq/1e3, CalcData.thr.thr);
    if ~strcmpi(Param.fit, 'none'), [Xfit, Yfit] = deal(CalcData.fit.freq/1e3, CalcData.fit.thr); end
    [CForig, BWforig] = deal(CalcData.thr.cf/1e3, CalcData.thr.bwf/1e3);
    if ~strcmpi(Param.fit, 'none'), [CFfit, BWffit] = deal(CalcData.fit.cf/1e3, CalcData.fit.bwf/1e3); end
    FreqUnit = 'kHz'; 
end
if strcmpi(Param.fit, 'none'),
    DataStr = {sprintf('%.1fdB SPL @ %.0f%s SR = %.2fsp/s Q%d = %.2f', CalcData.thr.minthr, CForig, FreqUnit, CalcData.thr.sr, Param.thr, CalcData.thr.q10)};
else,    
    DataStr = { ...
        sprintf('%.1fdB SPL @ %.0f%s SR = %.2fsp/s Q%d = %.2f (ORIG)', CalcData.thr.minthr, CForig, FreqUnit, CalcData.thr.sr, Param.thr, CalcData.thr.q10); ...
        sprintf('%.1fdB SPL @ %.0f%s Q%d = %.2f (FIT)', CalcData.fit.minthr, CFfit, FreqUnit, Param.thr, CalcData.fit.q10)};
end
...
Str = [Str; DataStr];
AxHDR = axes('Position', [0.10 0.80 0.8 0.20], 'Visible', 'off');
text(0.2, 0.5, Str, 'Units', 'normalized', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center', ...
    'FontWeight', 'bold', 'FontSize', 12);

%Actual threshold curve ...
if ~strcmpi(Param.fit, 'none'), 
    TmpX = denan([Xorig(:); Xfit(:)]); TmpY = denan([Yorig(:); Yfit(:)]);
else, TmpX = denan(Xorig(:)); TmpY = denan(Yorig(:)); end
minFreq = min(TmpX); maxFreq = max(TmpX);
minSPL = min([10; TmpY]) - 10; maxSPL = max(TmpY) + 10;

AxHdl = axes('Position', [0.10 0.50 0.40 0.30], 'Box', 'off', 'TickDir', 'out');
line(Xorig, Yorig, 'LineStyle', '-', 'Marker', 'o', 'Color', 'b');
if ~strcmpi(Param.fit, 'none'), line(Xfit, Yfit, 'LineStyle', ':', 'Marker', 'none', 'Color', 'k'); end
if strcmpi(Param.xscale, 'log') | (strcmpi(Param.xscale, 'auto') & ((maxFreq/minFreq) > 5)),
    set(AxHdl, 'xscale', 'log');
else, set(AxHdl, 'xscale', 'lin'); end
line(CForig, CalcData.thr.minthr, 'LineStyle', 'none', 'Marker', 'o', 'Color', 'r');
if ~strcmpi(Param.fit, 'none'), line(CFfit, CalcData.fit.minthr, 'LineStyle', 'none', 'Marker', '.', 'Color', 'k'); end
line(BWforig, CalcData.thr.minthr([1 1])+Param.thr, 'LineStyle', '-', 'Marker', 'none', 'Color', 'r');
if ~strcmpi(Param.fit, 'none'), line(BWffit, CalcData.fit.minthr([1 1])+Param.thr, 'LineStyle', ':', 'Marker', 'none', 'Color', 'k'); end
axis([minFreq maxFreq minSPL maxSPL]);
xlabel(['Frequency (' FreqUnit ')']); ylabel('Threshold (dB SPL)');

%------------------------------------------------------------------
%AxHdl2 = axes('Position', [0.10 0.10 0.80 0.10])
%AxHdl3 = axes('Position', [0.10 0.25 0.80 0.10])
%AxHdl4 = axes('Position', [0.55 0.50 0.20 0.30], 'Visible', 'off')
%AxHdl5 = axes('Position', [0.75 0.50 0.20 0.30], 'Visible', 'off')
assignin('base','FigHdl',FigHdl);
