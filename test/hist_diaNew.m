...
for k=1:length(dia_contra_all)
    ...
    nlengthsum1=nlengthsum1 + nlength1;
end;
bar(xout1,nlengthsum1,'w');hold on;
 
dia_contra_terminal=structfilter(dia_contra_all,'$BP$ == 0');
Size=size((0:0.1:5),2);
nlengthsum2=zeros(1,Size);
for k=1:length(dia_contra_terminal)
    ...
    nlengthsum2=nlengthsum2 + nlength2;
end;
bar(xout2,nlengthsum2,'facecolor',[0.7 0.7 0.7]);hold on;
...
ylabel('Sum of segment length (\mum)','fontsize',8);%ylim([0 120]);
title('Contralateral segments from ML','fontsize',8);
...
for k=1:length(dia_ipsi_terminal)
    ...
    nlengthsum5=nlengthsum5 + nlength5;
end;
bar(xout5,nlengthsum5,'facecolor',[0.7 0.7 0.7]);hold on;
