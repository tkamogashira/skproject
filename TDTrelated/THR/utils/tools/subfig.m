...
%
%   SUBFIG('calibrate'), SUBFIG('calib') or SUBFIG('cal'), is used to calibrate the
...
%   SUBFIG('calibrate') will ask the user to manually maximize a figure.
%   The position of this figure is then used to get the screensize. Values are
%   saved in a setupfile that are automatically loaded upon execution of the function.
%   If the information is not available in the setupfile, the calibration routine is
...
%
%   See also  SUBPLOT, tosetupfile, fromsetupfile
...
%   Version: 1.0
%
%   Edited by Bas Meenderink, November 16, 2007
...
%Get screen borders from setupfile. If it does not exist, get them 
SB = fromsetupfile('ComputerProps','ScreenBorders','-default',0);

if isstruct(SB) ~= 1,
    %Screen borders were not defined in setupfile --> calibrate to get it
    SB = calibrate_subfig;
end

%"unpack" the struct SB
left_border = SB.left;
right_border = SB.right;
bottom_border = SB.bottom;
top_border = SB.top;
...
elseif nargin == 1
    ...
    % Check if calibration is desired, this step is somewhat obsolete
    if ischar(varargin{1})
        if strcmpi(varargin{1},'calibrate') || strcmpi(varargin{1},'calib')
            % 3) the string 'calibrate' (or 'calib') for setting screen size
            calibrate_subfig;
            return
        end
...
%
function ScreenBorders = calibrate_subfig
...
%Put screen borders in the struct ScreenBorders
ScreenBorders.left = left_border;
ScreenBorders.right = right_border;
ScreenBorders.bottom = bottom_border;
ScreenBorders.top = top_border;
ScreenBorders.unit = 'normalized';

%Save these borders to setupfile for later retrieval
tosetupfile('ComputerProps',ScreenBorders);
