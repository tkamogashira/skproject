...
IncList = Param.inclist(find(~ismember(Param.inclist, Param.exclist)));
T = construct(C(:, find(ismember(FNames, IncList))), IncList);
...
if ~(iscell(Param.inclist) & isempty(Param.inclist)) & ~iscellstr(Param.inclist), error('Property inclist must be a cell-array of strings or the empty cell-array.'); end
if ~(iscell(Param.exclist) & isempty(Param.exclist)) & ~iscellstr(Param.exclist), error('Property exclist must be a cell-array of strings or the empty cell-array.'); end
