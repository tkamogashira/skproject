...
C = varargin{1}; if ~iscell(C) | (ndims(C) > 2), error('First argument must be two-dimensional cell-array.'); end
Param = CheckPropList(DefParam, varargin{2:end});
CheckParam(Param);

%Subdivide the elements of the supplied cell-array in different sets ...
[NRow, NCol] = size(C); NElem = NRow*NCol;
...
if strncmpi(Param.coloriented, 'y', 1), %Column-oriented or not ...
    ...
    [rowNr, colNr] = ind2sub([NRow, NCol], idxScalar); colNrs = unique(colNr);
    for n = colNrs(:)',
        idx = idxScalar(find(colNr == n));
        if all(~mod(cat(2, C{idx}), 1)), idxInteger = [idxInteger; idx];
        else, idxFlPoint = [idxFlPoint; idx]; end
    end
else,    
    idxInteger = idxScalar(find(~mod(cat(2, C{idxScalar}), 1)));
...
if ~isempty(idxEmpty), [CS{idxEmpty}] = deal(Param.emptyformat); end
if ~isempty(Param.charstrformat), CS(idxCharStr) = FormatCharStr(C(idxCharStr), Param.charstrformat); 
else, CS(idxCharStr) = C(idxCharStr); end
...
if ~isempty(idxUnknown), [CS{idxUnknown}] = deal(Param.unknownformat); end
...
NElem = length(CS);
for n = 1:NElem, CS{n} = sprintf(FormatStr, CS{n}); end
...
for n = 1:NElem,
    if (ndims(C{n}) > 2), CS{n} = UnkwnFormatStr;
    elseif all(~mod(C{n}, 1)), CS{n} =  Mat2Str(C{n}, IntFormatStr);
    else, CS{n} =  Mat2Str(C{n}, FlPtFormatStr); end
end
...
if isempty(M),    
    if (Nrows == 0) & (Ncols == 0), Str = '[]';
    else, Str = ['zeros(' int2str(Nrows) ',' int2str(Ncols) ')']; end
    return;
end

if ((Nrows * Ncols) ~= 1), Str = '['; else, Str = ''; end
for i = 1:Nrows
    for j = 1:Ncols
        if (M(i,j) == Inf), Str = [Str,'Inf'];
        elseif (M(i,j) == -Inf), Str = [Str,'-Inf'];
        else,
            Str = [Str,sprintf(FormatStr,real(M(i,j)))];
            if (imag(M(i,j)) < 0), Str = [Str,'-i*',sprintf(FormatStr,abs(imag(M(i,j))))];
            elseif (imag(M(i,j)) > 0), Str = [Str,'+i*',sprintf(FormatStr,imag(matrix(i,j)))]; end
        end
        Str = [Str,','];
    end
    Str(end) = ';';
end
if (Nrows == 0), Str = [Str, ' ']; end
if ((Nrows * Ncols) ~= 1), Str(end)  = ']'; else, Str(end) = []; end
