...
excidx = strmatch(blanks(NCol), CharA, 'exact'); Nexc = length(excidx);
incidx = setdiff(1:NRow, excidx); Ninc = length(incidx);
if (Ninc == 0), Nrs = repmat(NaN, Nexc, N); return; end
CharA = CharA(incidx, :);
...
if ~isequal(Count, N*Ninc), Nrs = [Nrs; repmat(NaN, N*Ninc-Count, 1)]; end;

if length(Nrs) == 2, Nrs = Nrs';
else, 
    idx = repmat((1:N:(N*Ninc))', 1, N) + repmat(0:N-1, Ninc, 1);
    Nrs = Nrs(idx);
end

if (Nexc ~= 0), Nrs(incidx, :) = Nrs; Nrs(excidx, :) = NaN; end