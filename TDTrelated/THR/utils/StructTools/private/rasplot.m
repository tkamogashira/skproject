...
%Checking parameters ...
if (nargin < 1) | ~isa(ds, 'dataset'), error('Wrong input parameters.'); end
switch nargin,
case 1,
    iSubSeqs = 1:ds.nrec;
    AxHdl    = gca;
...
for nSubSeq = iSubSeqs(:)',
    ...
    if ~isempty(LnHdl), Hdl = [Hdl, LnHdl]; end
end

%Setting axis properties ...
%Indepval vector is invalid for incomplete BFS-dataset collected before August 2002 ...
if strcmpi(ds.FileFormat, 'IDF/SPK') & any(strcmpi(ds.StimType, {'BFS', 'FS'})) & (ds.nrec ~= ds.nsub) & ...
   (datenum(ds.Time([3, 2, 1, 4, 5, 6])) < datenum(2002, 8, 1)),
    warning('Incomlete FS-dataset collected before August 2002.');
    Freqs = SPKuetvar(ds); Indepval = Freqs(:, 1);
else, Indepval = ds.indepval; end
...
xlabel('Time (ms)'); ylabel(sprintf('%s (%s)', ds.indepname, ds.indepunit));