...
%   information on the syntax of a confiuration file, see READPROPLIST.

%B. Van de Sande 23-10-2003

%Default parameters ...
DefParam.cellidfields  = {'DF', 'CellNr'};
DefParam.totalidfields = {'DF', 'dsID'};
...
Hdl_Dots = min(line(X1, Y1, 'LineStyle', LineStyle1, 'Marker', Param.markers{1}, 'Color', Param.colors{1}, 'ButtonDownFcn', 'structplot(''CALLBACK'')'));
Hdl_Dots = [ Hdl_Dots min(line(X2, Y2, 'LineStyle', LineStyle2, 'Marker', Param.markers{2}, 'Color', Param.colors{2}, 'ButtonDownFcn', 'structplot(''CALLBACK'')'))];
...
text(0, 0, sprintf('Number of DataPoints: %d', N), 'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom', 'units', 'normalized');
