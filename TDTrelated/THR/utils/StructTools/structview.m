...
%------------------------------default parameters---------------------------
if (nargin >= 1), StructName = inputname(1); else, StructName = ''; end
...
ColumnNChar(find(ColumnNChar > Param.maxcolwidth)) = Param.maxcolwidth;
...
if UD.InCallBack, return; %Gate watch ...
else, 
    UD.InCallBack = logical(1); 
...
for n = 1:NCol, ReOrderIdx(n) = find(ismember(UD.HdrCaption, Captions(n))); end
...
case {0, 2}, SortOrder = 1; case 1, SortOrder = 2; end

if isa(UD.TblData{1, ColNr}, 'double'), [dummy, idx] = sort(cat(1, UD.TblData{:, ColNr}));
elseif isa(UD.TblData{1, ColNr}, 'char'), [dummy, idx] = sort(UD.TblData(:, ColNr)); 
else, warning('Cannot sort according to requested column.'); idx = 1:NRow; end;
...
if ~exist('JTableExt', 'class'), %Slow when using the MATLAB interpreter ...
    ...
    if ~isempty(PrevColNr), CMobj.getColumn(PrevColNr-1).setHeaderValue(UD.HdrCaption{PrevColNr}); end
...
else, JTableExt.updateTable(UD.TblObj, UD.TblDataDisp, UD.HdrCaption); end
...
for n = 0:(NCol-1), Captions{n+1} = CMObj.getColumn(n).getHeaderValue; end
