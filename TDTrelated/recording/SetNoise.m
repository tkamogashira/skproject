function varargout = SetNoise(varargin)
% SETNOISE Application M-file for SetNoise.fig
%    FIG = SETNOISE launch SetNoise GUI.
%    SETNOISE('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.0 29-Mar-2002 19:28:20
...
if nargin == 0  % LAUNCH GUI
    ...
    if exist('SetNoise.mat','file')==2
        load('SetNoise') %Load the settings

        if exist('Setting','var')==1
            SetNoise('SetSetting',Setting);
        end
...
if ~isempty(myh)
    ...
    Setting.UncorrelatedFlag=get(myhandles.check_uncorrelated,'Value');
...
if ~isempty(myh) & ~isempty(Setting)
    ...
    set(myhandles.check_uncorrelated,'Value',Setting.UncorrelatedFlag);
...
NITD=length(ITD);
UncorrelatedFlag=Setting.UncorrelatedFlag; %Fix envelope
