...
NPts=NPtsSilence1+NPtsTone+NPtsSilence2;
TotalDurMs=NPts/Fs*1000;
tTone=(1:NPtsTone)/Fs;
...
for iChan=1:2
    ...
    %Generate sine wave
    if iChan==1
        mySignal=sin(2*pi*Freq*tTone+Phase+CenterIPD);
    else
        mySignal=sin(2*pi*Freq*tTone);        
    end
    %Apply ramps
    mySignal=Cos2RampMs(mySignal,Ramp,Fs);

    %store in the mat
    Signal(iChan,NPtsSilence1+(1:NPtsTone))=mySignal;
end
