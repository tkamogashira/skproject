...
if (nargin == 1),
    ...
    if (length(Hdls) ~= 1) | ~strcmpi(get(Hdls, 'Type'), 'axes'), error('Figure contains no or more than one axes object.');
    else, AxHdl = Hdls; return; end
elseif (nargin == 2),
    if isa(varargin{1}, 'char'),
        LocStr = lower(varargin{1});
        if ~any(strcmp(LocStr, {'ul', 'll', 'lr', 'ur'})), error('Invalid location string.'); end
        AxHdls = GetAxHdls(FigHdl); 
        NHdls = length(AxHdls); if (NHdls == 0), error('No axes objects on figure.'); end
        switch LocStr
    ...
    elseif isa(varargin{1}, 'numeric'),
        Loc = varargin{1};
        if (length(Loc) ~= 1) | (Loc <= 0)| (mod(Loc, 1) ~= 0), error('Location should be positive integer.'); end
        AxHdls = GetAxHdls(FigHdl);
        NHdls = length(AxHdls);
        if (NHdls == 0), error('No axes objects on figure.'); end
        if (Loc > NHdls), error('Location doesn''t exist.'); end
        AxHdls = SortAxHdls(AxHdls);
        AxHdl = AxHdls(Loc);
...
Hdls = Hdls(idx);
if isempty(Hdls), error('Figure contains no axes objects.'); end  

%-----------------------------------------------------------------------------
function AxHdls = SortAxHdls(AxHdls)

[X, Y] = GetLLCorners(AxHdls);
[dummy, idx] = sort(X); idx = idx(end:-1:1); Y = Y(idx); AxHdls = AxHdls(idx);
[dummy, idx] = sort(Y); idx = idx(end:-1:1); AxHdls = AxHdls(idx);
...
if (length(AxHdls) > 1), Pos = cat(1, Pos{:}); end
X = Pos(:, 1); Y = Pos(:, 2);
