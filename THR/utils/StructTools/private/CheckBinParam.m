function boolean = CheckBinParam(ds, ParamType)
...
elseif ~isa(ds, 'dataset'), error('First argument should be dataset object.');
elseif ~ischar(ParamType) | ~strcmpi(ParamType, {'itd', 'bb'}), error('Second argument should be ''itd'' or ''bb''.'); end

%Thorough check of supplied dataset ...
[dsType, ChanNr] = CheckDataSet(ds); if (ChanNr ~= 2), error('Supplied dataset must have two active channels.'); end
...
case {'sgsr', 'idf/spk'},
    ...
    if isempty(LUTEntry), error(sprintf('Dataset with stimulus type ''%s'' doesn''t have binaural parameter of type ''%s''.', ds.StimType, ParamType)); end
    ...
    case 'itd',
        ...
        if ~strcmpi(RecSideOrigin, 'ud'), WrongRecSide = logical(0); warning(sprintf(WarnMsg, 'recording side'));
        else, WrongRecSide = ~strcmpi(RecSide, ExtractRecSide(ds, dsType, 'ds')); end
              
        if ~strcmpi(ChanMapOrigin, 'ud'), WrongChanMap = logical(0); warning(sprintf(WarnMsg, 'channel mapping'));
        else, WrongChanMap = ~strcmpi(Chan1atEar, ExtractChanMapping(ds, dsType, 'ds')); end
    ...
    case 'bb',
        ...
        %frequency administered at the channel is always in the first column and the frequency for the
        %second channel in the second column. The beat frequencies are calculated by subtracting the
        %frequency of the first channel from those of the second channel ...
        if strcmpi(LUTEntry.StorageType, 'fbeat'), Fbeat = GetBinParamValues(ds, LUTEntry.FieldName);
        else, Fbeat = diff([GetBinParamValues(ds, LUTEntry.FieldName, 2), GetBinParamValues(ds, LUTEntry.FieldName, 1)], 1, 2); end
        if isempty(Fbeat) | (~all(Fbeat > 0) & ~all(Fbeat < 0)), error('No valid beat frequency present in supplied dataset.'); end
        
        %Channel 1 associated with the ipsilateral ear, so when the beat frequency is positive 
        %(this is when the frequency administered at the second channel is higher) this denotes a higher
        %frequency supplied at the contralateral ear ...
        if isequal(Chan1atEar, RecSide), boolean = all(Fbeat > 0);
        %Channel 1 is contralateral ear, so when the beat frequency is positive (this is when
        %the frequency administered at the slave DSS is higher) this denotes a higher frequency supplied
        %at the ipsilateral ear ...
        else, boolean = ~all(Fbeat > 0); end
    end
case 'edf',
    ...
    if (MasteratChanNr == 1), MasteratEar = Chan1atEar;
    elseif strcmpi(Chan1atEar, 'l'), MasteratEar = 'r';
    else, MasteratEar = 'l'; end
    ...
    case 'bb',
        ...
        %frequency supplied at the contralateral ear ...
        if isequal(MasteratEar, RecSide), boolean = all(Fbeat > 0);
        %Master DSS is contralateral ear, so when the beat frequency is positive (this is when
        %the frequency administered at the slave DSS is higher) this denotes a higher frequency supplied
        %at the ipsilateral ear ...
        else, boolean = ~all(Fbeat > 0); end
    end
otherwise, error(sprintf('Dataset type ''%s'' not yet implemented.')); end
...
if (nargin == 2), Source = 'ud'; end
[DataFile, CellNr] = deal(ds.FileName, ds.icell); %Information for messages ...

try, %Extract recording side from UserDataInterface(UDI) ...
    Origin = 'ud'; 
    
    if strcmpi(Source, 'ds'), error('skipuserdata'); end
    UD = struct([]); try, UD = GetUserData(ds); end
    if isempty(UD), error('nouserdata'); end
    if ~ismember('RecSide', fieldnames(UD.CellInfo)), error('nofield'); else, RecSide = UD.CellInfo.RecSide; end
    if ~strncmpi(RecSide, {'l', 'r'}, 1), error('wrongvalue'); else, RecSide = lower(RecSide(1)); end
    ...
    case 'wrongvalue', warning(sprintf('Recording side for cell %d in userdata of datafile ''%s'' is not valid.  Trying to extract recording side from dataset.', CellNr, DataFile)); end    
    ...
    case 'edf', error('Recording side could not be determined for supplied dataset.'); end    
end
...
try, %Extract channel mapping from UserDataInterface(UDI) ...
    ...
    switch lower(dsType),
    case {'sgsr', 'idf/spk'},
        if ~ismember('Chan1', fieldnames(UD.CellInfo)), error('nofield'); 
        else, Chan1atEar = UD.CellInfo.Chan1; end
    case 'edf',
        if ~ismember('DSS1', fieldnames(UD.CellInfo)), error('nofield'); 
        else, Chan1atEar = UD.CellInfo.DSS1; end
    end        
    if ~strncmpi(Chan1atEar, {'l', 'r'}, 1), error('wrongvalue'); else, Chan1atEar = lower(Chan1atEar(1)); end
catch,
    Origin = 'ds';
    ...
    case 'sgsr',
        if isfield(ds.SessionInfo, 'leftDACear'), 
            ...
            if strncmpi(Chan1atEar, {'l', 'r'}, 1), Chan1atEar = lower(Chan1atEar(1));
            else, error('Channel mapping derived from dataset is invalid.'); end
        else, Chan1atEar = 'l'; end
    case 'idf/spk', Chan1atEar = 'l';
    case 'edf', error('Channel mapping could not be determined for supplied dataset.'); end    
end    
...
FNames = CleanStr(Words2Cell(FieldName, ',')); N = length(FNames);
if (N > 1), %Recursion ...
    for n = 1:N, 
    ...
    if (nargin == 3), ChanNr = varargin{1}; FieldName = sprintf(FieldName, ChanNr); end
    ...
    try,
        ...
        if all(isnan(V)) | all(V == 0), error('To catch block ...'); end
    catch, V = []; end
end
