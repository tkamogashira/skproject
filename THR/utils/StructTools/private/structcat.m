...
%   S = STRUCTCAT(S1, S2, ..., Sn) makes new structures S with fieldnames of S1, S2, ... and Sn.
...
elseif nargin > 2, %Iteratie ...
    S = varargin{1};
    for n = 2:nargin, S = structcat(S, varargin{n}); end
    return;
else, [S1, S2] = deal(varargin{1:2}); end
...
try S = cell2struct(C, F);
catch error('Fieldnames of structures are not unique.'); end
