...
MaxNBranches = max(sum(ismember(char(F), '.'), 2)) + 1; NLeaves = length(F);
FieldMap = repmat({''}, NLeaves, MaxNBranches); FieldLayout = [];
for n = 1:NLeaves,
    Path = Words2cell(F{n}, '.'); NEntries = length(Path);
...
for nBranch = (NBranches-1):-1:1,
    idx = find(~cellfun('isempty', FieldMap(:, nBranch)));
    SuperFields = unique(MergeFNames(FieldMap(idx, 1:nBranch))); NSuperFields = length(SuperFields);
    ExcIdx = [];
    ...
    FieldMap(:, nBranch+1) = []; FieldMap(ExcIdx, :) = [];
    C(:, ExcIdx) = [];
end
...
[Nr, Nc] = size(FieldMap); Sep = repmat('.', Nr, 1); 
FNames = []; for n = 1:(Nc-1), FNames = [FNames, char(FieldMap(:, n)), Sep]; end
