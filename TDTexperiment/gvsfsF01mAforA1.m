...
%MAXmA=4
if MAXmA>=1.5
    error('The max current has to be below 1.5mA.')
end;
RMSmA=MAXmA/5 % rmsVがmaxVの1/5なら早くループから抜けられる

rmsV=RMSmA*50 %アイソレータが0.1mA[10]0の場合
maxV=MAXmA*50 %アイソレータが0.1mA[10]0の場合
...
assignin('base','Fs',Fs);
L=round(Duration*Fs/2);PassBand=[1 500];%Duration=3.5s.
...
limit=300;limitM=limit;%アイソレータが0.1mA[10]0の場合
while limit>=maxV
...
Fin_MAXmA=Fin_maxV*0.02;%アイソレータが0.1mA[10]0の場合
Fin_RMSmA=Fin_rmsV*0.02;%アイソレータが0.1mA[10]0の場合
...
subplot(4,1,3)
plot(time,(bn*0.02),'b',time,tr,'r');%アイソレータが1mA[10]0の場合
set(gca,'Ygrid','on');set(gca,'Xgrid','on');grid(gca,'minor')
ylabel('[mA]');
