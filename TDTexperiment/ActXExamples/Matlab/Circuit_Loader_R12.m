...
%     Note: code must be modified to work with non-RP2 devices or with *.rco files
    ...
    invoke(RP,'ConnectRP2',connectionType,deviceNumber); % Connects RP2 via USB or GB given the proper device number
    invoke(RP,'Halt'); % Stops any processing chains running on RP2
    ...
    status=double(invoke(RP,'GetStatus')); % Gets the status
