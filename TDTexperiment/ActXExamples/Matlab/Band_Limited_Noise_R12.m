...
if (invoke(RP,'GetStatus'))==7 % checks to see that the circuit was loaded correctly
    ...
    invoke(RP,'SetTagVal','Gain',Gain); % Gain of band limited filter
    invoke(RP,'SetTagVal','Freq',Freq); % CenterFrequency
    invoke(RP,'SetTagVal','BW',Bandwidth); % Bandwidth of filter
    invoke(RP,'SetTagVal','Amp',Amp); % Amplitude of the Gaussian Noise
    invoke(RP,'SetTagVal','Enable',1); % Loads Coefficients to Biquad Filter
    invoke(RP,'SetTagVal','Enable',0); % Stops Coefficient generator from sending signal (saves on cycle usage)
    
	quit=0;
	while quit==0
   	Clip=invoke(RP,'GetTagVal','Clip'); % Checks to see if signal is clipped (top light on panel is on while clipping occurs)
   	if Clip==1
      	...
      	if a==1
         	...
         	invoke(RP,'SetTagVal','Gain',Gain); % Alters coefficient Gain
      	elseif a==2
         	Amp=input('Enter Amplitude value:' );
         	invoke(RP,'SetTagVal','Amp',Amp); % Alters noise amplitude
        elseif a==3
         	Amp=input('Enter Amplitude value:' ); % Alters both
         	invoke(RP,'SetTagVal','Amp',Amp);
				Gain=input('Enter Gain value:' );
         	invoke(RP,'SetTagVal','Gain',Gain);
        else
            disp('Error: Invalid response');
      	end
      	invoke(RP,'SetTagVal','Enable',1); % Starts the coefficient generator
      	invoke(RP,'SetTagVal','Enable',0); % Stops ParCoef generation
   	end
