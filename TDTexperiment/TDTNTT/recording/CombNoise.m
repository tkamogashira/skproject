...
if NDelay
    n=length(mynoise)-NDelay;
    mynoise1=mynoise(1:n);
    mynoise2=mynoise(NDelay+(1:n));
    mynoise=mynoise1+mynoise2;
end
RMS=[1; 1]*sqrt(mean(mynoise.^2));
mynoise=mynoise(1000+(1:NPts4Noise)); %Remove the first points
%mynoise=mynoise(1000+NDelay+(1:NPts4Noise)); %Remove the first points
