function Rtn=RunSeriesM
%RunSeriesM -- Run an experimental series
%Run from ExptManM
%By SF, 10/9/01

global STIM StimType RESPONSE ExptManParam RunSeriesStatus CommandStatus H_EXPTMANM Gain
...
if ExptManParam.Frozen
    for iStim=1:NStim
        ...
        ExptManM('ShowMessage',mystr); %Show message
...
ExptManM('ShowMessage',RunSeriesStatus); %Show message

%Reset the data for monitor
if ExptManParam.ChanFlag(1)
    Monitor1 ResetData;
end
if ExptManParam.ChanFlag(2)
    Monitor2 ResetData;
end
if ExptManParam.ChanFlag(3)
    Monitor3 ResetData;
end
if ExptManParam.ChanFlag(4)
    Monitor4 ResetData;
end
...
while 1 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    ExptManM('UpdateRepNo',RepCount); %Show the current rep #
    ...
    for iStim=1:NStim %%%%%%%%%%%%%
        ...
        ExptManM('UpdateCondNo',StimIdx); %Show the current stim idx
        ...
        ExptManM('ShowCurrentGain',num2str(Gain)); %
        ...
        while 1 %Repeat the stim until recording is made -- for the purpose of error trapping for RA16
            ...
            %Arm RA16
            RecDur=Duration;
            e=e*RA16RecordEzM('SetRecDur',RecDur);
            e=e*RA16RecordEzM('ResetIndex');
            if ~e
            ...
            pause(RecDur/1000);
            LastIndex=ones(sum(ExptManParam.ChanFlag>0),1)*(-100); %Dummy number
            CurrentIndex=ones(sum(ExptManParam.ChanFlag>0),1); %Dummy number
            t0=cputime;
            ErrorFlag=0;
            while 1
                myidx=RA16RecordEzM('GetIndexRaw',find(ExptManParam.ChanFlag));
                CurrentIndex=myidx(find(ExptManParam.ChanFlag));
                %Stop waiting when all the indeces stop increasing
                if all(CurrentIndex(:)==LastIndex(:)) & all(CurrentIndex(:))
                    ErrorFlag=0;
                    break;
                elseif any(CurrentIndex(:)<LastIndex(:))
                    error('Wrapping detected -- Something is wrong with RA16.');
                else
                    LastIndex=CurrentIndex;
                end
                if cputime-t0>5 & any(CurrentIndex(:)==LastIndex(:))
                    ErrorFlag=1;
                    break;
                end
            end
            ...
            if ErrorFlag %If error is detected, re-initialize RA16 and retry
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % Get the waveform
                RESPONSE.Wave=RA16RecordEzM('GetRawWave',find(ExptManParam.ChanFlag)); %Waveform
                %RESPONSE.Time=(1:RESPONSE.Wave)/FsRA16*1000; %Time
        ...
        myflag=1;
        for iChan=1:4
            if ~isempty(ExptManParam.Path) & ~isempty(ExptManParam.SeriesNo) & ExptManParam.ChanFlag(iChan)
                
                %Directory and file name for the data
                DirName=fullfile(ExptManParam.Path,sprintf('#%d',ExptManParam.SeriesNo));
                InfoFileName=fullfile(DirName,sprintf('Info_%d.txt',iChan));
                StimFileName=fullfile(DirName,'StimInfo');
                WaveFileName=fullfile(DirName,sprintf('%03d_%d.raw',StimIdx,iChan));
                MaxVolt=ExptManParam.MaxVolt;
                NPts=length(RESPONSE.Wave{iChan});
                
                %Compress the waveform to 16-bit integer
                mywave=RESPONSE.Wave{iChan}/MaxVolt*1000; %Normalize the response to MaxVolt
...
if ~isempty(ExptManParam.Path) & ~isempty(ExptManParam.SeriesNo)
    DirName=fullfile(ExptManParam.Path,sprintf('#%d',ExptManParam.SeriesNo));
    for ii=1:4
        if ExptManParam.ChanFlag(ii)
            myname=['Monitor' num2str(ii) 'Setting'];
            
            %Directory and file name for the data
            MonitorFileName=fullfile(DirName,myname);
            save(MonitorFileName,myname);
        end
    end
end    
...
%ExptManM LastSeriesNo %Last series number
ExptManM('ShowMessage',''); %Show message
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
