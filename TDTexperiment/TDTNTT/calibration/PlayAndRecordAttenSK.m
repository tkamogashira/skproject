function [DataOut,Atten,SysEnv]=PlayAndRecordAttenSK(DataIn,NPtsOut,RP2Config,PA5Config,MaxSPL,MicSensitivity);
...
%invoke(PA5,'ConnectPA5',TDTInterface,PA5Config.DevNo);%%%modified by SK
...
while 1
    ...
    %invoke(RP2,'ConnectRP2',TDTInterface,RP2Config.DevNo); %connects an RP2 to a USB or Xbus%%%SK
...
%Coefficients for low-cut filter
[B,A] = ellip(5,0.5,40,50/(Fs/2),'high');
...
for i=1:3
    for ii=1:NAv
        %Play and record
        myDataOut=PlayAndRecord(RP2,DataIn,RP2Config.DAChan,RP2Config.ADChan,NPtsOut,Fs);display(['i=' num2str(i) ';ii=' num2str(ii) 'played'])%%%SK
        if ii==1
            DataOut=myDataOut;
        else
    ...
    if PA5Config.AdjustGainFlag %Adjust gain
        ...
        RMS=sqrt(mean(DataOut.^2))*1e6; %Etymotic ER7C gives 20log10(1uV) for 0 dB SPL %%%SK 
        %CurrentLevel=20*log10(RMS);
        MicSensitivityV=MicSensitivity(1);
        MicSensitivitySPL=MicSensitivity(2);
        %%%RMS=sqrt(mean(DataOut.^2));%%%SK
        ...
        if ~isnan(myAtten)
            invoke(PA5,'SetAtten',myAtten);display(['i=' num2str(i) ';myAtten=' num2str(myAtten)])%%%SK
        else
            break;
        end
    else %Fixed gain
        Atten=PA5Config.InitAtten;display(Atten);display(['i=' num2str(i) ';Atten=InitAtten=' num2str(Atten)])%%%SK
        break;
