function Rtn=GetFiltCoef(ChanNo,PassBand,NFiltCoef,InitialAtten,AdjustAttenFlag,MicSensitivity,PlotFlag)
...
MaxVolt=3; %Maximum voltage allowed to feed to the earphone (Set 3 V for Etymotic ER4)
...
RMS=sqrt(mean([A B].^2)); 
Scale=min([10/abs(max(DataIn(:))) MaxVolt/RMS]);
DataIn=DataIn*Scale;
...
%Scale the signal so that the maximum unsigned amplitude is no greater than 10 V
%RMS=sqrt(mean(FiltNoise(1:2*L).^2)); 
%RMS=sqrt(mean(FiltNoise.^2)); 
Scale=10/max(abs(FiltNoise(:)));
...
%keyboard
%Play and record
[DataOut,Atten,SysEnv]=PlayAndRecordAtten(FiltNoise,length(FiltNoise),RP2Config,PA5Config,MaxSPL,MicSensitivity);
