function [F0, ttf, A2, A3, A4, A5, A6, AB, AV, AH, AF, AVS, tt, F, BW] = makespeech2ss(utt)
...
id = getid(utt(i));
    if id >= 24 & id < 53
        tw = segments(id, 33); % adjust length of vowel according to parameters in col 33 above. =Weight
        vowels = vowels + 1;
        if i == 1
            tt(length(tt)+1) = 0;
        else
        ...
        %%% vowellength=2100;
        vowellength=1100;
        tt(length(tt)+1) = round(tt(length(tt))+(vowellength/2*tw));
    ...
    elseif id <=23 %
        ...
        if id <=15
            %%% frisonolength1=2000;
            frisonolength1=1000;
            time1 = frisonolength1;
            %%% frisonolength2=500;
            frisonolength2=250;
            time2 = frisonolength2;
            fric = 0; % values for fricatives, and sonorant consonants    
        elseif id >= 21
            lessstoplength1=100;
            time1 = lessstoplength1;
            lessstoplength2=600;
            time2 = lessstoplength2;
            fric = 30; stop = 1; % values for voiceless stops
            
            lessstoplength3=500;
            tt(length(tt)+1) = tt(length(tt))+lessstoplength3;
            [y x] = size(vals);
            vals(y+1:y+2,:) = [segments(id, :); segments(id, :)];
            vals(y+1:y+2, 30) = [0; 0];
            lessstoplength4=200;
            tt(length(tt)+1) = tt(length(tt))+lessstoplength4;
        ...
        elseif id == 16 | id == 17
            ...
            fric = 10; % affricates  
            affrilength3=500;
            tt(length(tt)+1) = tt(length(tt))+affrilength3;
            [y x] = size(vals);
            vals(y+1:y+2,:) = [segments(id, :); segments(id, :)];
            vals(y+1:y+2, 30:31) = [0, 0; 0, 0;];
            affrilength4=200;
            tt(length(tt)+1) = tt(length(tt))+affrilength4;
            ...
            stoplength2=600;
            time2 = stoplength2;
            fric = 10; stop = 1;% values for voiced stops
        end
