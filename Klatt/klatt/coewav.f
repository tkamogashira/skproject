...
C   MAXIMUM VALUE FOR A WAVEFORM SAMPLE (LEFT-JUSTIFIED IN 36-BIT WORD)
...
C   SET AMPLITUDE OF NORMAL VOICING IMPULSE
      INPUT=IMPULS
C   AMPLITUDE OF QUASI-SINUSOIDAL VOICING
      INPUTS=SINAMP
      GO TO 275
C   SET INPUT TO ZOER BETWEEN GLOTTAL IMPULSES
260   INPUT=0.
      INPUTS=0.
C   RESONATOR RGP:
...
C   GLOTTAL SOURCE VOLUME VELOCITIY = VOICING+ASPIRATION
      AASPIR=AASPIR+DAHH
      UASP=AASPIR*NOISE
...
C   IF PLOSIVE RELEASED (I.E. IF PLSTEP.GT.0)
...
C   EXCITE FORMANT RESONATORS R5'-R6' WITH FRIC NOISE
      ...
      YL62P=YL61P
      YL61P=Y6P
C   ADD UP OUTPUTS FROM AN', R1' - R6' AND BYPASS PATH
