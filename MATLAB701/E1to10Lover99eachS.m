% E1to10Lover99eachS
function H=E1to10Lover99eachS(S,M,N,V)
...
L=find(V(2,(1:202))>-1);
for f=1:123
    I1=(Sn(f,L)>mean(Mn(f,L))+std(Mn(f,L))*2.33);
    I2=(Sn(f,L)>mean(Nn(f,L))+std(Nn(f,L))*2.33);
    J1=(Mn(f,L)>mean(Sn(f,L))+std(Sn(f,L))*2.33);
    J2=(Mn(f,L)>mean(Nn(f,L))+std(Nn(f,L))*2.33);
    H(f,1)=nnz(I1.*I2);
    H(f,2)=nnz(J1.*J2);
end;
assignin('base','H',H);
