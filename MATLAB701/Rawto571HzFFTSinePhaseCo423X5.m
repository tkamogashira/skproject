% Raw to 571HzFFTSinePhaseCo 423pointsX 5zones
function RESULTS=Rawto571HzFFTSinePhaseCo423X5(X,L,ch)
%X:averaged data during 4BB ex)O2l240r24571attend
%L:fft data by channels202XY ex)O2l240r24571attend_fft_SelectChs95newC
%ch:channels204XY5

% 40Hz low-pass filter
[b,a] = ellip(10,3,100,40/300);
X2=filtfilt(b,a,X(:,(1:204)));
% baseline through 700ms
...
for m=1:size(M,2)
    Mdata((1:423),m)=X3((1:423),(ch(1,:)==M(1,m)));
end;
...
% 571Hz sine waves 5zones
T=[-0.35:(1/600):(-0.35+(422/600))];
for b=1:size(M,2)
    Y(:,b)=(A0(1,b)+real(RFFT(4,b))*cos(2*pi*FreqAxis(4)*T)-imag(RFFT(4,b))*sin(2*pi*FreqAxis(4)*T))/(423/2);
end;
...
T1=[-0.0875:(1/600):(-0.0875+(105/600))];
for c=1:size(M,2)
    y(:,c)=(A0(1,c)+real(RFFT(4,c))*cos(2*pi*FreqAxis(4)*T1)-imag(RFFT(4,c))*sin(2*pi*FreqAxis(4)*T1))/(423/2);
...
% Coherence
for e=1:size(M,2)
    [Co,F]=mscohere(Mdata(:,e),Y(:,e),[],[],422,600);
    Co4(1,e)=Co(5);
end;
