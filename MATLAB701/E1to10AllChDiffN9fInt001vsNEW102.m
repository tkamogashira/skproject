% E1to10AllChDiffN9fInt001vsNEW102
function SigChN=E1to10AllChDiffN9fInt001vsNEW102(M1,M2,M3,M4,M5,M6,M7,M8,M9,N1,N2,N3,N4,N5,N6,N7,N8,N9,V)
...
for C=1:100
    for fp=8:116
        if mean(mean(MM(((fp-7):(fp+7)),C,:),1),3)<mean(mean(NN(((fp-7):(fp+7)),C,:),1),3)
            P(fp,C)=1;
        else
            [H,P(fp,C),ci]=ttest((mean(MM(((fp-7):(fp+7)),C,:),1)-mean(NN(((fp-7):(fp+7)),C,:),1)),0,0.01,0);
        end;
    end;
end;
...
for Fp=8:116
    SigChN(Fp)=sum(P(Fp,(1:100))<0.01);
end;
PlotP=[[P;V] [SigChN 0 0 0 0]'];
assignin('base','PlotP',PlotP);
[i,j]=find(PlotP((1:123),(1:100))<0.01);
X=[i j];
for k=1:size(X,1)
