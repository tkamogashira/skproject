% SS to 666HzFFTSinePhaseCo 375points
function T1=SSto666HzFFTSinePhaseCo375(V)
% FFT of SS
FFT=fft(V);
A0=FFT(1);
FFT(1)=[];
...
Y=(A0+real(RFFT(4))*cos(2*pi*FreqAxis(4)*T)-imag(RFFT(4))*sin(2*pi*FreqAxis(4)*T))/(375/2);
% Phase analysis by radian
T1=[-0.075:0.0016:(-0.075+(93*0.0016))];
y=(A0+real(RFFT(4))*cos(2*pi*FreqAxis(4)*T1)-imag(RFFT(4))*sin(2*pi*FreqAxis(4)*T1))/(375/2);
[Max,MaxPhaseI]=max(y);
...
% Coherence
[Co,F]=mscohere(V,Y,[],[],374,(1/0.0016));
