% SS to 4HzSinePhase 621points +Coherence
function FreqAxis=SSto4HzSinePhase621Co(V)
FFT=fft(V);
A0=FFT(1);
FFT(1)=[];
...
T=[-0.5:0.0016:(-0.5+(620*0.0016))];
Y=(A0+real(RFFT(4))*cos(2*pi*FreqAxis(4)*T)-imag(RFFT(4))*sin(2*pi*FreqAxis(4)*T))/(621/2);
subplot(4,1,2);
plot(T,V,'k-',T,Y,'r-'),grid;
axis([-0.500 0.500 -10 10]);
set(gca,'xtick',[-0.375 -0.125 0 0.125 0.375]);
% Phase analysis
T1=[-0.125:0.0016:(-0.125+(155*0.0016))];
y=(real(RFFT(4))*cos(2*pi*FreqAxis(4)*T1)-imag(RFFT(4))*sin(2*pi*FreqAxis(4)*T1))/(621/2);
[Max,MaxPhaseI]=max(y);
...
% Coherence
[Co,F]=mscohere(V,Y,[],[],620,(1/0.0016));
