% E1to10Lanova2non4BB
function H4L=E1to10Lanova2non4BB(S1,S2,S3,S4,S5,S6,S7,S8,S9,M1,M2,M3,M4,M5,M6,M7,M8,M9,N1,N2,N3,N4,N5,N6,N7,N8,N9,V)
...
for f=1:123
    ...
    if (mean(mean(Sn(f,L,:),2),3)>mean(mean(Mn(f,L,:),2),3))&(mean(mean(Sn(f,L,:),2),3)>mean(mean(Nn(f,L,:),2),3))
        p1=anova2(X,9,'off');
        H4L(f,(1:3))=p1;
    else
        H4L(f,(1:3))=[1 1 1];
