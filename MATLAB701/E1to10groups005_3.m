% E1to10groups005_3
function SigChN4=E1to10groups005_3(BB4,BB6,BBN,V)
...
for C=1:202
    for f=8:116
        if (sum(find((BB4(f,C,:)-BBN(f,C,:))>0))==9)&(mean(BB4(f,C,:),3)>mean(BB6(f,C,:),3))
...
for D=1:202
    for g=8:116
        if (sum(find((BB6(g,D,:)-BBN(g,D,:))>0))==9)&(mean(BB6(g,D,:),3)>mean(BB4(g,D,:),3))
