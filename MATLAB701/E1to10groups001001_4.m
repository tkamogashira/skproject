% E1to10groups001001_4
function SigChN4=E1to10groups001001_4(BB4,BB6,BBN,V)
...
for C=1:202
    for f=8:116
        if (mean((BB4(f,C,:)-BBN(f,C,:)),3)>mean((BB6(f,C,:)-BBN(f,C,:)),3))&(mean((BB6(f,C,:)-BBN(f,C,:)),3)>0)
            [H,P4(f,C),ci]=ttest((BB4(f,C,:)-BBN(f,C,:)),(BB6(f,C,:)-BBN(f,C,:)));
        else
            P4(f,C)=1;
        end;
    end;
...
for D=1:202
    for g=8:116
        if (mean((BB6(g,D,:)-BBN(g,D,:)),3)>mean((BB4(g,D,:)-BBN(g,D,:)),3))&(mean((BB4(g,D,:)-BBN(g,D,:)),3)>0)
            [H,P6(g,D),ci]=ttest((BB6(g,D,:)-BBN(g,D,:)),(BB4(g,D,:)-BBN(g,D,:)));
        else
            P6(g,D)=1;
        end;
    end;
