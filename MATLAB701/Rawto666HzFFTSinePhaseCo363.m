% Raw to 666HzFFTSinePhaseCo 363points
function T1=Rawto666HzFFTSinePhaseCo363(X,M,ch)
...
for b=1:size(M,2)
    ...
    if M(2,b)==1
        plot(T,Y(:,b),'b-'),grid,hold on;
    elseif M(2,b)==2
        plot(T,Y(:,b),'r-'),grid,hold on;
    else
        plot(T,Y(:,b),'k-'),grid,hold on;
    end;
    axis([-0.3 0.3 -10 10]);set(gca,'xtick',[-0.225 -0.075 0 0.075 0.225]);
end;
grid;
