function [e1_fs2, e2_fs1, b, zfilt1cell, zfilt2cell, zfilt1wave, zfilt2wave, chim12cell, chim21cell] = multi_band_chimera2(x1, x2, Fco, Fs, refilter, border)
...
zfilt1cell={};zfilt2cell={};
zfilt1wave={};zfilt2wave={};
chim12cell={};chim21cell={};
% loop over filters in bank 
for k = 1:size(b,2),

	zfilt1 = fftfilt(b(:,k), x1);zfilt1cell{k,1}=zfilt1;
	zfilt2 = fftfilt(b(:,k), x2);zfilt2cell{k,1}=zfilt2;
	
    zfilt1wave{k,1}= abs(zfilt1) .* cos(angle(zfilt1));
    zfilt2wave{k,1}= abs(zfilt2) .* cos(angle(zfilt2));
    
	% interchange envelope and fine structure	
  	e1_fs2_filt = abs(zfilt1) .* cos(angle(zfilt2));chim12cell{k,1}=e1_fs2_filt;
    e2_fs1_filt = abs(zfilt2) .* cos(angle(zfilt1));chim21cell{k,1}=e2_fs1_filt;
