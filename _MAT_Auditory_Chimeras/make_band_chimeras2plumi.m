function [env1_fts2, Fs]=make_band_chimeras2plumi(name1, name2, Nbands, do_play, do_plot, border)
...
for nb = Nbands,
    ...
    assignin('base',['Fco_' num2str(nb)],Fco);
        ...
        [env1_fts2, env2_fts1, b, zfilt1cell, zfilt2cell, zfilt1wave, zfilt2wave] = multi_band_chimera2(orig1, orig2, Fco, Fs, refilter, border);
...
%     xlabel('Frequency(Hz)');ylabel('Magnitude(dB)');
    plot(20*log10(abs(h)), f);hold on;
    plot([0 0],[Fco(s) Fco(s+1)],'r*');hold off;
    ylabel('Frequency(Hz)');xlabel('Magnitude(dB)');
    %%%set(gca,'YScale','log');
    ylim([100 Fs/2]);grid on;
    
    subplot(nb,4,4*s-2)
    spectrogram( zfilt1wave{s,:}, 128,[],100,Fs,'yaxis');colorbar;
...
%     ylim([100 Fs/2]);
    ...
    subplot(nb,4,4*s)
    spectrogram( zfilt2wave{s,:}, 128,[],100,Fs,'yaxis');colorbar;
