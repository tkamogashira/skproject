...
% Write full sine and cosine table for HDL NCO
...
if(~LUTCompress)
    ...
    % LUT compress algorithm
    WL_lut =  quantWL-2;
    bs = ceil(WL_lut/3);
    A = 2*pi*2^-(bs+2); %0~pi/4
    B= 2*pi*2^-bs*2^-(bs+2);
    Cbs = WL_lut-2*bs;
...
if strcmpi(OutputDataType,'Binary point scaling')
    
    quarterSine = fi(quarterSineDouble, outType, Fsat);
    if LUTCompress
        % optimize the data word length for second LUT
