...
switch action
  ...
  otherwise % should not come here
      error(message('dsp:dspblkfblms:unhandledCase'));
end
...
newflags = {'off', 'on', 'on', 'on', 'on', 'on', 'off', 'on', 'on', 'on', 'on'};
if wantStepPort
...
if (errorstatus)
    error(message('dsp:dspblkfblms:paramOutOfRange'));
end
...
if (noStepPort)
    ...
    if (errorstatus)
        error(message('dsp:dspblkfblms:paramRealScalarError'));
    end
end
...
if (errorstatus)
    error(message('dsp:dspblkfblms:invalidFilterOrder'));
end
