function [s] = dspblklmsupdate(action)
...
switch action
  ...
  otherwise % should not come here
      error(message('dsp:dspblklmsupdate:unhandledCase'));
end
...
if wantStepPort && ~isStepPortPresent,
    ...
    % remove gain block
    Delete_Block_Between(blk, 'mu', fromblk, 'Product/1');
elseif ~wantStepPort && isStepPortPresent,
    % Remove ports
    delete_line(blk, 'Stepsize/1', 'Product/3');
    delete_block([blk, '/Stepsize']);
    Add_Block_Between(blk, 'mu', 'built-in/Gain', fromblk, 'Product/1',...
                [170    80   210   110] );
...
if (strcmp(Algo, 'Sign-Data LMS') || strcmp(Algo, 'Sign-Sign LMS'))
    ...
    if (exist_block(blk, 'conj'))
        sbpos = get_param([blk '/conj'],'Position');
        Delete_Block_Between(blk, 'conj', 'Input/1', 'Product/2');
        Add_Block_Between(blk, 'SignSig', 'built-in/Signum',...
    ...
    if (exist_block(blk, 'SignSig'))
        ...
        Delete_Block_Between(blk, 'SignSig', 'Input/1', 'Product/2');
        Add_Block_Between(blk, 'conj', 'built-in/Math', 'Input/1', 'Product/2', sbpos);
...
if (leakage < 0 || leakage > 1)
    error(message('dsp:dspblklmsupdate:paramOutOfRange'));
end

% -----------------------------------------------
function Add_Block_Between(sys, name, libname, betweenthis, betweenthat, athere)
% Adds block libname as 'name' to 'sys' 'betweenthis' and 'betweenthat' 
% positioned 'athere'.
...
% -----------------------------------------------
function Delete_Block_Between(sys, name, betweenthis, betweenthat)
