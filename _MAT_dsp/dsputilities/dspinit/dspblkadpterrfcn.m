function [emsg] = dspblkadpterrfcn(bh, ~, oldME)
...
blk = -1;
if (strfind(get_param(bh,'MaskInitialization'), 'dspblklms'))
    blk = LMS; 
end
if (strfind(get_param(bh,'MaskInitialization'), 'dspblkfblms')) 
    blk = BLMS; 
end
if (strfind(get_param(bh,'MaskInitialization'), 'dspblkrls')) 
    blk = RLS; 
end
if (blk == -1) 
    return; 
...
if (blk == BLMS)
    if (~isempty(strfind(eid, 'SL_PortDimsMismatch')) ||...
	~isempty(strfind(eid, 'Simulink:blocks:AssignmentInvDataPortWidth')) )
