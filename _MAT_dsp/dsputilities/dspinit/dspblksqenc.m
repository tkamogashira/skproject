...
% DSPBLKSQENC DSP System Toolbox Scalar quantization encoder mask helper function.
...
case 'dynamic'
  ...
  %---------------------------------------------------------------------------
  % STEP-2:
  ALWAYS_ON_ITEMS = [puPARAMSRC puPARTTYPE puSEARCHMETHOD puTIEBREAKRULE cbOUTQU cbOUTQERROR ...
                     puIDTYPE puROUNDINGMODE cbOVERFLOWMODE];
  ...
  cbADDITIONALPARAMSstatus = 0; % permanently disable checkbox

  NOT_ALWAYS_ON_ITEMSstatus = [ebBBOUNDARYstatus ebUBOUNDARYstatus ebCODEBOOKstatus cbOUTSTATUSstatus ...
                               puINVALIDINstatus cbALLOWOVERRIDESstatus cbADDITIONALPARAMSstatus];
  mask_enables(NOT_ALWAYS_ON_ITEMS) = {'off'};
  for i=1:length(NOT_ALWAYS_ON_ITEMS)
      if NOT_ALWAYS_ON_ITEMSstatus(i),
          mask_enables{NOT_ALWAYS_ON_ITEMS(i)}   = 'on';
      end
  end
  ...
  if (~isequal(mask_enables, old_mask_enables))
      ...
      % Mask visibilities and mask enables must be in sync
      set_param(blkh, 'MaskVisibilities', mask_enables);
  end
