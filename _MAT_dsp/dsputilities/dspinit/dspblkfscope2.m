...
% DSPBLKFSCOPE2 DSP System Toolbox vector scope block helper function.
...
% is this callback for a vector scope or from the Eye Rendering block
sfunname = 'sdspfscope2'; % default value for sfunction name
% find out if this is an eye diagram or xy block so we can change the
...
case 'init'
    ...
    varargout{1} = s;
    feval(sfunname,[],[],[],'CheckParams',blk,s);  
...
case 'dynamic'
    ...
    otherwise
        error(message('dsp:dspblkfscope2:unhandledCase'));
    end
...
% Open tab number tabNum (1 through 4)
