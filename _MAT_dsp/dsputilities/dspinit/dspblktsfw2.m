...
% DSPBLKTSFW DSP System Toolbox Triggered Signal From Workspace block helper function.
...
function argsOut = initBlock(fullblk, FromWorks_blk, ExtendOutput, argsIn)
   ...
   if isempty(X) || isempty(nSamps),
       ...
       if(strcmp(get_param(bdroot(gcs),'SimulationStatus'),'initializing'))
       		if isempty(X),      error(message('dsp:dspblktsfw2:paramEmptyError1')); end
       		if isempty(nSamps), error(message('dsp:dspblktsfw2:paramEmptyError2'));  end
       end

   else
       if nSamps <= 0,
           error(message('dsp:dspblktsfw2:paramOutOfRange1'));
       end
...
if ndims(X)==3
    ...
    if nSamps ~= 1,
        error(message('dsp:dspblktsfw2:paramOutOfRange2'));
    end
    ...
    case 'Cyclic repetition'
       ...
       if(xRows > nSamps)
        ...
        if( (R == 0) || nSamps == 1)
            ...
            % of framesize
            error(message('dsp:dspblktsfw2:invalidBlkInput'));
