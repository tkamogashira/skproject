...
try
    ...
    if ~isnormalized,
        ...
        if difference(blk_Ts,fda_Ts),
            ...
            fullname(fullname == char(10)) = ' ';
            msg = [fullname ' ' msg];
            warning(message('dsp:slfdatoolinterace:invalidSamplingFrequency', msg));
        end
    end
catch me %#ok<NASGU>
...
% ---------------------------------------------------------------
function filtercheck(hFDA, eventData) %#ok
...
% Check if the designed structure supports being "block"ed.
[isFiltSupported, msg] = issupported(new_filt);
if ~isFiltSupported,
    ...
    if strcmpi(get(hFDA, 'filterMadeBy'), 'designed'),
        ...
        % Throw a FDATool statusbar warning and bring up the convert dialog
        % to convert the filter object to an acceptable structure.
        sendwarning(hFDA, msg);
        ...
        l  = handle.listener(hC, 'DialogApplied', @convert_listener); %#ok
...
% ---------------------------------------------------------------
function filterlistener(hFDA,eventData) %#ok
...
if isappdata(hFDA,'DSPBlks') && ~islockedModel(hFDA),
    ...
    try
        ...
        status = get_param(bdroot(hBlk), 'SimulationStatus');
        switch status
            case {'running', 'pause'}
                if isBiquad(hBlk)
                    lastErrStr = sprintf('%s %s %s', ...
                        'Cannot change the structure, filter order or coefficients', ...
                        'of a second-order sections filter in the Digital Filter', ...
                        'Design block while the simulation is running or paused.');
                else
                    lastErrStr = sprintf('%s %s', ...
                        'Cannot change the structure or filter order in the Digital', ...
                        'Filter Design block while the simulation is running or paused.');
                end
            otherwise
...
%---------------------------------------------------------------
function b = isBiquad(hBlk)

genBlock = [get(hBlk, 'Path') '/' get(hBlk, 'Name') '/Digital Filter'];

b = strcmp(get_param(genBlock, 'ReferenceBlock'), 'dsparch4/Biquad Filter');

%---------------------------------------------------------------
function hidelistener(hFDA, eventData) %#ok
...
if isempty(hFDA),
    ...
    addmenu(hFDA, [7 2], 'Digital Filter Design Block Help', @help_cb, 'dfd_help');
...
%---------------------------------------------------------------
function fs_listener(hFDA, eventData, hBlk) %#ok

storeGUIstate(hBlk, hFDA);

%---------------------------------------------------------------
function help_cb(hcbo, eventData) %#ok

helpview(fullfile(docroot, 'toolbox', 'dspblks', 'dspblks.map'), ...
    'fdatool_overview');
...
hFDA = [];
if isfield(blk_ud,'hFig')
    condtn = isgraphics( blk_ud.hFig,'figure');
    if ~isempty(condtn) && condtn
...
boolflag = (abs(ts1-ts2) > sqrt(eps(ts1)));
...
port_frame  = get_param(hBlk,'CompiledPortFrameData');

if port_frame.Inport
...
%---------------------------------------------------------------
function [boolflag msg] = issupported(Hd)

msg = '';
try
    params = blockparams(Hd, 'off'); %#ok
    [lib, b] = blocklib(Hd); %#ok
    boolflag = true;
catch me
    msg = me.message;
    boolflag = false;
...
%---------------------------------------------------------------
function convert_listener(hC, eventData) %#ok
