...
% DSPBLKRANDSRC2 DSP System Toolbox Random Source Generator block helper function.
 ...
 case 'seed'
  ...
  if isempty(ud),
    ...
    elseif (strcmp(RepMode,'Repeatable'))
      ...
      % Not repeatable: recalculate seed in s-function
      % here we pass a constant 'dummy' seed to s-fcn
      seedVal = 23113; 
      ud.Seed = num2str(seedVal);
      ud.SeedFlag = 'DoNotSaveSeed';
    end
  ...
  if ~isequal(ud,ud_old),
    % Using set_param for same reasons as
    % above
