...
% Detect if block is called in the lmsxydemo or via test code
persistent indemo
...
switch flag
  ...
  case 'Stop'
    ...
    if demo_flag
        % In test case, need to delete the figure block.
        LocalBlockDeleteFcn;
    end
  ...
  otherwise
    if ischar(flag),
      error(message('dsp:lmsxy:unhandledCase1', flag));
    else
      error(message('dsp:lmsxy:unhandledCase2', flag));
    end
...
if length (ax)<4
  error(message('dsp:lmsxy:invalidFigureParameter'))
end
...
%=============================================================================
%
function sys=mdlUpdate(t,x,u,flag,ax,varargin) %#ok<INUSL>
...
% GraphicsBranchingPoint
if matlab.graphics.internal.isGraphicsVersion1
    if isempty(ud.XData)
        for i=1:4
			set(ud.XYLine(i),...
				'Xdata',[u(2*i-1) u(2*i-1)],...
				'YData',[u(2*i) u(2*i)]);
        end
    else
        for i=1:4
			set(ud.XYLine(i),...
				'Xdata',[ud.XData(i,s2) u(2*i-1)],...
				'YData',[ud.YData(i,s2) u(2*i)]);
        end
    end
else
    for i=1:4
        addpoints(ud.XYLine(i), u(2*i-1), u(2*i));
    end
end
...
if matlab.graphics.internal.isGraphicsVersion1
    ...
    clearpoints(ud.XYLineb);
    for i=1:4
        clearpoints(ud.XYLine(i));
    end
end
...
ud.XData = [];
ud.YData = [];
set(ud.Text(1), 'Position', [0, -0.08]); %lms
set(ud.Text(2), 'Position', [0, 1.03]);  %nlms
set(ud.Text(3), 'Position', [0.95, -0.08]);  %selms
set(ud.Text(4), 'Position', [0.95, 1.03]);   %sslms
set(ud.XYAxes,'Xlim',ax(1:2),'Ylim',ax(1:2)); % Set temporary axes limits
...
% This function call is not needed in Graphics Version 2
...
if ishandle(FigHandle),
  ...
  for i=1:4
      ...
      %           'Ydata',ud.YData(i,:),...
      %           'MArker','.', 'LineStyle','none');
      if ~isempty(ud.XData) && ~isempty(ud.YData)
          if matlab.graphics.internal.isGraphicsVersion1
              set(ud.XYLine(i),...
                  'Xdata',ud.XData(i,:),...
                  'YData',ud.YData(i,:),...
                  'Color','r','LineStyle','-','LineWidth',2);
              % This is not needed if using HG2 because the points in the
              % lines are not erased using EraseMode.
          end
      end
  end
end
...
if matlab.graphics.internal.isGraphicsVersion1
    ...
    ud.XYLineb = animatedline('Marker','.','LineStyle','none');
	hold on;
    for i=1:4
        ud.XYLine(i) = animatedline('Color','r',...
...
set(ud.XYTitle, 'String', 'X Y Plot');          % Axes title
set(ud.XYAxes, 'Box', 'on', 'Visible', 'off');
...
% LocalWords:  lmsxydemo varargs ud Xdata YData Erasemode XData brt xd selms
% LocalWords:  sslms Ydata MArker block'Delete Sfun subsystem's XYGRAPH
