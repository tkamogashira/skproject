...
LIBMW_SRC_API void MWDSP_RandSrc_GZ_C(creal32_T *outPtr,     /* output signal */
                ...
                if (s > c2) {
                    ...
                    if (y > c-aa*expf(-0.5F*x*x)) {
                        ...
                        do {
                            ...
                            jsr ^= (jsr<<13); jsr ^= (jsr>>17); jsr ^= (jsr<<5);
                            i = icng+jsr;
                            logf_value = logf(0.5F+i*tpm32);
                        } while (-2.0F*logf_value <= x*x);
                        val = (r < 0 ? x-xn : xn-x);
                    }
                }
            }
