/*
 * SVD_C_RT - DSP System Toolbox Singular Value Decomposition helper functions
 *
 ...
 */
LIBMW_SRC_API int_T MWDSP_SVD_C
(
                   creal32_T	*x,
                   int_T	n,
                   int_T	p,
    ...
    */
    ...
    for (l=0; l<lu; l++) {
        ...
        */
        ...
        snorm = 0;
        for (l=0; l<m; l++) {
            snorm = MAX(snorm, MAX(fabsf((s+l)->re), fabsf((e+l)->re)));
        }
        ...
        *	kase = 4     if er(m-1) is negligible (convergence).
            */
            ...
            mm2 = m - 2;
            for (l=mm2; l>=0; l--) {
                test = fabsf((s+l)->re) + fabsf((s+l+1)->re);
                ztest = fabsf((e+l)->re);
            ...
            if (l == mm2) {
                ...
                for (ls=m; ls>lp1; ls--) {
                    test = 0.0F;
                    if (ls != m) test += fabsf((e+ls-1)->re);
                    if (ls != l + 2) test += fabsf((e+ls-2)->re);
                    ztest = fabsf((s+ls-1)->re);
                    if (!svd_IsFinite32(test) || !svd_IsFinite32(ztest)) {
                        return(info);
                    }
                    if ((ztest <= eps*test) || (ztest <= tiny)) {
            ...
            */
            ...
            case 3:				/* perform one qr step. */
                                                ...
                                                *	Calculate the shift.
                */
                scale = MAX(fabsf((s+mm1)->re), MAX(fabsf((s+mm2)->re), fabsf((e+mm2)->re)));
                scale = MAX(fabsf(scale), MAX(fabsf((s+l)->re), fabsf((e+l)->re)));
                sm = (s+mm1)->re / scale;
                smm1 = (s+mm2)->re / scale;
                emm1 = (e+mm2)->re / scale;
                sl = (s+l)->re / scale;
