...
%  ENH:
%    Record "raw" input data, transform it afterward
...
elseif (nargin == 2) % subsystem error function call back
   varargout{1} =  mdlError(varargin{1},varargin{2}); 
else
...
blkh = get_param(get_param(block.BlockHandle,'Parent'),'Handle');
...
% Make sure that the figure's userdata contains up-to-date copy of block
% handle
if (isfield(bd,'hfig') && ~isempty(bd.hfig)) % if figure exists make sure it contains correct block handle
...
blkh = get_param(block.BlockHandle,'UserData');  % handle to parent cached in S-Fcn block
...
% Read input and convert it to double if necessary.
if (block.InputPort(1).DatatypeID < 2) %% double or single
    u = block.InputPort(1).Data;
else  %fixed-point
    u = block.InputPort(1).DataAsDouble;
end
...
if dispEntries < M,
    ...
    % least some of the old display will remain visible
...
% We do not need a 'drawnow' when play button is hit but we need it for
% command-line sim to update the plot for each new frame: see sdspfscope2
% line 285
drawnow('update');
...
% Get around MATLAB file S-function limitation
% Data comes in two contiguous sections,  [real imag]
...
for i=1:length(fields),
    s.subs=fields{i};
    params1 = subsasgn(params1,s,subsref(params2,s));
end
...
if length(hsurf) ~= current_NumTraces,
    error(message('dsp:sdspwfall:invalidFigureParameter'));
end
...
%--------------------------------------------------------------------------
function update_zticklabels(haxis)
% Update z-axis tick labels
% 
...
%
% This function is called by the functions update_num_traces, ResizeFcn,
% and CameraModeMotion.
%

posTicks = get(haxis,'ztick');
numTicks = length(posTicks);
negTicks = cell(1,numTicks);
for i=1:numTicks,
    negTicks{i} = num2str(1-posTicks(i));
end
set(haxis,'zticklabel',negTicks);
...
% Formulate "data sizes" text readout
%   "N:# U:# H:#"
% where:
...
% Handle key presses in main scope window
...
% Update z-axis tick labels
haxes = get(fd.hfig,'CurrentAxes');
update_zticklabels(haxes);
...
if graphicsversion(newfig, 'handlegraphics')
    p = feval(printFcn, newfig);
    if ~isempty(p), uiwait(p); end
else
    feval(printFcn, newfig);
end 
...
%
% Also, don't use drawmode=fast or sortmethod=childorder, because it
% specifically turns off sorting in 3-D transparency mode, and the curves
% will render incorrectly from some view angles
...
iotype = get_param(blk,'iotype');
if strcmp(iotype,'viewer')
  fig_name = viewertitle(blk,false);
else
  fig_name = getfullname(blk);
end
...
delete(findall(hfig,'type','uimenu','tag','figMenuInsert'));
delete(findall(hfig,'type','uimenu','tag','figMenuFile'));
delete(findall(hfig,'type','uimenu','tag','figMenuEdit'));
delete(findall(hfig,'type','uimenu','tag','figMenuTools'));
delete(findall(hfig,'type','uimenu','tag','figMenuHelp'));
...
set(0,'showhid','on');
mView = findobj(hfig,'tag','figMenuView');
set(0,'showhid','off');
...
%
%xxx: work around for hg2 issue - start
% Issue: getting figure position fires call to ResizeFcn of the figure -
% this is not the case in hg1. 
% ResizeFcn uses stuff that is still not defined in create_scope fcn,
% resulting into hard errors.
% 
% Work-around: don't get figure's position. Use params.FigPos which is used
% to set figure position when figure is created (see above)
% RahulM's response: The figure position/resizefcn behavior is being
% reworked for HG2, so I will look for a workaround for the time being.
%
%fig_pos = get(hfig,'Position'); % original code
fig_pos = params.FigPos; % work around for hg2 issue
%xxx: work around for hg2 issue - end
...
% Resize 3-D display axes - leave space (margin) on all four sides for
% status bar, history slider control, and axes labels at different camera
% views.
set(hax,'Position',[0.05 0.1 0.8 0.8]);      % In Normalized units
...
if ~all(size(pu)==[1 13]),
    error(message('dsp:sdspwfall:unhandledCase'));
end
cam_stuff = pu;
...
if hfig ~= fig_data.hfig,
   error(message('dsp:sdspwfall:invalidHandle'));
end
...
if ~isempty(bd)
  iotype = get_param(blk,'iotype');
  if strcmp(iotype,'viewer')
    full_name = viewertitle(blk,false);
  else
...
if bd.params.OpenScopeAtSimStart,
    ...
    % Defer the OpenScope by registering as a "PreShow" callback
    % Ensures the scope does not open when the model is not visible
    bdobj = get_param(bdroot,'Object');
    id = genvarname(getfullname(blkh));
    if ~bdobj.hasCallback('PreShow', id)
        % Only add this PreShow CallBack if not already registered
        bdobj.addCallback('PreShow',id,@() OpenScope(blkh));        
    end
...
switch bd.params.ExportMode
    ...
    otherwise
        error(message('dsp:sdspwfall:unhandledCase1', bd.params.ExportMode));
end
...
%      data is coming in, and offset 0 is now "traveling with the new
...
% Update visibility of history review scrollbar,
...
y = 6.02059991327962 .* log2(abs(u)+eps(u));
...
% 20*log10(x) = 20*log2(x)/log2(10) = 6.02*log2(x)
uSubset = u(1:floor(length(u)/2+1));
y = abs(uSubset);
...
% 20*log10(x) = 20*log2(x)/log2(10) = 6.02*log2(x)
uSubset = u(1:floor(length(u)/2+1));
y = 6.02059991327962 .* log2(abs(uSubset)+eps(uSubset));
...
y = 3.01029995663981 .* log2(abs(u)+eps(u));
...
% LocalWords:  un userdata hfig haxis Xform axisgrid checkable hsurf Rahul
% LocalWords:  hdiscontig discontiguous indims eg Cnt Discontig Vis dlg CMap
% LocalWords:  TNewest TOldest displayfull YMin YMax XLabel YLabel ZLabel Svc
% LocalWords:  ENH xform inportdimshint Ns fastdraw xformed blkh svc traceback
% LocalWords:  whitespace linefeeds wfall xdata sdspfscope discontig attribs Nx
% LocalWords:  vert screenful blockdiagramtype nexst CData Dlgs prestore eval'd
% LocalWords:  colorspec foregr nd chans vis ydata zdata edgealpha edgecolor
% LocalWords:  ztick zticklabel horiz syncupdate hopt cameratools warptofill
% LocalWords:  hcb GL drawmode zbuffer iotype numbertitle toggletoolbar
% LocalWords:  ticklength submenu eters showhid Suspen dspblks dsp
% LocalWords:  clickedcallback bgc dx backgr sliderstep fontweight fontsize
% LocalWords:  windowbutton currentpoint tgt dar cameraposition cameratarget
% LocalWords:  cameraupvector cameraviewangle dataaspectratio dataaspect
% LocalWords:  blockname dialogapply showhiddenhandles sigbrowser ith varname
% LocalWords:  hax de hpatch linewidth windowbuttondownfcn
% LocalWords:  windowbuttonmotionfcn windowbuttonupfcn cp zcoord buttondown
% LocalWords:  windowbuttondown pc portconnectivity stationarity ve smax
% LocalWords:  scrollbar unpause setmenuchecks Fs
