...
if isempty(L) | isempty(M)
	d='x[ M * n / L]';
	warning(message('dsp:dsprprim:undefinedInput', blknam));
	return
end
...
if (L<1) | (M<1),
  msg = ['Conversion factors must be integers ' ...
	...
	warning(message('dsp:dsprprim:invalidConversionFactor1', blknam));
...
if (g > 1),
  warning(message('dsp:dsprprim:invalidConversionFactor2', blknam, L, M, Li, Mi));
end
