...
%      
%      If the DSP System Toolbox is installed, SQDTOOL realizes 
...
ud=get(hObject,'UserData');
  ud.version = 2.0;% for backward compatibility
 ...
 if (errorStatus)
     ...
     return
      ...
      try 
          ...
          errordlg([me.message ' Consider reducing the Number of levels.']);
          localStartEndProcessing (handles,'on','Ready', 'Arrow');
          return;
      end
...
if isempty(errStr),
    ...
    elseif (isempty(val) || length(val)>1  || val < 2 || val >= double(intmax('int32')) || (val-floor(val)~=0)),
        if (isempty(eventdata)), errordlg('The NUMBER OF LEVELS must be a positive scalar integer (greater than 1 and less than intmax(''int32'')).'); end
        ud.errFlag_editNumLevel = 1;
    else
        ud.NumLevelchanged = 0;
