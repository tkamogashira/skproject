function [new_UD, OtherTypeBlock] = sqcreatemodel(UD)
...
if (UD.whichBlock==1)%encoder
   SQLibBlkName=sprintf('Scalar Quantizer\nEncoder');
   SQSfcnName = 'sdspsqenc';
   posSQ = [25    18   120    72];
else %decoder   
   SQLibBlkName=sprintf('Scalar Quantizer\nDecoder') ;
   SQSfcnName = 'sdspsqdec';
   posSQ = [200   18   295    72];
end   
srcBlockWithPathAndName = strcat('dspquant2/',SQLibBlkName);
pos = posSQ;

SQblkNameInEditBox = UD.blockName;
if isempty(SQblkNameInEditBox),
    SQblkNameInEditBox = SQLibBlkName;
end

newTag = 'SQDESIGN_GUI_REALIZE_MODEL';
if ((UD.destinationMdl == 1) && (UD.overwriteFlag == 1)), % current model & overwrite
    subsys = [sys '/' SQblkNameInEditBox];
    currentblk = find_system(gcs, 'SearchDepth', 1,'Name', SQblkNameInEditBox);
    ...
    if ~isempty(currentblk),
        ...
        try
          CurBlockIsSQblock = strcmp(get_param(currentblk{1},'FunctionName'),SQSfcnName); % for dspblks
                                          % simulink block doesn't have parameter 'FunctionName'
        catch
            % if the block doesn't have a parameter named 'FunctionName', it is not the SQ CODEC block
          CurBlockIsSQblock = 0;
        end                                 
        pos = get_param(currentblk{1}, 'Position');
        if ~CurBlockIsSQblock 
            % Get a new SQ ENC/DEC block only if it is not SQ ENC/DEC block
...
if  ((UD.destinationMdl == 1) && (UD.overwriteFlag == 0)),  % current model & non-overwrite
    old_subsys = [sys '/' SQblkNameInEditBox];
