...
ud=get(hObject,'UserData');
  ud.version = 1.0;% for backward compatibility
  ...
  ud.weight=[1];
  ud.RelTh  =1e-7;
  ud.MaxIter=2147483647; %double(int32(inf));
  ...
  ud.weightForCurrentFig  = ud.weight;
  ud.NeedWeightForCurrentFig = (ud.popupDistMeasure~=1);
...
if strcmp(get(hObject,'Visible'),'off')    
   ...
   ylabel('Entropy');
   set(h_line,'ButtonDownFcn',@setdatamarkers);% always call after setting axes, xlabel and ylabel, and plot command
   set(handles.pushbutnRealzMdl,'string','Generate Block');
   set(handles.pushbutnExport,'string','Export Outputs...');
   grid on;
...
% check if any of FINAL edit box entry is invalid (including workspace changes)
 ...
 if (errorStatus)
     ...
     hMainFig=handles.vqdtool;
       ...
       ud2.weightForCurrentFig = ud.weight;
       ud2.NeedWeightForCurrentFig = NeedWeight;
...
% handles    structure with handles and user data (see GUIDATA)
  ...
  % Recompute the computed VQ output parameters if any of the dialog 
  % widgets have been changed since we last generated the parameters.
  if vqDataChanged(handles)
      pushbutnPlotFig_Callback([], [], handles);
  end
  ...
  UD.weightForCurrentFig = ud.weightForCurrentFig;
  UD.NeedWeightForCurrentFig = ud.NeedWeightForCurrentFig;
...
if (val==1)%% auto-generate initCB
    ...
    set(handles.pushbutnRealzMdl,'string','Generate Model');
end   
hfig=handles.vqdtool;
vqmake_fig_dirty(hfig);
ud=get(hfig,'UserData');
