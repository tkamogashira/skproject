...
% block.DialogPrm(4):  AxisColorbar: checkbox
% block.DialogPrm(5):  AxisOrigin: Lower left cornerUpper left corner
% block.DialogPrm(6):  XLabel: Cell array. x-axis label for main image
% block.DialogPrm(7):  YLabel: Cell array. y-axis label for main image
% block.DialogPrm(8):  ZLabel: Cell array. color bar scaling label
% block.DialogPrm(9):  FigPos:       figure position
% block.DialogPrm(10): AxisZoom:     checkbox
...
% If in Graphics Version 1, set erase mode of data channel lines:
...
if isfield(block_data, 'hfig') && ~isempty(block_data.hfig)
  ...
  if ~isequal(curpos, block.DialogPrm(9).Data)
    set(block_data.hfig,'Position', block.DialogPrm(9).Data); % FigPos
  end
...
% Since EraseMode='none', we don't need a drawnow in Graphics Version 1
if ~matlab.graphics.internal.isGraphicsVersion1
    drawnow('update');
end
...
%
axisZoom = block.DialogPrm(10).Data;
if axisZoom
    azoom_chkd = 'on';
    cBarVis='off';
else
...
%
showColorBar = block.DialogPrm(4).Data;
if showColorBar
    cBarCheck = 'on';
else
    cBarCheck = 'off';
...
% block.DialogPrm(4):  AxisColorbar: checkbox
% block.DialogPrm(5):  AxisOrigin: Lower left cornerUpper left corner
% block.DialogPrm(6):  XLabel: x-axis label for main image
% block.DialogPrm(7):  YLabel: y-axis label for main image
% block.DialogPrm(8):  ZLabel: color bar scaling label
% block.DialogPrm(9):  FigPos:       figure position
% block.DialogPrm(10): AxisZoom:     checkbox
...
% Check XLabel:
% -------------
if ~ischar(block.DialogPrm(6).Data{1}),
...
% Check YLabel:
% -------------
if ~ischar(block.DialogPrm(7).Data{1}),
...
% Check ZLabel:
% -------------
if ~ischar(block.DialogPrm(8).Data{1}),
...
xLabel = block.DialogPrm(6).Data{1};
...
% Setup Y-axis label:
% -------------------
yLabel = block.DialogPrm(7).Data{1};
...
% ---------------------
cLabel = block.DialogPrm(8).Data{1};
...
% ----------------------
useColorbar = block.DialogPrm(4).Data;
axiszoom = block.DialogPrm(10).Data;
...
hfig = figure('numbertitle', 'off', ...
   ...
   'menubar','none', ...
   'toolbar','none', ...
   'position',     block.DialogPrm(9).Data, ...
   'nextplot',     'add', ...
   'integerhandle','off', ...
   'doublebuffer', 'off', ...
   'DeleteFcn',    @FigDelete, ...
...
% Set 'Renderer' property of 'hfig' to 'painters' if MATLAB Graphics
% Version 1 is used
if matlab.graphics.internal.isGraphicsVersion1
...
if nargin == 2
    ...
    if matlab.graphics.internal.isGraphicsVersion1
        % In Graphics Version 1, setting the EraseMode to none can speed up
        % updates
...
% EraseMode is only set when using Graphics Version 1
if matlab.graphics.internal.isGraphicsVersion1
