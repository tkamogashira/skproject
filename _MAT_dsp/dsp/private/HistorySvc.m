...
%         end
    ...
    case 'initialize'
        ...
        if ~isempty(db),
            ...
            if length(idx)>1,
                error(message('dsp:Waterfall:BadHistoryIndex'));
            end
        end
        if isempty(idx),
            idx = getNewIndex(db);
    ...
    case 'replace'
        ...
        if ~iscell(data),
            error(message('dsp:HistorySvc:invalidFcnInput'));
        end
    ...
    case 'delete'
        ...
        % when all the structs are empty structs,
        %  we delete all entries
        if ~isempty(db)        
            dbtmp = [db.Data];
            if  ~iscell(dbtmp)
                % The case where the history contains only 
                % one entry
                % dbtmp is not a cell array object
                if isempty (dbtmp)
                   db = struct([]);
                end
            else
                % The usual case where the history contains
                % valid data from the most recent simulation
                % dbtmp is a cell array object
                if  isempty([dbtmp{:}])
                    db = struct([]);
                end
            end
    ...
    otherwise
        error(message('dsp:HistorySvc:unhandledCase'));
end
