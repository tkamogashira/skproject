...
% Dialog figure handle, and dialog position
%
...
% that no change in tab occurred, and thus doesn't kick off the
...
% ---------------------------------------------------------------
function CloseDialog(hco, eventStruct, blk, parentClose) %#ok
...
% ---------------------------------------------------------------
function DialogDelete(hcb, eventStruct) %#ok
...
if isempty(fig_data) || (hfig ~= fig_data.hfig),
   error(message('dsp:sdspwfall_params:invalidHandle'));
end
...
% -------------------------------------------------------------------------
function DialogKeypress(hcb, eventStruct) %#ok
% Handle key presses in dialog window
...
% -------------------------------------------------------------------------
function refresh_dialog_from_blk(blk,varargin) %#ok
...
dlg_vals = cell(length(dlg_names),1);
for i=1:length(dlg_names),
    param_name = dlg_names{i};
    hDlg = hDlgs.(param_name);
    ...
    switch get(hDlg,'style')
        ...
        otherwise
            error(message('dsp:sdspwfall_params:unhandledCase1'));
    end
    dlg_vals{i} = x;
end
...
% -------------------------------------------------------------------------
function s = getDialogParamsAsStruct(hfig) %#ok
...
% -------------------------------------------------------------------------
function update_dialog(hcb,eventStruct,hfig) %#ok
...
% -------------------------------------------------------------------------
function refresh_dialog_xform(hfigDlg, eventStruct, tabNum) %#ok
...
% -------------------------------------------------------------------------
function refresh_dialog_trigger(hfigDlg, eventStruct, tabNum) %#ok
...
switch bd.params.TrigStartMode
    ...
    otherwise, error(message('dsp:sdspwfall_params:unhandledCase2'));
end
...
for i=1:length(dlg_names),
    ...
    switch get(hDlg,'style')
        case 'edit'
            if ~ischar(param_val)
                param_val = mat2str(param_val);
            end
            set(hDlg, 'string', param_val);
        case 'popupmenu'
            all_strs = cellstr(get(hDlg,'string'));
            idx=strmatch(param_val,all_strs);
            if length(idx)~=1,
                idx=1;  % error('dsp:sdspwfall_params:unhandledCase','Invalid enum selected in dialog');
            end
            set(hDlg, 'value', idx);
        case 'checkbox'
...
%
% These are not on the "usual" list since the corresponding edit
...
hStatic{end+1} = uicontrol(ctxt{:}, ...
    'string','Newest:', ...
    'tooltip',tip, ...
    'pos', [33 53-5 110 dy]);  % dx=90 would be aligned with above prompts
...
hStatic{end+1} = uicontrol(ctxt{:}, ...
    ...
    'tooltip',tip, ...
    'pos', [25+8 150+8-24 130 dy]);  % 150+8
tip=[...
...
hDlgs.HistoryFull=uicontrol('parent', hFig, ...
    ...
    'pos', [25+8+130+3 152+8-24 95 dye]);
...
% -------------------------------------------------------------------------
function handles = CreateTriggerTab(hFig, textExtent) %#ok
...
tip = ['Immediately: no trigger delay' CR ...;
        ...
        'User-defined: MATLAB file function name'];
...
hPrompts.XformFcn = uicontrol(ctxt{:}, ...
    'string','Function:', ...
    'tooltip',tip, ...
    'pos', [33 160-24 120 dy]);
hDlgs.XformFcn = uicontrol(cedt{:}, ...
    'string','(XformFcn)', ...
    'pos', [33+120+3 160-24 120 dye]);
...
hPrompts.XformExpr = uicontrol(ctxt{:}, ...
    'string','Expression:', ...
    'tooltip',tip, ...
    'pos', [33 160-48 120 dy]);
hDlgs.XformExpr = uicontrol(cedt{:}, ...
    'string','(XformExpr)', ...
    'pos', [33+120+3 160-48 120 dye]);
...
TOld = str2num(get(fig_data.hDlgs.TOldest,'string')); %#ok
set(fig_data.transp.Slider_TOldest, 'value', TOld);

TNew = str2num(get(fig_data.hDlgs.TNewest,'string')); %#ok
...
% LocalWords:  hfig Dlgs YMin YMax CMap XLabel YLabel ZLabel TNewest TOldest
% LocalWords:  dlg vis eg userdata Keypress keypress popupmenu ena dsp backgr
% LocalWords:  horiz dx sliderstep dy bgc nonrepetitive Fs Xform Fontname
% LocalWords:  Fontsize nextplot tabcallbk tabpress htext alloc
