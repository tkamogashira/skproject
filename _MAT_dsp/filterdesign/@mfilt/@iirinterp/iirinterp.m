function this = iirinterp(varargin)
%IIRINTERP   Construct an IIR interpolator filter object.
%   Hm = MFILT.IIRINTERP(C1,C2,...) constructs an IIR interpolator filter
...
%   phase IIR interpolators.
%
%   Note that one usually does not construct IIR interpolators explicitly as
%   discussed so far. Instead, one obtains these filters as a result from a
%   design of a halfband interpolator. An example of this is shown below.
%
...
%   % Now perform actual design. Hm results in an mfilt.iirinterp
%   Hm = design(f,'ellip','FilterStructure','iirinterp'); 
%   realizemdl(Hm) % Requires Simulink; build a Simulink model for the filter
%
...
%   f = fdesign.interpolator(L,'halfband','TW,Ast',TW,Ast,Fs); % Store interpolator design specs
%   % Now perform actual design. Hm results in an mfilt.iirinterp
%   Hm = design(f,'iirlinphase','FilterStructure','iirinterp'); 
%   realizemdl(Hm) % Requires Simulink; build a Simulink model for the filter
%
...
this = mfilt.iirinterp;

this.FilterStructure = 'IIR Polyphase Interpolator';
