...
%   Copyright 2004 The MathWorks, Inc.
  ...
  switch lower(filterobj.arithmetic)
   ...
   otherwise
    result = false;
    errorObj = message('dsp:mfilt:firdecim:ishdlable:HdlNotSupportedArith',filterobj.arithmetic);    
    errstr = getString(errorObj);
  end
  firlen = length(filterobj.numerator);
  if firlen == 1                        % zero order case
    result = false;
    errorObj = message('dsp:mfilt:firdecim:ishdlable:HdlNotSupportedZeroOrder');    
    errstr = getString(errorObj);
  end
% [EOF]
