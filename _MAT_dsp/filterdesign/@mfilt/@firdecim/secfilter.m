...
if ~isreal(p) && Hm.PersistentMemory && ~strcmp(Hm.Arithmetic,'fixed')
    error(message('dsp:mfilt:firdecim:secfilter:UnsupportedPersistentMemory'));
end
...
if ~(isempty(polyacc) || any(size(polyacc,2) == [nchan,1])),
	error(message('dsp:mfilt:firdecim:secfilter:InvalidDimensions'));
end
...
[y, zf, polyacc, phaseidx, tapidx] = firdecimfilter(Hm.filterquantizer, ...
    int32(M),p,x,zi,polyacc,int32(phaseidx),int32(Hm.TapIndex),nx,nchans,ny);
