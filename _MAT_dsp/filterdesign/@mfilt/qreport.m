...
%
%   % EXAMPLE #1: Quantization report of a FIR decimator filter.
%   fipref('LoggingMode', 'on');
%   Hd = design(fdesign.decimator, 'equiripple');
%   Hd. arithmetic = 'fixed';
%   rng default; % for reproducible results from rand
%   y = filter(Hd, rand(100,1));
%   R = qreport(Hd)
%
%   % EXAMPLE #2: Quantization report of a CIC interpolator filter.
%   fipref('LoggingMode', 'on');
%   Hd = design(fdesign.interpolator(5,'cic'));
%   Hd. arithmetic = 'fixed';
%   rng default; % for reproducible results from rand
%   y = filter(Hd, rand(100,1));
%   R = qreport(Hd)
