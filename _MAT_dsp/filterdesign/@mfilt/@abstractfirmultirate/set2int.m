...
if ~isfield(get(this),'Arithmetic'),
    error(message('dsp:mfilt:abstractfirmultirate:set2int:FixedPointFcn1'));
end

if ~strcmpi(this.Arithmetic, 'fixed'),
    error(message('dsp:mfilt:abstractfirmultirate:set2int:FixedPointFcn2'));
end

if ~strcmpi(this.FilterInternals,'FullPrecision'),
    error(message('dsp:mfilt:abstractfirmultirate:set2int:FixedPointFcn3'));
end
