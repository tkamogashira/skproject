...
%ALLPASSLP2LP Allpass for lowpass to lowpass frequency transformation.
%   [AllpassNum,AllpassDen] = ALLPASSLP2LP(Wo,Wt) returns the numerator,
...
%
%   Example:
%        % Allpass mapper changing the lowpass cutoff frequency originally
%        % at Wo=0.5 to Wt=0.25
%        Wo = 0.5;
%        Wt = 0.25;
...
alpha      = sin(pi*(wo-wt)/2) / sin(pi*(wo+wt)/2);
allpassden = [1     -alpha];
allpassnum = [-alpha 1];
