...
if ~strcmpi(flag, 'dfilt')
    error(message('dsp:coeread:invalidInput'));
end
...
if isempty(findstr(filename,'.'))
    ...
    if ~strcmpi(ext,'.coe'),
        error(message('dsp:coeread:FixedPtErr1'));
    end
end
...
if isempty(radix),
    error(message('dsp:coeread:FixedPtErr2')); 
end
...
specifyall(Hq);
set(Hq, 'CoeffWordLength', q.Format(1), ...
...
try
    ...
    error(message('dsp:coeread:FixedPtErr3', c));
end

if max(x)>=2^(maxwl-1)
    wordlen = maxwl;
else
    error(message('dsp:coeread:FixedPtErr4'));
end 
...
% and ends with the first occurrence of a semicolon.  If no keyword was 
...
% Build a cell array containing keyword information
