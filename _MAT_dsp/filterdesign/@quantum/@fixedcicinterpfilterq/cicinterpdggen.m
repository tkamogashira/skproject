function DGDF = cicinterpdggen(q,Hd,states)
% CICDECIMDGGEN Directed Graph generator for cic interpolator filter
...
samplefactor = Hd.InterpolationFactor;
...
Stg(2) = combstage(sectnum,diffdelay,H,info);
Stg(3) = sampstage(samplefactor,H,info);
Stg(4) = integstage(sectnum,H,info);
...
for stage = 1:sectnum
    ...
    castqparam(stage).outQ = [WLPS(stage) FLPS(stage)];
    castqparam(stage).RoundMode = H.RoundMode;
    castqparam(stage).OverflowMode = H.OverflowMode;
end
...
NL.setnode(filtgraph.node('upsample'),1);

% specify the block label

set(NL.nodes(1).block,'label','Up Sample');
