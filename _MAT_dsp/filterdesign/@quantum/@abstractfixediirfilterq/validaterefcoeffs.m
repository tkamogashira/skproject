...
if ~(strcmpi(class(val), 'double') || strcmpi(class(val), 'embedded.fi') ...
    ...
    error(message('dsp:quantum:abstractfixediirfilterq:validaterefcoeffs:invalidDataType1', prop));
end

if issparse(val),
    error(message('dsp:quantum:abstractfixediirfilterq:validaterefcoeffs:invalidDataType2', prop));
end
...
if strcmpi(prop, 'Denominator'),
    if (strcmpi(class(val), 'double') || ...
        ...
        error(message('dsp:quantum:abstractfixediirfilterq:validaterefcoeffs:invalidA0'));
    elseif strcmpi(class(val), 'embedded.fi') && val(1,1)<1-eps(val),
        error(message('dsp:quantum:abstractfixediirfilterq:validaterefcoeffs:invalidA0Values', num2str(double(1-eps(val)))));
    end
end
