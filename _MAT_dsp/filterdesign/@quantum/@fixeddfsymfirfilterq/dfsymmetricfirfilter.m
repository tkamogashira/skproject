function [y,z,tapIndex] = dfsymmetricfirfilter(q,b,x,z,tapIndex)
...
if isloggingon(q),
    ...
    %----------------------------------------------------------------------
    [prodlog, outlog, acclog, tapsumlog] = getsymlog(q,b,y,acc,tapsum,x,inlog);
    %----------------------------------------------------------------------
    % Build Report
    %----------------------------------------------------------------------
    if strcmpi(f.DataTypeOverride , 'forceoff'),
        qlog = quantum.dfsymfirreport('Fixed', ...
    ...
    elseif strcmpi(f.DataTypeOverride , 'ScaledDoubles')
        qlog = quantum.dfsymfirreport('Double', ...
