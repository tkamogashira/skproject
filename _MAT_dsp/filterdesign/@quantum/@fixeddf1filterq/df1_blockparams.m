function p = df1_blockparams(this,forceDigitalFilterBlock)
%DF1_BLOCKPARAMS   Return the DF1 specific block parameters.
...
if ~forceDigitalFilterBlock
  ...
  if s.NumAccumFracLength ~= s.DenAccumFracLength
    warning(message('dsp:quantum:fixeddf1filterq:df1_blockparams:cannotMap1'));
  end
  
  if s.NumProdFracLength ~= s.DenProdFracLength
    warning(message('dsp:quantum:fixeddf1filterq:df1_blockparams:cannotMap2'));
  end
