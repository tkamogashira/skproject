...
ftransfargchk(wt,'Desired frequency location in the target filter', ...
    'vector', 'real', 'full normalized', 'even');
ftransfargchk([wo(:).';wt(:).'], 'Prototype and target frequencies', 'overlap');
...
if Cost>1e-5,
   warning(message('dsp:allpasslp2xc:InaccurateResult', num2str( Cost )));
end;

allpassnum = fliplr(conj(allpassnum));
allpassden = fliplr(conj(allpassden));
