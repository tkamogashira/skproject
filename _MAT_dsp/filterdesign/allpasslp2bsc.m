function [allpassnum, allpassden] = allpasslp2bsc(wo, wt)
%ALLPASSLP2BSC Allpass for lowpass to complex bandstop frequency transformation.
%   [AllpassNum,AllpassDen] = ALLPASSLP2BSC(Wo,Wt) returns numerator,
...
%        % at Wo=0.5 to the complex bandstop with bandedges at Wt1=0.2 nad Wt2=0.4
%        Wo = 0.5;
%        Wt = [0.2,0.4];
...
%
%   See also IIRLP2BSC and ZPKLP2BSC.
...
wc         = sum(wt) / 2;
bw         = max(wt) - min(wt);
alpha      = cos(pi*(wo+bw/2)/2) / cos(pi*(wo-bw/2)/2);
beta       = exp(-pi*i*wc);
allpassnum = fliplr(conj([      -beta +alpha]));
allpassden = fliplr(conj([-alpha*beta 1]));
