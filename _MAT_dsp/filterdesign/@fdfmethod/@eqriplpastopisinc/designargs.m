...
switch this.StopbandShape,
    case 'flat',
        if this.StopbandDecay~=0,
            warning(message('dsp:fdfmethod:eqriplpastopisinc:designargs:InvalidSpecifications'));
        end
        decay = 0;
    case 'linear',
...
if freqFactor > 1/hspecs.Fpass
   warning(message('dsp:fdfmethod:eqriplpastopisinc:designargs:InvalidFreqFactor'));
end
