function h = pbufdaf(varargin)
%PBUFDAF  Partitioned block unconstrained frequency-domain FIR adaptive
%         filter with bin Step size normalization.
%   H = ADAPTFILT.PBUFDAF(L,STEP,LEAKAGE,DELTA,LAMBDA,BLOCKLEN,OFFSET,
%   COEFFS,STATES) constructs a partitioned block unconstrained
%   frequency-domain FIR adaptive filter H with bin Step size normalization.
%
...
%   parameter is set to a value between zero and one, a leaky PBUFDAF
...
%      N   = 8;                             % Block size
%      h = adaptfilt.pbufdaf(32,mu,1,del,lam,N);
%      [y,e] = filter(h,x,d); 
...
h = adaptfilt.pbufdaf;
construct(h,[0,9],'Partitioned Block Unconstrained Frequency-Domain FIR Adaptive Filter',varargin{:});
