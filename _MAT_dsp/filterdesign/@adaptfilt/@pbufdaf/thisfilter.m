...
%THISFILTER  Execute the partitioned block unconstrained
%        frequency-domain FIR adaptive filter.
%   
%   Y = THISFILTER(H,X,D) applies an PBUFDAF adaptive filter H to the input
...
%      h = adaptfilt.pbufdaf(32,mu,1,del,lam);
%      [y,e] = filter(h,x,d); 
...
if (rem(length(x),h.BlockLength)~=0),
    error(message('dsp:adaptfilt:pbufdaf:thisfilter:InvalidBlockLength'));
end
...
for n=1:ntrB,
    ...
    FFTW = lam*FFTW + FFTEinvnormFFTX.*conj(FFTX);  
                            %  update frequency domain coefficient matrix
end
