...
%
%   EXAMPLE: 
%       % Simulation of a 32-coefficient FIR filter 
...
%       x = filter(sqrt(0.75),[1 -0.5],sign(randn(s1,size(x)))); 
%       n = 0.1*randn(s2,size(x));             % observation noise signal
...
error(message('dsp:adaptfilt:msesim:FilterErr'))
