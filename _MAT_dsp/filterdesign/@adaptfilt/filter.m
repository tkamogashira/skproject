...
%
%   If H.PersistentMemory is false (default), the properties of H that
...
%
%   To filter data in streaming mode (in a loop), set H.PersistentMemory
%   to true so the history of the filter is conserved when invoking FILTER.
%
%    
%   EXAMPLE: (Adaptive linear prediction)
%     % Reset filter before reuse
%     s1= RandStream('mrg32k3a');
%     x = randn(s1,1000,1);        % Input
...
%     h.PersistentMemory = true;        % Don't reset
