...
%THISFILTER  Execute the APRU adaptive filter.
%   Y = THISFILTER(H,X,D) applies an APRU adaptive filter H to the input
...
[N,tmp] = size(h.InvOffsetCov);%  projection order
...
P = h.InvOffsetCov;         %  initialize input covariance matrix inverse
RR = h.CorrelationCoeffs;   %  initialize correlation vector
...
for n=1:ntr
    ...
    %  Update input covariance matrix inverse and correlation vector
    ...
    RR = RR + XN(1)*XN(nnNp1)'; %  update correlation vector
    
    %  Compute output and error signals
    
    XL = X(nL);                %  select most-recent L input signal samples
    y(n) = W*XL + RR*Eps(nnN); %  compute current output signal sample
    ...
    Eps = [0;Eps(nnN)] + P*E;  %  compute temporary epsilon signal buffer
    
    %  Compute alternative coefficient updates
    
    W = W + Eps(N)*X(nLpNm1)'; %  update alternative coefficient vector
    
    %  "Down-date" input covariance matrix inverse and correlation vector
    ...
    invden = 1/(-1 + XP*XN);
    K = invden*PX;              %  compute Kalman gain vector
    P = P - K*XP;               %  "down-date" inverse covariance matrix
    RR = RR - XN(1)*XN(nnNp1)'; %  "down-date" correlation vector
...
h.InvOffsetCov = P;         %  save final input covariance matrix inverse
h.CorrelationCoeffs = RR;   %  save final correlation vector
