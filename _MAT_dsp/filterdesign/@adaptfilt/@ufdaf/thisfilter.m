...
%THISFILTER  Execute the unconstrained frequency-domain FIR adaptive filter.
%   Y = THISFILTER(H,X,D) applies a UFDAF adaptive filter H to the input
...
if (rem(length(x),h.BlockLength)~=0),
    error(message('dsp:adaptfilt:ufdaf:thisfilter:InvalidBlockLength'));
end
