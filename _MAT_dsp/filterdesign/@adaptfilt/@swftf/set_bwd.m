...
if ~isstruct(bwd),
    error(message('dsp:adaptfilt:swftf:set_bwd:InvalidParam1'));
end

L = get(h,'FilterLength');

if length(bwd.Coeffs) ~= L,
    error(message('dsp:adaptfilt:swftf:set_bwd:InvalidDimensions'));
end
...
if length(bwd.Error) ~= 1,
    error(message('dsp:adaptfilt:swftf:set_bwd:InvalidParam2'));
end

if bwd.Error < 0
    error(message('dsp:adaptfilt:swftf:set_bwd:MustBePositive'));
end
