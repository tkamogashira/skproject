...
if ~isstruct(fwd),
    error(message('dsp:adaptfilt:swftf:set_fwd:InvalidParam1'));
end

L = get(h,'FilterLength');

if length(fwd.Coeffs) ~= L,
    error(message('dsp:adaptfilt:swftf:set_fwd:InvalidDimensions'));
end
...
if length(fwd.Error) ~= 1,
    error(message('dsp:adaptfilt:swftf:set_fwd:InvalidParam2'));
end

if fwd.Error < 0
    error(message('dsp:adaptfilt:swftf:set_fwd:MustBePositive'));
end
