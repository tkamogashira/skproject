...
if ~isstruct(FwdPrediction),
    error(message('dsp:adaptfilt:lsl:set_fwd:InvalidParam'));
end

L = get(h,'FilterLength');

if length(FwdPrediction.Coeffs) ~= L-1,
    error(message('dsp:adaptfilt:lsl:set_fwd:InvalidDimensions1'));
end
...
if length(FwdPrediction.Error) ~= L,
    error(message('dsp:adaptfilt:lsl:set_fwd:InvalidDimensions2'));
end
...
if any(FwdPrediction.Error <= 0)
    error(message('dsp:adaptfilt:lsl:set_fwd:MustBePositive'));
end
