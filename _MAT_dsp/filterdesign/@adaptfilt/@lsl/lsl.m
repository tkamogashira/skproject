function h = lsl(varargin)
%LSL  Least-squares lattice adaptive filter.
%   H = ADAPTFILT.LSL(L,LAMBDA,DELTA,COEFFS,STATES) constructs a
%   Least-squares lattice adaptive filter H.
%
%   L is the length of the joint process filter coefficients. It must be
%   a positive integer and must be equal to the length of the prediction
...
%   
%   DELTA is the soft-constrained initialization factor in the least-squares 
%   lattice algorithm. It should be positive. DELTA defaults to 1.
%   
...
%   adaptive filter. STATES defaults to a length L vector of all zeros
...
h = adaptfilt.lsl;
construct(h,[0 5],'Least-Squares Lattice Adaptive Filter',varargin{:});
