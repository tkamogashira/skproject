...
%   B = FIRCEQRIP(N,Fo,DEV) designs length N+1 filter coefficients for a
%   lowpass linear-phase FIR filter with cut-off frequency Fo. The
%   2-element vector DEV = [d1 d2] specifies the maximum deviation or
%   ripple (in linear units) in the pass-band and stop-band. At the cut-off
%   frequency Fo the frequency response magnitude is 0.5. Fo should be
%   between 0 and 1 (0 < Fo < 1).
%
%   This function produces the same equiripple lowpass filters as the FIRPM
%   (Parks-McClellan) algorithm, however, the specifications are supplied
%   differently. 
%
%   B = FIRCEQRIP(...,'slope',R) designs a filter with a nonequiripple
%   stop-band. R is the attenuation slope dB/(pi rad/sample) (R > 0) [try R
%   = 10].
%
%   B = FIRCEQRIP(...,'passedge') designs a filter where Fo is the passband
%   edge. B = FIRCEQRIP(...,'stopedge') designs a filter where Fo is the
%   stopband edge.
%
%   B = FIRCEQRIP(...,'high') designs a highpass filter.
%
%   B = FIRCEQRIP(...,'min') designs a minimum-phase FIR filter.
%
%   B = FIRCEQRIP(...,'invsinc',C) designs a lowpass filter with a
%   pass-band having the shape of an inverse-sinc function, 1/sinc(C*f). If
%   C is a two-element vector, C = [c p], then the shape of the pass-band
%   is 1/sinc(c*f)^p. [c < 1/wo].
%  
%   B = FIRCEQRIP(...,'invdiric',C) designs a lowpass filter with a
%   pass-band having the shape of an inverse-Dirichlet-sinc function
%   C*sin(f/2)/sin(C*f/2). If C is a two-element vector, C = [c p], then
%   the shape of the pass-band is [c*sin(f/2)/sin(c*f/2)]^p. If C is a
%   three-element vector, C = [c p r], then the shape of the pass-band is
%   [r*c*sin(f/(2*r))/sin(c*f/2)]^p.
%
%   Several trailing arguments can be used simultaneously. The order of the
%   optional trailing arguments is not important.
%
...
%
%   % Examples
%   b = firceqrip(30,0.4,[0.05 0.03]); fvtool(b,'MagnitudeDisplay','Magnitude')
%   b = firceqrip(30,0.4,[0.05 0.03]); fvtool(b,'MagnitudeDisplay','Zero-phase')
...
%   b = firceqrip(30,0.4,[0.05 0.03],'invdiric',[2 1.5 4]); fvtool(b)
%
...
%   vol. 44, no. 2, pp. 137-143, February 1997. 
...
IST = 'invsinc';       % IST: Inverse-Sinc type
R  = 1;                % CIC rate change factor
C = 0;
pow = 1;
...
tw = exp(pi*(N-1)/2*1i*f');         % (for plotting)
...
for k = 1:7
    ...
    % H is not used, but we aren't sure why its here, so we're going to
    % leave it in, but commented out.
%     H = cos(w*v) * a;
    H1 = -sin(w*v) * (v'.*a);
    H2 = -cos(w*v) * ((v.^2)'.*a);
    w = w - H1./H2;
...
if strcmpi(IST,'invsinc')
  % Inverse sinc response
  % C is the sinc frequency factor
  x = x*C*pi;
  y = ones(size(x));
  i = find(x & sin(x));
  y(i) = (x(i))./sin(x(i));
elseif strcmpi(IST,'invdiric')
  % Inverse Dirichlet sinc  
  x = x*pi;
  y = ones(size(x));
