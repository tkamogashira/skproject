...
while loopcounter <= length(varargin)
    ...
    if ischar(value)
        ...
        switch lower(property)
            ...
            case readonlyproperties
                warning(message('dsp:qfft:set:ReadOnly', property));
            otherwise
                if ~isempty(ind)
                    error(message('dsp:qfft:set:FilterErr1', property, allproperties{ ind }( 1:end - 1 )));
                else
                    error(message('dsp:qfft:set:FilterErr2', property));
                end
    ...
    elseif isa(value,'cell')
        ...
        if loopcounter>length(varargin)
            error(message('dsp:qfft:set:InvalidParam'));
        end
...
[msgObj,F.scalevalues] = scalevaluescheck(F);
if ~isempty(msgObj),
    warning(msgObj);
end
...
loopcounter = loopcounter+1;
if loopcounter > length(varargin)
    error(message('dsp:qfft:set:FixedPtErr'));
end
switch property
...
switch lower(property)
    ...
    case 'scalevalues'
        [msgObj,F.scalevalues] = scalevaluescheck(F,value);
        if ~isempty(msgObj),
            error(msgObj);
        end
        if prod(size(value))~=1
            if length(value) ~= numberofsections(F)
                warning(message('dsp:qfft:set:InvalidDimensions'));
            end
        end
  end
...
switch value
    ...
    otherwise
        ...
        error(message('dsp:qfft:set:InvalidEnum', value, property));
end
