...
switch ffttype
  case 'ifft'
    ...
    % FFT algorithm for the IFFT, and then conjugate the output.
    x = conj(x);
...
warning(warn)
nover = noverflows(F);
wrec = warning('query', 'dsp:qfft:Overflows');
if nover>0 && ~strcmpi(wrec.state, 'off')
  warning(message('dsp:qfft:Overflows',num2str(nover), ffttype));
  qfftreport(F)
end
