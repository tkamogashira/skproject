...
%      Copyright (C) Mike Brookes 2009-2016
%      Version: $Id: psycest.m 7501 2016-02-21 17:45:33Z dmb $
%
...
% mq0(2,1)      = prior means [x-mean; s-mean]
% pq0(2,1)      = scale factors for prior distribution
...
if iq<0  % initialization
    ...
    wq=repmat(pq0(2)*(sq-mq0(2)).^2,1,nxq)+repmat(pq0(1)*(xq-mq0(1))'.^2,nsq,1); % log probs of prior
    ...
    xmm=[Inf(1,ni); -Inf(1,ni)];  % min/max of probe values for each model
    if any(qr(1,:)<=0)
        [dum,i]=min(qr(1,:));
...
elseif iq>0
    ...
    if  nres(iq)<2              % keep old limits if nres(iq)<2
        xq2lim=xqi([1 end]);
        sq2lim=sqi([1 end]);
    else
    ...
    if xq2(1)<xqi(1) || xq2(end)>xqi(end) || sq2(1)<sqi(1) || sq2(end)>sqi(end)
        ...
        wq2=repmat(pq0(2)*(sq2-mq0(2)).^2,1,nxq)+repmat(pq0(1)*(xq2-mq0(1))'.^2,nsq,1); % log probs of prior
            ...
            for jq=1:length(nresq)             % loop for each model
                if any(res(nresr+1:nres),1)==jq   % if we have had any recent probe results for this model
                    ...
                    if nr(19)>guessj  % find lower bound for SNR with r=1
                        switch nr(10)   % switch on model type
                            case 1
                                xlim(1,jq)=xe-(xtstdj+log(pscalej/(nr(19)-guessj)-1))/sqisj; % outlier if r=1 and x < xlim
                            case 2
                                xlim(1,jq)=xe+(norminv((nr(19)-guessj)/pscalej)-xtstdj)/sqisj; % outlier if r=1 and x < xlim
                        end
                    ...
                    if any(mou) % if any results must be excluded
                        ...
                        wqj=repmat(pq0(2)*(sqj-mq0(2)).^2,1,nxq)+repmat(pq0(1)*(xqj-mq0(1))'.^2,nsq,1); % log probs of prior
                        switch nr(10)               % switch on model type
                        ...
                        xmr=(2-xmr)*xqj(1)+(xmr-1)*xqj(2);             % marginal mode(x0) ====== this assumes that the xqi are uniformly spaced
                        [pspk,smr]=max(psr);
                        ...
                        mqr(:,jq,1)=[xer; ser];                   	% save means
                        mqr(:,iq,2)=[xjr; sjr];                    	% save joint mode
                        mqr(:,iq,3)=[xmr; smr];                    	% save marginal modes
                        vqr(:,iq)=[xvr; sxvr; svr];                	% save covariance matrix
            ...
            switch poi
                ...
                case 'h' % plot history
                    ...
                    if nresi>0
                        ...
                        msky=resi(:,3)==1;
                        plot(find(msky),res(msky,2),'rx');
                        msky=resi(:,3)==0;
                        plot(find(msky),res(msky,2),'bo');
                        ...
                        axisenlarge(-1.02);
                        if poutb
                            title('SRT mean + slope intercepts [red=outlier bounds]');
                        else
                ...
                case 'x' % plot probe choice
                    ...
                    if nr(18) % if doing look 2-ahead
                        ...
                        imagesc([2*xt(1)-xt(2) xt],xt,[hminj' hx2e]);
                        axis xy;
                        colorbar;
                        hold on
                        plot(2*xt(1)-xt(2),xn(iq),'*w');
                        plot(xt(ix2m(:,1,1)),xt,'ow');
                        plot(xt(ix2m(:,1,2)),xt,'xw');
                        hold off
                        xlabel('SNR probe 2 (dB) [x,o=best for r1=1,0]');
                        ylabel(sprintf('SNR probe 1 (dB) [* = %.2f]',xn(iq)));
                        title('Expected Cost after next 2 probes');
